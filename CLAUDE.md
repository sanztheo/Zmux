...
```
App path:
  /Users/someone/Library/Developer/Xcode/DerivedData/zmux-my-tag/Build/Products/Debug/zmux DEV my-tag.app
```

**Claude Code** outputs:
```markdown
=======================================================
[zmux DEV my-tag.app](file:///Users/someone/Library/Developer/Xcode/DerivedData/zmux-my-tag/Build/Products/Debug/zmux%20DEV%20my-tag.app)
=======================================================
```
...
```
=======================================================
[my-tag: file:///Users/someone/Library/Developer/Xcode/DerivedData/zmux-my-tag/Build/Products/Debug/zmux%20DEV%20my-tag.app](file:///Users/someone/Library/Developer/Xcode/DerivedData/zmux-my-tag/Build/Products/Debug/zmux%20DEV%20my-tag.app)
=======================================================
```

Never use `/tmp/zmux-<tag>/...` app links in chat output.

After making code changes, always use `reload.sh --tag` to build. **Never run bare `xcodebuild` or `open` an untagged `zmux DEV.app`.** Untagged builds share the default debug socket and bundle ID with other agents, causing conflicts and stealing focus.
...
```bash
xcodebuild -project GhosttyTabs.xcodeproj -scheme zmux -configuration Debug -destination 'platform=macOS' -derivedDataPath /tmp/zmux-<your-tag> build
```
...
When rebuilding zmuxd for release/bundling, always use ReleaseFast:

```bash
cd zmuxd && zig build -Doptimize=ReleaseFast
```
...
`reloads` = kill and launch the Release app as "zmux STAGING" (isolated from production zmux):
...
Cloud VM build, test, and local dev scripts use provider secrets from `~/.secrets/zmux.env`.
...
```bash
set -a
source ~/.secrets/zmux.env
set +a
```

`~/.secrets/zmuxterm-dev.env` is for local Stack/web env and does not contain the provider build keys.
`bun dev` sources `~/.secrets/zmux.env` first when present, then `~/.secrets/zmuxterm-dev.env` so
zmuxterm-specific Stack settings override broader zmux secrets. The web dev loader still accepts
the legacy `~/.secret/zmuxterm.env` and `~/.secrets/zmuxterm.env` paths while machines migrate.
...
OIDC/RDS IAM path. Runtime env names are `ZMUX_DB_DRIVER=aws-rds-iam`, `AWS_ROLE_ARN`,
...
Local development keeps using the `ZMUX_PORT`-derived Docker Postgres path from `bun dev`.
...
```bash
tail -f "$(cat /tmp/zmux-last-debug-log-path 2>/dev/null || echo /tmp/zmux-debug.log)"
```

- Untagged Debug app: `/tmp/zmux-debug.log`
- Tagged Debug app (`./scripts/reload.sh --tag <tag>`): `/tmp/zmux-debug-<tag>.log`
- `reload.sh` writes the current path to `/tmp/zmux-last-debug-log-path`
- `reload.sh` writes the selected dev CLI path to `/tmp/zmux-last-cli-path`
- `reload.sh` updates `/tmp/zmux-cli` and `$HOME/.local/bin/zmux-dev` to that CLI

- Implementation: `Packages/ZMUXDebugLog/Sources/ZMUXDebugLog/DebugEventLog.swift`
- App shim: `Sources/App/DebugLogging.swift`
- Free function `zmuxDebugLog("message")` — logs with timestamp and appends to file in real time from zmux code
- The package implementation and app shim are `#if DEBUG`; all call sites must be wrapped in `#if DEBUG` / `#endif`
- 500-entry ring buffer; `ZMUXDebugLog.DebugEventLog.shared.dump()` writes full buffer to file
...
- To add a debug toggle or visual option: create an `NSWindowController` subclass with a `shared` singleton, add it to the "Debug Windows" menu in `Sources/zmuxApp.swift`, and add a SwiftUI view with `@AppStorage` bindings for live changes.
- When the user says "debug menu" or "debug window", they mean this menu, not `defaults write`.

## Pitfalls

- **Custom UTTypes** for drag-and-drop must be declared in `Resources/Info.plist` under `UTExportedTypeDeclarations` (e.g. `com.splittabbar.tabtransfer`, `com.zmux.sidebar-tab-reorder`).
...
- **E2E / UI tests:** trigger via `gh workflow run test-e2e.yml` (see zmuxterm-hq CLAUDE.md for details)
- **Unit tests:** `xcodebuild -scheme zmux-unit` is safe (no app launch), but prefer CI
- **Python socket tests (tests_v2/):** these connect to a running zmux instance's socket. Never launch an untagged `zmux DEV.app` to run them. If you must test locally, use a tagged build's socket (`/tmp/zmux-debug-<tag>.sock`) with `ZMUX_SOCKET=/tmp/zmux-debug-<tag>.sock`
- **Never `open` an untagged `zmux DEV.app`** from DerivedData. It conflicts with the user's running debug instance.
...
gh run watch --repo manaflow-ai/zmux
```

Notes:
- Requires GitHub secrets: `APPLE_CERTIFICATE_BASE64`, `APPLE_CERTIFICATE_PASSWORD`,
  `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, `APPLE_TEAM_ID`.
- The release asset is `zmux-macos.dmg` attached to the tag.
- README download button points to `releases/latest/download/zmux-macos.dmg`.
