---
name: zmux-debug-windows
description: Manage zmux debug windows and related debug menu wiring for Sidebar Debug, Background Debug, and Menu Bar Extra Debug. Use this when the user asks to open/tune these debug controls, add or adjust Debug menu entries, or capture/copy a combined debug config snapshot.
---
...
1. Verify debug menu wiring in `Sources/zmuxApp.swift` under `CommandMenu("Debug")`.
...
```bash
skills/zmux-debug-windows/scripts/debug_windows_snapshot.sh --copy
```
5. After code edits, run build + tagged reload:
```bash
xcodebuild -project GhosttyTabs.xcodeproj -scheme zmux -configuration Debug -destination 'platform=macOS' build
./scripts/reload.sh --tag <tag>
```

## Key Files

- `Sources/zmuxApp.swift`: Debug menu entries and debug window controllers/views.
...
Examples:
```bash
skills/zmux-debug-windows/scripts/debug_windows_snapshot.sh
skills/zmux-debug-windows/scripts/debug_windows_snapshot.sh --copy
skills/zmux-debug-windows/scripts/debug_windows_snapshot.sh --domain <bundle-id> --copy
```
