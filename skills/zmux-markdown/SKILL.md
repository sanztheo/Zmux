...
# Absolute path
zmux markdown open /path/to/PLAN.md

# Target a specific workspace
zmux markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in zmux:
...
```bash
# Open in the caller's workspace (default -- uses ZMUX_WORKSPACE_ID)
zmux markdown open plan.md

# Open in a specific workspace
zmux markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
zmux markdown open plan.md --surface surface:5

# Open in a specific window
zmux markdown open plan.md --window window:1
```
