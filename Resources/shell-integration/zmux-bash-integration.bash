# zmux shell integration for bash

# Cache which send tool is available to avoid repeated PATH lookups.
_ZMUX_SEND_TOOL=""
_zmux_detect_send_tool() {
    if command -v ncat >/dev/null 2>&1; then
        _ZMUX_SEND_TOOL=ncat
    elif command -v socat >/dev/null 2>&1; then
        _ZMUX_SEND_TOOL=socat
    elif command -v nc >/dev/null 2>&1; then
        _ZMUX_SEND_TOOL=nc
    fi
}
# Detection deferred to after _zmux_fix_path (end of file).

_zmux_send() {
    local payload="$1"
    case "$_ZMUX_SEND_TOOL" in
        ncat)
            printf '%s\n' "$payload" | ncat -w 1 -U "$ZMUX_SOCKET_PATH" --send-only
            ;;
        socat)
            printf '%s\n' "$payload" | socat -T 1 - "UNIX-CONNECT:$ZMUX_SOCKET_PATH" >/dev/null 2>&1
            ;;
        nc)
            if printf '%s\n' "$payload" | nc -N -U "$ZMUX_SOCKET_PATH" >/dev/null 2>&1; then
                :
            else
                printf '%s\n' "$payload" | nc -w 1 -U "$ZMUX_SOCKET_PATH" >/dev/null 2>&1 || true
            fi
            ;;
    esac
}
...
_zmux_restore_scrollback_once
_ZMUX_CLAUDE_WRAPPER="${_ZMUX_CLAUDE_WRAPPER:-}"
...
_ZMUX_PWD_LAST_PWD="${_ZMUX_PWD_LAST_PWD:-}"
_ZMUX_GIT_LAST_PWD="${_ZMUX_GIT_LAST_PWD:-}"
_ZMUX_GIT_LAST_RUN="${_ZMUX_GIT_LAST_RUN:-0}"
_ZMUX_GIT_JOB_PID="${_ZMUX_GIT_JOB_PID:-}"
_ZMUX_GIT_JOB_STARTED_AT="${_ZMUX_GIT_JOB_STARTED_AT:-0}"
_ZMUX_GIT_HEAD_LAST_PWD="${_ZMUX_GIT_HEAD_LAST_PWD:-}"
_ZMUX_GIT_HEAD_PATH="${_ZMUX_GIT_HEAD_PATH:-}"
_ZMUX_GIT_HEAD_SIGNATURE="${_ZMUX_GIT_HEAD_SIGNATURE:-}"
_ZMUX_PR_POLL_PID="${_ZMUX_PR_POLL_PID:-}"
_ZMUX_PR_POLL_PWD="${_ZMUX_PR_POLL_PWD:-}"
_ZMUX_PR_LAST_BRANCH="${_ZMUX_PR_LAST_BRANCH:-}"
_ZMUX_PR_NO_PR_BRANCH="${_ZMUX_PR_NO_PR_BRANCH:-}"
_ZMUX_PR_POLL_INTERVAL="${_ZMUX_PR_POLL_INTERVAL:-45}"
_ZMUX_PR_FORCE="${_ZMUX_PR_FORCE:-0}"
_ZMUX_PR_DEBUG="${_ZMUX_PR_DEBUG:-0}"
_ZMUX_ASYNC_JOB_TIMEOUT="${_ZMUX_ASYNC_JOB_TIMEOUT:-20}"
_ZMUX_LAST_PR_ACTION="${_ZMUX_LAST_PR_ACTION:-}"
_ZMUX_LAST_PR_TARGET="${_ZMUX_LAST_PR_TARGET:-}"

_ZMUX_PORTS_LAST_RUN="${_ZMUX_PORTS_LAST_RUN:-0}"
_ZMUX_SHELL_ACTIVITY_LAST="${_ZMUX_SHELL_ACTIVITY_LAST:-}"
_ZMUX_TTY_NAME="${_ZMUX_TTY_NAME:-}"
_ZMUX_TTY_REPORTED="${_ZMUX_TTY_REPORTED:-0}"
_ZMUX_TMUX_PUSH_SIGNATURE="${_ZMUX_TMUX_PUSH_SIGNATURE:-}"
_ZMUX_TMUX_PULL_SIGNATURE="${_ZMUX_TMUX_PULL_SIGNATURE:-}"
_ZMUX_TMUX_SYNC_KEYS=(
    ZMUX_BUNDLED_CLI_PATH
    ZMUX_BUNDLE_ID
    ZMUXD_UNIX_PATH
    ZMUXTERM_REPO_ROOT
...
_ZMUX_TMUX_SURFACE_SCOPED_KEYS=(
    ZMUX_PANEL_ID
    ZMUX_SURFACE_ID
)
...
_zmux_report_shell_activity_state() {
    ...
    {
        _zmux_send "report_shell_state $state --tab=$ZMUX_TAB_ID --panel=$ZMUX_PANEL_ID"
    } >/dev/null 2>&1 & disown
}
...
_zmux_ports_kick() {
    ...
    if _zmux_socket_is_unix; then
        {
            _zmux_send "ports_kick --tab=$ZMUX_TAB_ID --panel=$ZMUX_PANEL_ID --reason=$reason"
        } >/dev/null 2>&1 & disown
    else
...
_zmux_clear_pr_for_panel() {
    ...
    _zmux_send "clear_pr --tab=$ZMUX_TAB_ID --panel=$ZMUX_PANEL_ID"
}
...
_zmux_pr_cache_prefix() {
    ...
    printf '%s\n' "/tmp/zmux-pr-cache-${ZMUX_PANEL_ID}"
}

_zmux_pr_force_signal_path() {
    [[ -n "$ZMUX_PANEL_ID" ]] || return 1
    printf '%s\n' "/tmp/zmux-pr-force-${ZMUX_PANEL_ID}"
}
...
_zmux_start_pr_poll_loop() {
    ...
    _ZMUX_PR_POLL_PID=$!
    disown "$_ZMUX_PR_POLL_PID" 2>/dev/null || disown
}
...
_zmux_prompt_command() {
    ...
    if (( ! zmux_has_unix_socket )); then
        if (( now - _ZMUX_PORTS_LAST_RUN >= 10 )); then
    ...
    if [[ "$pwd" != "$_ZMUX_PWD_LAST_PWD" ]]; then
        ...
        {
            local qpwd="${pwd//\"/\\\"}"
            _zmux_send "report_pwd \"${qpwd}\" --tab=$ZMUX_TAB_ID --panel=$ZMUX_PANEL_ID"
        } >/dev/null 2>&1 & disown
    fi
    ...
    if [[ -n "$_ZMUX_GIT_JOB_PID" ]] && kill -0 "$_ZMUX_GIT_JOB_PID" 2>/dev/null; then
        if [[ "$pwd" != "$_ZMUX_GIT_LAST_PWD" || "$git_head_changed" == "1" ]]; then
    ...
    if [[ -z "$_ZMUX_GIT_JOB_PID" ]] || ! kill -0 "$_ZMUX_GIT_JOB_PID" 2>/dev/null; then
...
_zmux_install_prompt_command() {
    [[ -n "${_ZMUX_PROMPT_INSTALLED:-}" ]] && return 0
    _ZMUX_PROMPT_INSTALLED=1
    ...
    if [[ "$decl" == "declare -a"* ]]; then
        ...
        for item in "${PROMPT_COMMAND[@]}"; do
            [[ "$item" == "_zmux_prompt_command" ]] && existing=1 && break
        done
        if (( existing == 0 )); then
            PROMPT_COMMAND=("_zmux_prompt_command" "${PROMPT_COMMAND[@]}")
        fi
    else
        case ";$PROMPT_COMMAND;" in
            ...
            *)
                if [[ -n "$PROMPT_COMMAND" ]]; then
                    ...
                    PROMPT_COMMAND="_zmux_prompt_command"
                fi
                ;;
        esac
    fi
        ...
        if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
            builtin readonly _ZMUX_BASH_PS0='${ _zmux_bash_preexec_hook "$BASH_COMMAND"; }'
        else
            builtin readonly _ZMUX_BASH_PS0='$(_zmux_bash_preexec_hook "$BASH_COMMAND" >/dev/null)'
        fi
        if [[ "$PS0" != *"${_ZMUX_BASH_PS0}"* ]]; then
            PS0=$PS0"${_ZMUX_BASH_PS0}"
        fi
    fi
}
