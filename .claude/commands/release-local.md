...
If the script fails, run `say "zmux release failed"`.

### 7. Verify homebrew cask

- Run `bash tests/test_homebrew_sha.sh` to confirm the cask SHA matches the release DMG
- Update the homebrew-zmux submodule pointer: `git add homebrew-zmux && git commit -m "Update homebrew-zmux submodule to latest" && git push origin main`
