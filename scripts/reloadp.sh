...
xcodebuild -project GhosttyTabs.xcodeproj -scheme zmux -configuration Release -destination 'platform=macOS' build
pkill -x zmux || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/zmux.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "zmux.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/zmux"
ATTEMPT=0
MAX_ATTEMPTS=20
