...
# This runner is intended for the UTM macOS VM (ssh zmux-vm).
# It is intentionally guarded so we don't accidentally kill the host user's zmux instances.
if [ "$(id -un)" != "zmux" ]; then
  echo "ERROR: This script is intended to be run on the zmux-vm (user: zmux)." >&2
  echo "Run via: ssh zmux-vm 'cd /Users/zmux/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/zmux-tests-v1"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/zmux DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: zmux DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "zmux DEV" || true
  pkill -x "zmux" || true
  rm -f /tmp/zmux*.sock || true
}
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "zmux DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.zmuxterm.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  ZMUX_TAG="$RUN_TAG" ZMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/zmux DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/zmux-debug*.sock /tmp/zmux*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/zmux*.sock)" >&2
    exit 1
  fi
  export ZMUX_SOCKET_PATH="$SOCK"
  export ZMUX_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  ZMUX_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from zmux import zmux  # type: ignore
