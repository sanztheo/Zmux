...
Go remote daemon for `zmux ssh` bootstrap, capability negotiation, and remote proxy RPC. It is not in the terminal keystroke hot path.

## Commands

1. `zmuxd-remote version`
2. `zmuxd-remote serve --stdio`
3. `zmuxd-remote serve --ws --auth-lease-file <path> [--rpc-auth-lease-file <path>] [--listen 127.0.0.1:7777]`
4. `zmuxd-remote cli <command> [args...]` — relay zmux commands to the local app over the reverse SSH forward

`serve --ws` is explicit opt-in for cloud VM images only. The normal `zmux ssh`
code path continues to use `serve --stdio` over an SSH exec channel and does not
open a WebSocket listener.

When invoked as `zmux` (via wrapper/symlink installed during bootstrap), the binary auto-dispatches to the `cli` subcommand. This is busybox-style argv[0] detection.
...
Release and nightly builds publish prebuilt `zmuxd-remote` binaries on GitHub Releases for:
1. `darwin/arm64`
2. `darwin/amd64`
3. `linux/arm64`
4. `linux/amd64`
...
Release and nightly apps download and cache the matching binary locally, verify its SHA-256, then upload it to the remote host if needed. Dev builds can opt into a local `go build` fallback with `ZMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1`.

To inspect what a given app build trusts, run:
1. `zmux remote-daemon-status`
2. `zmux remote-daemon-status --os linux --arch amd64`
...
The `cli` subcommand (or `zmux` wrapper/symlink) connects to the local zmux app through an SSH reverse forward and relays commands. It supports both v1 text protocol and v2 JSON-RPC commands.

Socket discovery order:
1. `--socket <path>` flag
2. `ZMUX_SOCKET_PATH` environment variable
3. `~/.zmux/socket_addr` file (written by the app after the reverse relay establishes)

For TCP addresses, the CLI dials once and only refreshes `~/.zmux/socket_addr` a single time if the first address was stale. Relay metadata is published only after the reverse forward is ready, so steady-state use does not rely on polling.
...
3. The remote shell never gets direct access to the local app socket. It only gets the reverse-forwarded relay port plus `~/.zmux/relay/<port>.auth`, which is written with `0600` permissions and removed when the relay stops.

Integration additions for the relay path:

1. Bootstrap installs `~/.zmux/bin/zmux` wrapper and keeps a default daemon target (`~/.zmux/bin/zmuxd-remote-current`).
2. A background `ssh -N -R` process reverse-forwards a TCP port to the authenticated local relay server. The relay address is written to `~/.zmux/socket_addr` on the remote.
3. Relay startup writes `~/.zmux/relay/<port>.daemon_path` so the wrapper can route each shell to the correct daemon binary when multiple local zmux instances or versions coexist.
4. Relay startup writes `~/.zmux/relay/<port>.auth` with the relay ID and token needed for HMAC authentication.
