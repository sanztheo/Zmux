...
func TestWebSocketPTYRequiresSessionMatchAndConsumesLeaseOnce(t *testing.T) {
	...
	writeTestLease(t, leasePath, "zmux-secret", "sess-good", true, time.Now().Add(time.Minute))
	conn := dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "zmux-secret", "sess-other", 80, 24)
	_, _, err := conn.Read(ctx)
	if websocket.CloseStatus(err) != websocket.StatusPolicyViolation {
	...
	conn = dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "zmux-secret", "sess-good", 100, 30)
...
func TestWebSocketPTYRunsShellOverBinaryFrames(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if msgType == websocket.MessageBinary {
			...
			if strings.Contains(output.String(), "ZMUX_WS_OK") {
...
func TestWebSocketPTYSeedsUTF8LocaleAndTerminalEnv(t *testing.T) {
	...
	command := "printf '%s\\n' \"$LANG|$LC_CTYPE|$LC_ALL|$TERM|$COLORTERM|$TERM_PROGRAM|$ZMUX_REMOTE_TRANSPORT\"; locale charmap; exit\r"
