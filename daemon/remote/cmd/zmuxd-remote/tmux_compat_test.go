...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/zmux-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"ZMUX_CLAUDE_TEAMS_ZMUX_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "ZMUX_SOCKET_PATH", "ZMUX_SOCKET", "TERM_PROGRAM",
		"ZMUX_WORKSPACE_ID", "ZMUX_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("ZMUX_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("ZMUX_SOCKET_PATH = %q", os.Getenv("ZMUX_SOCKET_PATH"))
	}
	...
	if os.Getenv("ZMUX_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("ZMUX_WORKSPACE_ID = %q", os.Getenv("ZMUX_WORKSPACE_ID"))
	}
	if os.Getenv("ZMUX_SURFACE_ID") != "surf-789" {
		t.Errorf("ZMUX_SURFACE_ID = %q", os.Getenv("ZMUX_SURFACE_ID"))
	}
	// Verify extra env
