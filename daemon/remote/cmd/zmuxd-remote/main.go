...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "zmuxd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  zmuxd-remote version")
	_, _ = fmt.Fprintln(w, "  zmuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  zmuxd-remote serve --ws --auth-lease-file <path> [--rpc-auth-lease-file <path>] [--listen 127.0.0.1:7777]")
	_, _ = fmt.Fprintln(w, "  zmuxd-remote cli <command> [args...]")
}
