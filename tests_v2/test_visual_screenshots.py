#!/usr/bin/env python3
"""
Visual Screenshot Tests for zmux
...
def take_screenshot(client: zmux, label: str) -> Optional[Screenshot]:
...
def capture_state(client: zmux) -> str:
    try:
...
def stamp_terminals(client: zmux, label: str) -> None:
    ...
    for h in terminal_surfaces:
        ...
        try:
            ...
            client.send_surface(idx, f"echo ZMUX_VIS {safe} surf={idx}\n")
        except Exception:
            pass


def capture(client: zmux, label: str):
...
def reset_workspace(client: zmux) -> zmux:
...
def surface_count(client: zmux) -> int:
    return len(client.list_surfaces())

def pane_count(client: zmux) -> int:
...
def wait_surface_count(client: zmux, expected: int, timeout: float = 3.0) -> bool:
    start = time.time()
...
def wait_url_contains(client: zmux, panel_id: str, needle: str, timeout: float = 8.0) -> bool:
...
def cleanup_workspaces(client: zmux):
...
def _verify_surface_responsive(client: zmux, surface_idx: int, marker: Path,
                                retries: int = 3) -> bool:
...
def verify_views_in_window(client: zmux, label: str = "", timeout: float = 5.0) -> Optional[str]:
...
def verify_all_responsive(client: zmux, label: str = "") -> Optional[str]:
    ...
    for idx, h in enumerate(terminal_surfaces):
        surface_idx = h["index"]
        marker = Path(tempfile.gettempdir()) / f"zmux_vis_{os.getpid()}_{idx}"
        try:
...
def test_a1_initial_state(client: zmux) -> StateChange:
...
def test_a2_split_right(client: zmux) -> StateChange:
...
def test_a3_split_down(client: zmux) -> StateChange:
...
def _close_and_verify(client: zmux, change: StateChange, close_idx: int,
...
def test_b4_close_right(client: zmux) -> StateChange:
...
def test_b5_close_left(client: zmux) -> StateChange:
...
def test_b6_close_bottom(client: zmux) -> StateChange:
...
def test_b7_close_top(client: zmux) -> StateChange:
...
def test_c8_3way_close_middle(client: zmux) -> StateChange:
...
def test_c9_grid_close_topleft(client: zmux) -> StateChange:
...
def test_c10_grid_close_bottomright(client: zmux) -> StateChange:
...
def test_d11_nested_close_bottomright(client: zmux) -> StateChange:
...
def test_d12_nested_close_top(client: zmux) -> StateChange:
...
def test_d13_4pane_close_second(client: zmux) -> StateChange:
...
def test_e14_browser_close_terminal(client: zmux) -> StateChange:
...
def test_e15_browser_close_browser(client: zmux) -> StateChange:
...
def test_f16_nested_tabs_close_first(client: zmux) -> StateChange:
...
def test_g17_rapid_down_close_top(client: zmux) -> StateChange:
...
def test_g18_rapid_right_close_left(client: zmux) -> StateChange:
...
def test_g19_alternating_close_reverse(client: zmux) -> StateChange:
...
def test_h20_workspace_switch_back(client: zmux) -> StateChange:
...
def _create_browser_surface(client: zmux, url: Optional[str] = None) -> str:
    return client.new_surface(panel_type="browser", url=url)


def test_i21_browser_drag_split_right_wait_load(client: zmux) -> StateChange:
...
def test_i22_browser_drag_split_right_immediate(client: zmux) -> StateChange:
...
def test_i23_browser_drag_split_right_webview_focused(client: zmux) -> StateChange:
...
def test_i24_browser_drag_split_right_focus_bounce(client: zmux) -> StateChange:
...
def test_i25_browser_drag_split_right_then_switch_panes(client: zmux) -> StateChange:
...
def test_i26_browser_drag_split_right_initial_url(client: zmux) -> StateChange:
...
def test_i27_browser_drag_split_right_after_reload(client: zmux) -> StateChange:
...
def test_i28_browser_drag_split_right_double_drag(client: zmux) -> StateChange:
...
<html>
<head>
    <title>zmux Visual Test Report</title>
    <style>
        body {
...
def run_visual_tests():
    ...
    print("=" * 60)
    print(f"zmux Visual Screenshot Tests ({len(test_fns)} scenarios)")
    print("=" * 60)
    print()
