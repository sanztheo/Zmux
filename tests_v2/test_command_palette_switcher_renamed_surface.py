...
def _rename_surface(client: zmux, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: zmux, workspace_id: str) -> str:
...
def main() -> int:
    with zmux(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise zmuxError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise zmuxError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise zmuxError("switcher returned no rows for renamed surface query")
