...
def _focused_surface_id(c: zmux) -> str:
    ...
    if not surfaces:
        raise zmuxError("Expected at least one terminal surface")
    return next((sid for _idx, sid, focused in surfaces if focused), surfaces[0][1])


def _surface_health_row(c: zmux, surface_id: str) -> Optional[dict]:
...
def _assert_surface_visible(c: zmux, surface_id: str, context: str) -> None:
    row = _surface_health_row(c, surface_id)
    if row is None:
        raise zmuxError(f"{context}: surface missing from health output: {surface_id}")
...
def _send_osc777_notification(c: zmux, surface_id: str, title: str, body: str) -> None:
...
def _wait_for_notification(c: zmux, title: str, surface_id: str) -> None:
    surface_id = surface_id.lower()
...
def _wait_for_terminal_text(c: zmux, surface_id: str, text: str) -> None:
    _wait_for(
...
def _assert_renders_after_notification(c: zmux, surface_id: str, marker: str) -> None:
...
def main() -> int:
    token = f"ZMUX_OSC777_{int(time.time() * 1000)}"
