...
def _wait_for_terminal_focus(c: zmux, panel_id: str, timeout_s: float = 4.0) -> None:
...
def _baseline_all(c: zmux, panel_ids: list[str], label: str) -> None:
    for pid in panel_ids:
        c.panel_snapshot(pid, label=f"{label}_base_{pid[:6]}")


def _after_all(c: zmux, panel_ids: list[str], label: str) -> dict[str, int]:
    diffs: dict[str, int] = {}
    for pid in panel_ids:
...
def _assert_routing(diffs: dict[str, int], target: str, *, min_changed: int = 250, ratio: float = 3.0) -> None:
    ...
    if tgt is None:
        raise zmuxError(f"missing diff for target {target}")
    # -1 means first diff or size mismatch; treat as failure here.
    if tgt < min_changed:
        raise zmuxError(f"target panel did not change enough (changed_pixels={tgt}): diffs={diffs}")
...
def main() -> int:
    with zmux(SOCKET_PATH) as c:
        ...
        if not surfaces0:
            raise zmuxError("expected initial surface")
        ...
        for pid in panel_ids:
            c.send_surface(pid, f"echo ZMUX_READY_{pid[:6]}\n")
        time.sleep(0.6)
        ...
        for i, target in enumerate(panel_ids):
            marker = f"ZMUX_ROUTE_{i}_{target[:6]}"
            ...
            if marker not in text:
                raise zmuxError(f"marker missing from read_terminal_text for {target}: {marker}")
