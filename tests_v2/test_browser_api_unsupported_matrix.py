...
def _expect_not_supported(c: zmux, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise zmuxError(f"Expected not_supported for {method}, got: {text}")
    raise zmuxError(f"Expected not_supported for {method}, but call succeeded")
