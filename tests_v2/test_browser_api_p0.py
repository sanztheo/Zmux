...
<!doctype html>
<html>
  <head><title>zmux-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("zmux" in str(out.get("value", "")), f"Expected #out text to include 'zmux': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "zmux", f"Expected eval value 'zmux': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("zmux-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on zmux webview with ref handles")
    return 0
