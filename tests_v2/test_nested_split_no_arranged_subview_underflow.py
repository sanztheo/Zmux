...
def main() -> int:
    with zmux(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise zmuxError(f"expected >=2 panes after first split, got {len(panes)}: {panes}")
        ...
        if underflows != 0:
            ...
            raise zmuxError(f"bonsplit arranged-subview underflow observed ({underflows}); screenshot: {shot}")
