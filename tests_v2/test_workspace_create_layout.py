...
def test_horizontal_split(c: zmux) -> None:
...
def test_nested_splits(c: zmux) -> None:
        ...
        ws = _create_and_get_id(c, {
            ...
            "layout": {
                "pane": {
                    "surfaces": [{
                        "type": "terminal",
                        "env": {"ZMUX_LAYOUT_TEST_TOKEN": token},
                    }],
                },
            },
        })
...
def test_surface_commands(c: zmux) -> None:
...
def test_multi_surface_pane(c: zmux) -> None:
...
def test_malformed_layout_rejected(c: zmux) -> None:
    ...
    try:
        ...
        raise zmuxError("Expected error for malformed layout, but call succeeded")
...
def test_empty_surfaces_rejected(c: zmux) -> None:
    ...
    try:
        ...
        raise zmuxError("Expected error for empty surfaces, but call succeeded")
    except zmuxError as e:
        if "invalid_params" in str(e) or "Invalid layout" in str(e):
            pass  # expected
        else:
...
def test_layout_overrides_initial_command_and_env(c: zmux) -> None:
    ...
    try:
        ...
        c.send_surface(
            surface_id,
            "printf 'LAYOUT_ENV=%s INITIAL_ENV=%s\\n' \"$ZMUX_LAYOUT_TEST_TOKEN\" \"$SHOULD_NOT_EXIST\"\\n",
        )
