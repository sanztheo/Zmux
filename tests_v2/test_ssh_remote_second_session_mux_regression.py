#!/usr/bin/env python3
"""Regression: opening a second `zmux ssh` workspace to the same host must not mux-refuse."""
...
def _wait_remote_ready(client: zmux, workspace_id: str, timeout: float = 20.0) -> None:
...
def _wait_text_contains(client: zmux, surface_id: str, needle: str, timeout: float = 8.0) -> str:
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
        ...
        if needle in last:
            return last
        time.sleep(0.1)
    raise zmuxError(f"Timed out waiting for {needle!r} in surface {surface_id}: {last[-800:]!r}")


def main() -> int:
    if not SSH_HOST:
        print("SKIP: set ZMUX_SSH_TEST_HOST to run second-session ssh mux regression")
        return 0
    ...
    try:
        with zmux(SOCKET_PATH) as client:
            ...
            _must(bool(first_workspace_id), f"first zmux ssh output missing workspace_id: {first}")
            ...
            _wait_text_contains(client, first_surface_id, "zmux in ~", timeout=12.0)

            second = _run_cli_json(cli, ["ssh", SSH_HOST])
            second_workspace_id = _workspace_id_from_payload(client, second)
            _must(bool(second_workspace_id), f"second zmux ssh output missing workspace_id: {second}")
            _must(
                second_workspace_id != first_workspace_id,
                f"second zmux ssh should create a distinct workspace: {first_workspace_id} vs {second_workspace_id}",
            )
            ...
            text = _wait_text_contains(client, second_surface_id, "zmux in ~", timeout=12.0)
            ...
            _must(
                not hits,
                "second zmux ssh session printed mux refusal text instead of starting cleanly: "
            ...
            _must(
                ...
                f"second zmux ssh session accepted corrupted input after startup: {text[-1200:]!r}",
            )
    finally:
        if workspace_ids:
            try:
    ...
    print("PASS: second zmux ssh session opens cleanly without mux refusal")
    return 0
