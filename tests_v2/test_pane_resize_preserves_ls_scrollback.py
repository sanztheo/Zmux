...
DEFAULT_SOCKET_PATHS = ["/tmp/zmux-debug.sock", "/tmp/zmux.sock"]
...
def _extract_segment_lines(
    ...
    *,
    ...
    if not saw_start:
        raise zmuxError(f"start marker not found in scrollback: {start_marker}")
    if require_end and not saw_end:
        raise zmuxError(f"end marker not found in scrollback: {end_marker}")
    return out


def _run_once(socket_path: str) -> int:
    workspace_id = ""
    fixture_dir = Path(tempfile.mkdtemp(prefix="zmux-ls-resize-regression-"))
    try:
        with zmux(socket_path) as client:
            ...
            start_marker = f"ZMUX_LS_SCROLLBACK_START_{secrets.token_hex(4)}"
            end_marker = f"ZMUX_LS_SCROLLBACK_END_{secrets.token_hex(4)}"
...
def main() -> int:
    env_socket = os.environ.get("ZMUX_SOCKET")
    if env_socket:
    ...
    if last_error is not None:
        raise last_error
    raise zmuxError("No socket candidates configured")
