...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["ZMUX_BUNDLE_ID"]?
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["ZMUX_SOCKET_PATH"] ?? processEnv["ZMUX_SOCKET"]
        self.workspaceId = processEnv["ZMUX_WORKSPACE_ID"]
        self.surfaceId = processEnv["ZMUX_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["ZMUX_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["ZMUX_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "zmux.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "zmux-cli", key: "component")
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if !tmpSockets.isEmpty {
            context["tmp_zmux_sockets"] = tmpSockets
        }
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "ZMUX_SOCKET_PATH/ZMUX_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func discoverSockets(in directory: String, limit: Int) -> [String] {
        ...
        for name in entries.sorted() {
            guard name.hasPrefix("zmux"), name.hasSuffix(".sock") else { continue }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.zmuxterm/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["ZMUX_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !overridePath.isEmpty {
            self.statePath = NSString(string: overridePath).expandingTildeInPath
        } else if let overrideDirectory = processEnv["ZMUX_AGENT_HOOK_STATE_DIR"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
enum SocketPasswordResolver {
    private static let service = "com.zmuxterm.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "zmux"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["ZMUX_SOCKET_PASSWORD"]) {
            return env
        }
    ...
    private static func keychainScope(
        ...
        if let tag = normalized(environment["ZMUX_TAG"]) {
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "zmux"
    private static let stableSocketFileName = "zmux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/zmux.sock"
    private static let fallbackSocketPath = "/tmp/zmux-debug.sock"
    private static let stagingSocketPath = "/tmp/zmux-staging.sock"
    private static let legacyLastSocketPathFile = "/tmp/zmux-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["ZMUX_TAG"]) {
            ...
            candidates.append("/tmp/zmux-debug-\(slug).sock")
            candidates.append("/tmp/zmux-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("zmux") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["ZMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    private static func relayCredentials(for endpoint: RelayEndpoint) throws -> RelayCredentials {
        ...
        if let relayID = trimmedEnvValue(environment["ZMUX_RELAY_ID"]),
           let relayTokenHex = trimmedEnvValue(environment["ZMUX_RELAY_TOKEN"]),
        ...
        let authURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent(".zmux/relay/\(endpoint.port).auth", isDirectory: false)
    ...
    private func authenticateRelay(credentials: RelayCredentials) throws {
        ...
        guard let challengeData = challengeLine.data(using: .utf8),
              ...
              (challenge["protocol"] as? String) == "zmux-relay-auth",
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "zmux app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.zmux.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.zmux.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct ZMUXCLI {
    ...
    private static let debugLastSocketHintPath = "/tmp/zmux-last-socket-path"
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/zmux-debug"),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = normalizedEnvValue(environment["ZMUX_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
        ...
        return "/tmp/zmux-debug.sock"
#else
        return "/tmp/zmux.sock"
#endif
    }
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["ZMUX_SOCKET_PATH", "ZMUX_SOCKET"] {
        ...
        // so help text is available even when zmux is not running.
        ...
        if command != "__tmux-compat",
           ...
           preSeparatorArgs.contains(where: { $0 == "--help" || $0 == "-h" }) {
            ...
            print("Unknown command '\(command)'. Run 'zmux help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside zmux
        // (before socket connection, so it doesn't fail when no socket exists)
        if command == "codex-hook" {
            guard ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] != nil else {
                print("{}")
                return
            }
        }

        // OpenCode hook handler: gracefully no-op when not inside zmux
        ...
        // Cursor hook handler: gracefully no-op when not inside zmux
        ...
        // Gemini hook handler: gracefully no-op when not inside zmux
        ...
        // Generic agent hook handlers: gracefully no-op outside zmux
        ...
        if command == "feed-hook" {
            guard ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"]?.isEmpty == false else {
                print("{}")
                return
            }
        }
        ...
        if command == "feed" {
            ...
            case "clear":
                ...
                return
            case "help", "--help", "-h":
                print("Usage: zmux feed clear [--yes]")
                return
            default:
        ...
        case "auth":
            ...
            case "login":
                ...
                if (statusBefore["signed_in"] as? Bool) == true {
                    ...
                    print("Already signed in\(email.map { " as \($0)" } ?? ""). Use `zmux auth logout` to sign out first.")
                    break
                }
                print("Opening sign-in popup on the zmux web app.")
                ...
                if (result["signed_in"] as? Bool) == true {
                    ...
                    print("Timed out waiting for sign-in. Run `zmux auth status` once you've finished in the popup.")
                } else {
                    print("Sign-in did not complete. Run `zmux auth status` to check.")
                }
            ...
            case "logout":
                ...
                if (result["signed_in"] as? Bool) != true {
                    ...
                    print("Sign-out requested but state hasn't cleared yet. Run `zmux auth status` to confirm.")
                }

            default:
                throw CLIError(message: "Usage: zmux auth <status|login|logout>")
            }
        ...
        case "vm", "cloud":
            ...
            case "ls", "list":
                ...
                if vms.isEmpty {
                    print("No cloud VMs. Try: zmux vm new")
                    break
                }
                for vm in vms {
            ...
            case "new", "create":
                ...
                // Stray positional args (e.g. a typo like `zmux vm new myvm`) previously fell
                ...
                // Create the VM then drop the user into a zmux-managed workspace. Freestyle
                // attaches over SSH; E2B attaches over zmuxd-remote WebSocket PTY.
            ...
            case "shell", "attach":
                guard let vmId = rest.first else {
                    throw CLIError(message: "Usage: zmux \(command) shell <id>")
                }
            ...
            case "rm", "destroy", "delete":
                guard let vmId = rest.first else {
                    throw CLIError(message: "Usage: zmux vm rm <id>")
                }
            ...
            case "ssh-info", "ssh":
                guard let vmId = rest.first else {
                    throw CLIError(message: "Usage: zmux \(command) ssh <id>")
                }
                ...
                if credKind == "password" {
                    ...
                    print("authorizedKey credential not yet supported by `zmux \(command) ssh`; raw response:")
                    print(jsonString(response))
                }
            ...
            case "exec":
                guard let vmId = rest.first else {
                    throw CLIError(message: "Usage: zmux vm exec <id> -- <command...>")
                }
                ...
                // dropped quoting so `zmux vm exec <id> -- printf '%s\n' "a b"` reached the
            ...
            default:
                throw CLIError(message: "Usage: zmux \(command) <ls|new|shell|rm|exec|ssh> [args...]")
            }

        case "rpc":
            guard let method = commandArgs.first?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !method.isEmpty else {
                throw CLIError(message: "Usage: zmux rpc <method> [json-params]")
            }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
            ...
            let callerSurfaceArg = explicitSurfaceArg == nil && preferTTYFallback == false && windowId == nil
                ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"]
                : nil
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"],
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: zmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: zmux markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: zmux markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: zmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: zmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in zmux by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["ZMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["ZMUX_TAB_ID"] ?? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"])
                : nil)
    ...
    struct SSHCommandOptions {
        ...
        /// True when the remote is a cloud VM with zmuxd-remote pre-baked in the image.
        /// Set by `zmux vm new/shell/attach`; false for plain `zmux ssh`.
    ...
    /// Generic "open a workspace, SSH into the remote, bootstrap zmuxd-remote, forward socket,
    /// drop the user in a shell" pipeline. The inner loop of `zmux ssh`; also called from
    /// `zmux vm new`/`shell`/`attach` so cloud VMs reuse the exact same bootstrap.
    private func runSSHWithOptions(
        ...
        // script is only meaningful if zmuxd-remote is participating. Let ssh open a plain
        ...
        if let vmIDForSplitAttach,
           sshOptions.skipDaemonBootstrap {
            let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "zmux")
        ...
        do {
            ...
            // `zmux ssh` is an explicit "open this remote workspace now" action,
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: zmux ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        var lines: [String] = [
            "zmux_workspace_id=\"${ZMUX_WORKSPACE_ID:-}\"",
            "zmux_surface_id=\"${ZMUX_SURFACE_ID:-}\"",
            "zmux_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "zmux_remote_bootstrap=\"$(printf %s \"$zmux_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$zmux_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "zmux_remote_bootstrap=\"$(printf '%s' \"$zmux_remote_bootstrap\" | sed \"s/__ZMUX_WORKSPACE_ID__/$zmux_workspace_id/g; s/__ZMUX_SURFACE_ID__/$zmux_surface_id/g\")\"",
            "if ! printf '%s' \"$zmux_remote_bootstrap\" | command \(installSSHPrefix) -T \(shellQuote(options.destination)) \(shellQuote(remoteBootstrapInstallCommand)); then",
            "  exit 1",
            "fi",
            "zmux_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "zmux_remote_command=\"$(printf '%s' \"$zmux_remote_command_template\" | sed \"s/__ZMUX_WORKSPACE_ID__/$zmux_workspace_id/g; s/__ZMUX_SURFACE_ID__/$zmux_surface_id/g\")\"",
        ]

        var sshInvocation = "command \(sessionSSHPrefix) -o \"RemoteCommand=$zmux_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func stagedRemoteBootstrapCommandShell(
        ...
        lines.append("/bin/sh \"$HOME/.zmux/relay/\(remoteRelayPort).bootstrap.sh\"")
        return lines.joined(separator: "\n")
    }
    ...
    private func remoteBootstrapInstallShell(remoteRelayPort: Int) -> String {
        [
            "set -eu",
            "umask 077",
            "zmux_bootstrap_path=\"$HOME/.zmux/relay/\(remoteRelayPort).bootstrap.sh\"",
            "mkdir -p \"$HOME/.zmux/relay\"",
            "cat > \"$zmux_bootstrap_path\"",
            "chmod 700 \"$zmux_bootstrap_path\" >/dev/null 2>&1 || true",
        ].joined(separator: "\n")
    }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(
        ...
        lines += [
            "zmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/zmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$zmux_tmp\" || { rm -f \"$zmux_tmp\"; exit 1; }",
            "chmod 700 \"$zmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$zmux_tmp\"",
            "zmux_status=$?",
            "rm -f \"$zmux_tmp\"",
            "exit $zmux_status",
    ...
    private func remoteBootstrapTTYCaptureLines(
        ...
        var lines: [String] = [
            "zmux_bootstrap_tty=\"$(tty 2>/dev/null || true)\"",
            "zmux_bootstrap_tty=\"${zmux_bootstrap_tty##*/}\"",
            "if [ -n \"$zmux_bootstrap_tty\" ] && [ \"$zmux_bootstrap_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.zmux/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$zmux_bootstrap_tty\" > \"$HOME/.zmux/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "  export ZMUX_BOOTSTRAP_TTY=\"$zmux_bootstrap_tty\"",
        ]

        if includeRelayRPC {
            lines += [
                "  zmux_relay_cli=\"$HOME/.zmux/bin/zmux\"",
                "  if [ ! -x \"$zmux_relay_cli\" ]; then zmux_relay_cli=\"$(command -v zmux 2>/dev/null || true)\"; fi",
                "  if [ -n \"$zmux_relay_cli\" ]; then",
                "    zmux_relay_report_tty='{\"workspace_id\":\"__ZMUX_WORKSPACE_ID__\",\"tty_name\":\"'$zmux_bootstrap_tty'\"}'",
                "    zmux_relay_ports_kick='{\"workspace_id\":\"__ZMUX_WORKSPACE_ID__\",\"reason\":\"command\"}'",
                "    if [ -n \"__ZMUX_SURFACE_ID__\" ]; then",
                "      zmux_relay_report_tty='{\"workspace_id\":\"__ZMUX_WORKSPACE_ID__\",\"surface_id\":\"__ZMUX_SURFACE_ID__\",\"tty_name\":\"'$zmux_bootstrap_tty'\"}'",
                "      zmux_relay_ports_kick='{\"workspace_id\":\"__ZMUX_WORKSPACE_ID__\",\"surface_id\":\"__ZMUX_SURFACE_ID__\",\"reason\":\"command\"}'",
                "    fi",
                "    ZMUX_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" ZMUX_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$zmux_relay_cli\" rpc surface.report_tty \"$zmux_relay_report_tty\" >/dev/null 2>&1 || true",
                "    ZMUX_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" ZMUX_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$zmux_relay_cli\" rpc surface.ports_kick \"$zmux_relay_ports_kick\" >/dev/null 2>&1 || true",
                "    unset zmux_relay_cli zmux_relay_report_tty zmux_relay_ports_kick",
                "  fi",
            ]
        }
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__ZMUX_WORKSPACE_ID__' ]; then export ZMUX_WORKSPACE_ID='__ZMUX_WORKSPACE_ID__'; fi",
            "if [ -n '__ZMUX_WORKSPACE_ID__' ]; then export ZMUX_TAB_ID='__ZMUX_WORKSPACE_ID__'; fi",
            "if [ -n '__ZMUX_SURFACE_ID__' ]; then export ZMUX_SURFACE_ID='__ZMUX_SURFACE_ID__'; export ZMUX_PANEL_ID='__ZMUX_SURFACE_ID__'; fi",
        ]
        ...
        commonShellExportLines.append("export PATH=\"$HOME/.zmux/bin:$PATH\"")
        commonShellExportLines.append("export ZMUX_BUNDLED_CLI_PATH=\"$HOME/.zmux/bin/zmux\"")
        commonShellExportLines.append("export ZMUX_SHELL_INTEGRATION_DIR=\"\(shellStateDir)\"")
        if let relaySocket {
            commonShellExportLines.append("export ZMUX_SOCKET_PATH=\(relaySocket)")
            commonShellExportLines.append("export ZMUX_SOCKET=\(relaySocket)")
        }
        ...
        zshShellLines.append(
            #"if [ "${ZMUX_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${ZMUX_SHELL_INTEGRATION_DIR}/zmux-zsh-integration.zsh" ]; then . "${ZMUX_SHELL_INTEGRATION_DIR}/zmux-zsh-integration.zsh"; fi"#
        )
        var bashShellLines = commonShellExportLines
        bashShellLines.append(
            #"if [ "${ZMUX_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${ZMUX_SHELL_INTEGRATION_DIR}/zmux-bash-integration.bash" ]; then . "${ZMUX_SHELL_INTEGRATION_DIR}/zmux-bash-integration.bash"; fi"#
        )
        ...
        let bundledZshIntegration = bundledShellIntegrationScript(named: "zmux-zsh-integration.zsh")
        let bundledBashIntegration = bundledShellIntegrationScript(named: "zmux-bash-integration.bash")
        let bashRCLines = [
        ...
        var outerLines: [String] = [
            "mkdir -p \"$HOME/.zmux/relay\"",
            "zmux_shell_dir=\"\(shellStateDir)\"",
            "mkdir -p \"$zmux_shell_dir\"",
        ]
        if let bundledZshIntegration {
            outerLines += [
                "cat > \"$zmux_shell_dir/zmux-zsh-integration.zsh\" <<'ZMUXZMUXZSH'",
        ...
        if let bundledBashIntegration {
            outerLines += [
                "cat > \"$zmux_shell_dir/zmux-bash-integration.bash\" <<'ZMUXZMUXBASH'",
        ...
        outerLines += [
            "ZMUX_LOGIN_SHELL=\"${SHELL:-/bin/zsh}\"",
            "case \"${ZMUX_LOGIN_SHELL##*/}\" in",
            "  zsh)",
            "    cat > \"$zmux_shell_dir/.zshenv\" <<'ZMUXZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "ZMUXZSHENV",
            "    cat > \"$zmux_shell_dir/.zprofile\" <<'ZMUXZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "ZMUXZSHPROFILE",
            "    cat > \"$zmux_shell_dir/.zshrc\" <<'ZMUXZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "ZMUXZSHRC",
            "    cat > \"$zmux_shell_dir/.zlogin\" <<'ZMUXZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "ZMUXZSHLOGIN",
            "    chmod 600 \"$zmux_shell_dir/.zshenv\" \"$zmux_shell_dir/.zprofile\" \"$zmux_shell_dir/.zshrc\" \"$zmux_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export ZMUX_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$zmux_shell_dir\"",
            "    exec \"$ZMUX_LOGIN_SHELL\" -il",
            "    ;;",
            "  bash)",
            "    cat > \"$zmux_shell_dir/.bashrc\" <<'ZMUXBASHRC'",
        ]
        outerLines.append(contentsOf: bashRCLines)
        outerLines += [
            "ZMUXBASHRC",
            "    chmod 600 \"$zmux_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$ZMUX_LOGIN_SHELL\" --rcfile \"$zmux_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
        outerLines.append(contentsOf: commonShellExportLines)
    ...
    private func shellStateDirForRemoteRelayPort(_ remoteRelayPort: Int) -> String {
        "$HOME/.zmux/relay/\(max(remoteRelayPort, 0)).shell"
    }
    ...
    private func interactiveRemoteRelayWarmupLines(remoteRelayPort: Int) -> [String] {
        ...
        return [
            "zmux_relay_cli=\"${ZMUX_BUNDLED_CLI_PATH:-$HOME/.zmux/bin/zmux}\"",
            "if [ ! -x \"$zmux_relay_cli\" ]; then zmux_relay_cli=\"$(command -v zmux 2>/dev/null || true)\"; fi",
            "zmux_relay_tty=\"${ZMUX_BOOTSTRAP_TTY:-}\"",
            "if [ -z \"$zmux_relay_tty\" ]; then zmux_relay_tty=\"$(tty 2>/dev/null || true)\"; fi",
            "zmux_relay_tty=\"${zmux_relay_tty##*/}\"",
            "if [ -n \"$zmux_relay_tty\" ] && [ \"$zmux_relay_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.zmux/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$zmux_relay_tty\" > \"$HOME/.zmux/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "fi",
            "if [ -n \"$zmux_relay_cli\" ] && [ -n \"$ZMUX_WORKSPACE_ID\" ] && [ -n \"$zmux_relay_tty\" ] && [ \"$zmux_relay_tty\" != \"not a tty\" ]; then",
            "  zmux_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$ZMUX_WORKSPACE_ID\\\",\\\"tty_name\\\":\\\"$zmux_relay_tty\\\"}\"",
            "  zmux_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$ZMUX_WORKSPACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  if [ -n \"$ZMUX_SURFACE_ID\" ]; then",
            "    zmux_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$ZMUX_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$ZMUX_SURFACE_ID\\\",\\\"tty_name\\\":\\\"$zmux_relay_tty\\\"}\"",
            "    zmux_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$ZMUX_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$ZMUX_SURFACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  fi",
            "  \"$zmux_relay_cli\" rpc surface.report_tty \"$zmux_relay_report_tty\" >/dev/null 2>&1 || true",
            "  \"$zmux_relay_cli\" rpc surface.ports_kick \"$zmux_relay_ports_kick\" >/dev/null 2>&1 || true",
            "fi",
            "unset ZMUX_BOOTSTRAP_TTY zmux_relay_cli zmux_relay_tty zmux_relay_report_tty zmux_relay_ports_kick",
        ]
    }
    ...
    func encodedRemoteBootstrapCommand(
        ...
        lines += [
            "zmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/zmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$zmux_tmp\" || { rm -f \"$zmux_tmp\"; exit 1; }",
            "chmod 700 \"$zmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$zmux_tmp\"",
            "zmux_status=$?",
            "rm -f \"$zmux_tmp\"",
    ...
    private func buildSSHStartupScriptBody(
        ...
        scriptLines += [
            ...
            "ZMUX_SSH_SESSION_ENDED=0",
            "zmux_ssh_session_end() { if [ \"${ZMUX_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; ZMUX_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'zmux_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
        ...
        scriptLines += [
            ...
            "if [ \"$zmux_ssh_status\" -ne 0 ]; then",
            "  printf '\\n\\033[31m[zmux] ssh exited with status %s.\\033[0m\\n' \"$zmux_ssh_status\" >&2",
            "  printf '\\033[2m[zmux] the remote VM may have been paused, destroyed, or lost network.\\033[0m\\n' >&2",
            "  printf '\\033[2m[zmux] press Enter to close this pane.\\033[0m\\n' >&2",
            "  IFS= read -r _zmux_dismiss_key 2>/dev/null || true",
            "fi",
            "exit $zmux_ssh_status",
        ]
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "zmux-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func reusableShellStartupCommand(
        ...
        let encodedLiteral = shellQuote(encodedScript)
        let wrapper = [
            "zmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/\(tempPrefix).XXXXXX\") || exit 1",
            "zmux_cleanup() { rm -f -- \"$zmux_tmp\" 2>/dev/null || true; }",
            "trap 'zmux_cleanup' EXIT HUP INT TERM",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$zmux_tmp\" || exit 1",
            "chmod 700 \"$zmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$zmux_tmp\"",
            "zmux_status=$?",
            "trap - EXIT HUP INT TERM",
            "zmux_cleanup",
            "unset zmux_tmp zmux_status",
            "unset -f zmux_cleanup 2>/dev/null || true",
            "exit $zmux_status",
        ].joined(separator: "\n")
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${ZMUX_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${ZMUX_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${ZMUX_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${ZMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${ZMUX_SURFACE_ID:-}\" ]; then",
            "\"${ZMUX_BUNDLED_CLI_PATH}\" --socket \"${ZMUX_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${ZMUX_WORKSPACE_ID}\" --surface \"${ZMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v zmux >/dev/null 2>&1",
            "&& [ -n \"${ZMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${ZMUX_SURFACE_ID:-}\" ]; then",
            "zmux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${ZMUX_WORKSPACE_ID}\" --surface \"${ZMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }

    /// Open an interactive zmux-managed shell on a cloud VM. Freestyle uses the existing SSH
    /// workspace path. E2B uses the zmuxd-remote WebSocket PTY path because E2B does not expose
    /// raw TCP/22.
    ...
    private func vmOpenShell(id: String, workspaceName: String?, client: SocketClient, jsonOutput: Bool, idFormat: CLIIDFormat) throws {
        ...
        if transport == "websocket" {
            ...
            guard endpoint.daemon != nil else {
                throw CLIError(
                    message: "vm.attach_info returned a WebSocket PTY without daemon/proxy support. Rebuild the cloud VM image or snapshot with the current zmuxd-remote."
                )
            }
    ...
    private func vmSSHOptions(
        ...
        guard kind == "password" else {
            if kind == "authorizedKey" {
                throw CLIError(
                    message: "authorizedKey credentials aren't supported by `zmux vm shell` yet; received from server."
                )
            }
    ...
    private func runVMSSHAttach(commandArgs: [String], client: SocketClient) throws {
        ...
        guard remaining.isEmpty else {
            throw CLIError(message: "Usage: zmux vm ssh-attach --id <vm-id>")
        }
    ...
    private func writeVMPtyWebSocketConfig(_ endpoint: VMPtyWebSocketEndpoint) throws -> URL {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-vm-pty-\(UUID().uuidString.lowercased()).json")
        try data.write(to: url, options: .atomic)
    ...
    private func runVMPtyConnect(commandArgs: [String]) throws {
        ...
        guard let configPath else {
            throw CLIError(message: "Usage: zmux vm-pty-connect --config <path>")
        }
    ...
    private func runVMPtyAttach(commandArgs: [String], client: SocketClient) throws {
        ...
        guard remaining.isEmpty else {
            throw CLIError(message: "Usage: zmux vm-pty-attach --id <vm-id>")
        }
        guard let vmID = vmIDOpt?.trimmingCharacters(in: .whitespacesAndNewlines),
              !vmID.isEmpty else {
            throw CLIError(message: "Usage: zmux vm-pty-attach --id <vm-id>")
    ...
    private final class VMPtyWebSocketBridge {
        ...
        private let sendQueue = DispatchQueue(label: "com.zmux.vm-pty.websocket.send")
        ...
        private func startResizeSource() -> DispatchSourceSignal {
            ...
            let source = DispatchSource.makeSignalSource(
                signal: SIGWINCH,
                queue: DispatchQueue(label: "com.zmux.vm-pty.resize")
            )
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or ZMUX_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or ZMUX_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let downloadCommand = "gh release download \(releaseTag) --repo manaflow-ai/zmux --pattern \(assetName)"
        let downloadChecksumsCommand = "gh release download \(releaseTag) --repo manaflow-ai/zmux --pattern \(checksumsAssetName)"
        let checksumVerifyCommand = "shasum -a 256 -c \(checksumsAssetName) --ignore-missing"
        let signerWorkflow = releaseTag == "nightly"
            ? "manaflow-ai/zmux/.github/workflows/nightly.yml"
            : "manaflow-ai/zmux/.github/workflows/release.yml"
        let verifyCommand = "gh attestation verify ./\(assetName) --repo manaflow-ai/zmux --signer-workflow \(signerWorkflow)"

        let payload: [String: Any] = [
            "app_version": remoteDaemonVersionString(from: info),
            "build": info["CFBundleVersion"] ?? NSNull(),
            "commit": info["ZMUXCommit"] ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["ZMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set ZMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["ZMUXRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("zmux-remote-daemons", isDirectory: true)
    ...
    private func deferredRemoteReconnectLocalCommand(
        ...
        let escapedForegroundAuthToken = foregroundAuthToken
            ...
            .replacingOccurrences(of: "\"", with: "\\\"")
        return [
            preferredCLIPath.map { "zmux_reconnect_cli=\(shellQuote($0));" } ?? "zmux_reconnect_cli=\"\";",
            "zmux_reconnect_socket=\"${ZMUX_SOCKET_PATH:-${ZMUX_SOCKET:-}}\";",
            "if [ -z \"$zmux_reconnect_cli\" ] && [ -n \"${ZMUX_BUNDLED_CLI_PATH:-}\" ]; then zmux_reconnect_cli=\"$ZMUX_BUNDLED_CLI_PATH\"; fi;",
            "if [ ! -x \"$zmux_reconnect_cli\" ]; then zmux_reconnect_cli=\"$(command -v zmux 2>/dev/null || true)\"; fi;",
            "if [ -n \"${ZMUX_WORKSPACE_ID:-}\" ]; then",
            "if [ -z \"$zmux_reconnect_socket\" ]; then printf '%s\\n' 'zmux: deferred SSH reconnect skipped, local zmux socket not found' >&2;",
            "elif [ -z \"$zmux_reconnect_cli\" ] || [ ! -x \"$zmux_reconnect_cli\" ]; then printf '%s\\n' 'zmux: deferred SSH reconnect skipped, local zmux CLI not found' >&2;",
            "else",
            "zmux_reconnect_payload=\"{\\\"workspace_id\\\":\\\"$ZMUX_WORKSPACE_ID\\\",\\\"foreground_auth_token\\\":\\\"\(escapedForegroundAuthToken)\\\"}\";",
            "\"$zmux_reconnect_cli\" --socket \"$zmux_reconnect_socket\" rpc workspace.remote.foreground_auth_ready \"$zmux_reconnect_payload\" >/dev/null 2>&1 || true;",
            "unset zmux_reconnect_payload;",
            "fi;",
            "fi;",
            "unset zmux_reconnect_socket zmux_reconnect_cli;",
        ].joined(separator: " ")
    }
    ...
    private func sshConnectionTimingLocalCommand(target: String, relayPort: Int) -> String {
        let escapedTarget = target
            ...
            .replacingOccurrences(of: "\"", with: "\\\"")
        return [
            "zmux_ssh_log_path=\"$(tr -d '\\r\\n' < /tmp/zmux-last-debug-log-path 2>/dev/null || true)\";",
            "if [ -n \"$zmux_ssh_log_path\" ]; then",
            "zmux_ssh_ts=\"$(python3 -c 'from datetime import datetime, timezone; print(datetime.now(timezone.utc).isoformat(timespec=\"milliseconds\").replace(\"+00:00\", \"Z\"))' 2>/dev/null || date -u +%Y-%m-%dT%H:%M:%SZ)\";",
            "printf '%s [zmux-cli] cli.ssh.handshake target=\(escapedTarget) relayPort=\(relayPort) stage=ssh.connected workspace=%s surface=%s\\n' \"$zmux_ssh_ts\" \"${ZMUX_WORKSPACE_ID:-nil}\" \"${ZMUX_SURFACE_ID:-nil}\" >> \"$zmux_ssh_log_path\";",
            "fi;",
            "unset zmux_ssh_log_path zmux_ssh_ts;",
        ].joined(separator: " ")
    }
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/zmux-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/zmux-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["ZMUX_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/zmux-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [zmux-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'zmux browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__zmux_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["ZMUX_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("zmux-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the zmux socket server.
            """
        case "capabilities":
            return """
            Usage: zmux capabilities

            Print server capabilities as JSON.
            """
        case "auth":
            return """
            Usage: zmux auth <status|login|logout>

            status   Print whether the user is signed in (add `zmux --json` for JSON).
            login    Open the sign-in popup on the zmux web app and wait for it to finish.
            logout   Clear the current session.
            """
        case "vm", "cloud":
            return """
            Usage: zmux \(command) <new|ls|rm|exec|shell|attach|ssh> [args...]

            Manage cloud VMs. `cloud` is an alias for `vm`. Requires `zmux auth login`.
            ...
            Env:
              ZMUX_VM_API_BASE_URL       Override the backend origin (default: the zmux website).
                                         `bun run dev` derives this from ZMUX_PORT/PORT for
                                         local testing from the web worktree.

            Example:
              zmux vm new
              zmux vm ls
              zmux cloud exec <id> -- echo hello
              zmux vm rm <id>
            """
        case "rpc":
            return """
            Usage: zmux rpc <method> [json-params]

            Call a raw v2 method with an optional JSON object for params.
            Example: zmux rpc surface.report_tty '{"workspace_id":"...","surface_id":"...","tty_name":"ttys001"}'
            """
        case "help":
            return """
            Usage: zmux help
        ...
        case "welcome":
            ...
            Show a welcome screen with the zmux logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: zmux shortcuts

            Open the Settings window to Keyboard Shortcuts.
            """
        case "restore-session":
            return """
            Usage: zmux restore-session

            Reopen the previous saved zmux session.

            If the app is already running, this restores the last saved session into the current app.
            If the app is not running, this launches zmux and lets startup restore reopen the saved session.
            """
        case "feedback":
            return """
            Usage: zmux feedback
                   zmux feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "feed":
            return """
            Usage: zmux feed clear [--yes|-y]

            Manage persisted Feed workstream history.
            """
        case "opencode":
            return """
            Usage: zmux opencode <install-hooks|uninstall-hooks> [--project] [--yes|-y]

            Manage the zmux OpenCode Feed plugin.
            """
        case "themes":
            return """
            Usage: zmux themes
                   zmux themes list
                   zmux themes set <theme>
                   zmux themes set --light <theme> [--dark <theme>]
                   zmux themes set --dark <theme> [--light <theme>]
                   zmux themes clear

            When run in a TTY, `zmux themes` opens an interactive theme picker with
            live app preview. Use `zmux themes list` for a plain listing.

            The picker previews the selected theme across the running zmux app and
            ...
            Commands:
              ...
              clear                     Remove the zmux theme override and fall back to other config

            Examples:
              zmux themes
              zmux themes list
              zmux themes set "Catppuccin Mocha"
              zmux themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              zmux themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: zmux claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses zmux splits
            ...
            The tmux shim translates supported tmux window/pane commands into zmux
            workspace and split operations in the current zmux session.

            Examples:
              zmux claude-teams
              zmux claude-teams --continue
              zmux claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: zmux omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in a zmux-aware environment.
            ...
            zmux splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses zmux splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into zmux workspace
            and split operations in the current zmux session.

            Examples:
              zmux omo
              zmux omo --continue
              zmux omo --model claude-sonnet-4-6
            """)
        case "opencode":
            return """
            Usage: zmux opencode <install-hooks|uninstall-hooks> [--yes]

            Install or remove the OpenCode plugin that lets zmux remember
            restorable OpenCode session IDs for reopened terminals.

            Files:
              ~/.config/opencode/plugins/zmux-session.js

            Examples:
              zmux opencode install-hooks --yes
              zmux opencode uninstall-hooks
            """
        case "omx":
            return String(localized: "cli.omx.usage", defaultValue: """
            Usage: zmux omx [omx-args...]

            Launch Oh My Codex (OMX) with native zmux pane integration.
            ...
            become native zmux splits.

            This command:
              - sets a tmux-like environment so OMX uses zmux splits
              - prepends a private tmux shim to PATH
            ...
            Examples:
              zmux omx
              zmux omx --madmax --high
              zmux omx team
            """)
        case "omc":
            return String(localized: "cli.omc.usage", defaultValue: """
            Usage: zmux omc [omc-args...]

            Launch Oh My Claude Code (OMC) with native zmux pane integration.
            ...
            This command:
              - sets a tmux-like environment so OMC uses zmux splits
              - prepends a private tmux shim to PATH
            ...
            Examples:
              zmux omc
              zmux omc team 3:claude "implement feature"
              zmux omc --watch
            """)
        case "identify":
            return """
            Usage: zmux identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $ZMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $ZMUX_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: zmux list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: zmux current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: zmux new-window
        ...
        case "focus-window":
            return """
            Usage: zmux focus-window --window <id|ref|index>
            ...
            Example:
              zmux focus-window --window 0
              zmux focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: zmux close-window --window <id|ref|index>
            ...
            Example:
              zmux close-window --window 0
              zmux close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: zmux move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              zmux move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: zmux move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              zmux move-surface --surface surface:1 --workspace workspace:2
              zmux move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: zmux reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              zmux reorder-surface --surface surface:1 --index 0
              zmux reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: zmux reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              zmux reorder-workspace --workspace workspace:2 --index 0
              zmux reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: zmux workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$ZMUX_WORKSPACE_ID)
            ...
            Example:
              zmux workspace-action --workspace workspace:2 --action pin
              zmux workspace-action --action rename --title "infra"
              zmux workspace-action close-others
              zmux workspace-action --action set-color --color blue
              zmux workspace-action --action set-color --color "#C0392B"
              zmux workspace-action set-color Amber
              zmux workspace-action --action set-description --description "Ship checklist"
              zmux workspace-action --action set-description $'Ship checklist\n- verify build\n- post notes'
              zmux workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: zmux tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $ZMUX_TAB_ID, then $ZMUX_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$ZMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              zmux tab-action --tab tab:3 --action pin
              zmux tab-action --action close-right
              zmux tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: zmux rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            1) --tab
            2) --surface
            3) $ZMUX_TAB_ID / $ZMUX_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$ZMUX_WORKSPACE_ID)
            ...
            Examples:
              zmux rename-tab "build logs"
              zmux rename-tab --tab tab:3 "staging server"
              zmux rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: zmux new-workspace [--name <title>] [--description <text>] [--cwd <path>] [--command <text>] [--layout <json>]
            ...
            Flags:
              ...
              --layout <json>      Create workspace with a predefined split layout (inline JSON).
                                   Uses the same schema as zmux.json layout definitions.
                                   When provided, --command is ignored (layout surfaces define their own commands).

            Example:
              zmux new-workspace
              zmux new-workspace --name "Build Server"
              zmux new-workspace --name "Launch" --description "Ship checklist"
              zmux new-workspace --cwd ~/projects/myapp
              zmux new-workspace --cwd . --command "npm test"
              zmux new-workspace --name "Dev" --layout '{"direction":"horizontal","split":0.5,"children":[{"pane":{"surfaces":[{"type":"terminal","command":"vim"}]}},{"pane":{"surfaces":[{"type":"terminal","command":"npm run start"}]}}]}'
            """
        case "list-workspaces":
            return """
            Usage: zmux list-workspaces
        ...
        case "ssh":
            return """
            Usage: zmux ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            zmux will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              zmux ssh dev@my-host
              zmux ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              zmux ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: zmux remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded zmuxd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              zmux remote-daemon-status
              zmux remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: zmux new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $ZMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $ZMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              zmux new-split right
              zmux new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: zmux list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $ZMUX_WORKSPACE_ID)

            Example:
              zmux list-panes
              zmux list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: zmux list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              zmux list-pane-surfaces
              zmux list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: zmux tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `zmux tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              zmux tree
              zmux tree --all
              zmux tree --workspace workspace:2
              zmux --json tree --all
            """
        case "focus-pane":
            return """
            Usage: zmux focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              zmux focus-pane pane:1
              zmux focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: zmux new-pane [flags]
            ...
            Example:
              zmux new-pane
              zmux new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: zmux new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: zmux close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $ZMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $ZMUX_WORKSPACE_ID)

            Example:
              zmux close-surface
              zmux close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: zmux drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              zmux drag-surface-to-split --surface surface:1 right
              zmux drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: zmux refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "reload-config":
            return """
            Usage: zmux reload-config

            Run the same configuration reload as the Reload Configuration shortcut.
            This reloads Ghostty config, re-reads ~/.config/zmux/settings.json, and refreshes terminals.
        ...
        case "surface-health":
            return """
            Usage: zmux surface-health [--workspace <id|ref>]
            ...
            Example:
              zmux surface-health
              zmux surface-health --workspace workspace:2
            """
        case "debug-terminals":
            return """
            Usage: zmux debug-terminals
        ...
        case "trigger-flash":
            return """
            Usage: zmux trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $ZMUX_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $ZMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              zmux trigger-flash
              zmux trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: zmux list-panels [--workspace <id|ref>]
            ...
            Example:
              zmux list-panels
              zmux list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: zmux focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              zmux focus-panel --panel surface:2
              zmux focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: zmux close-workspace --workspace <id|ref|index>
            ...
            Example:
              zmux close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: zmux select-workspace --workspace <id|ref|index>
            ...
            Example:
              zmux select-workspace --workspace workspace:2
              zmux select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: zmux rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$ZMUX_WORKSPACE_ID)

            Example:
              zmux rename-workspace "backend logs"
              zmux rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: zmux current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: zmux capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $ZMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $ZMUX_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              zmux capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: zmux resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: zmux pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: zmux wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: zmux swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: zmux break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: zmux join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: zmux last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: zmux find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: zmux clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: zmux set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: zmux set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: zmux paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: zmux list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: zmux respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: zmux display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              zmux read-screen
              zmux read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: zmux send [flags] [--] <text>
            ...
            Example:
              zmux send "echo hello"
              zmux send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: zmux send-key [flags] [--] <key>
            ...
            Example:
              zmux send-key enter
              zmux send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: zmux send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              zmux send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: zmux send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              zmux send-key-panel --panel surface:2 enter
              zmux send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: zmux notify [flags]
            ...
            Example:
              zmux notify --title "Build done" --body "All tests passed"
              zmux notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: zmux list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: zmux clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: zmux set-status <key> <value> [flags]
            ...
            Example:
              zmux set-status build "compiling" --icon hammer --color "#ff9500"
              zmux set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: zmux clear-status <key> [flags]
            ...
            Example:
              zmux clear-status build
            """
        case "list-status":
            return """
            Usage: zmux list-status [flags]
            ...
            Example:
              zmux list-status
              zmux list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: zmux set-progress <0.0-1.0> [flags]
            ...
            Example:
              zmux set-progress 0.5 --label "Building..."
              zmux set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: zmux clear-progress [flags]
        ...
        case "log":
            ...
            Example:
              zmux log "Build started"
              zmux log --level error --source build "Compilation failed"
              zmux log --level success -- "All 42 tests passed"
            """
        case "clear-log":
            return """
            Usage: zmux clear-log [flags]
        ...
        case "list-log":
            ...
            Example:
              zmux list-log
              zmux list-log --limit 5
            """
        case "sidebar-state":
            return """
            Usage: zmux sidebar-state [flags]
            ...
            Example:
              zmux sidebar-state
              zmux sidebar-state --workspace workspace:2
            """
        case "set-app-focus":
            return """
            Usage: zmux set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              zmux set-app-focus inactive
              zmux set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: zmux simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: zmux claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | zmux claude-hook session-start
              echo '{}' | zmux claude-hook stop
            """
        case "codex":
            return """
            Usage: zmux codex <install-hooks|uninstall-hooks>

            Manage Codex CLI hooks integration.

            Subcommands:
              install-hooks     Install zmux hooks into ~/.codex/hooks.json
              uninstall-hooks   Remove zmux hooks from ~/.codex/hooks.json
            """
        case "codex-hook":
            return """
            Usage: zmux codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside zmux.
        ...
        case "browser":
            return """
            Usage: zmux browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $ZMUX_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              zmux browser open https://example.com
              zmux browser surface:1 navigate https://google.com
              zmux browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `zmux browser --help`
        case "open-browser":
            return "Legacy alias for 'zmux browser open'. Run 'zmux browser --help' for details."
        case "navigate":
            return "Legacy alias for 'zmux browser navigate'. Run 'zmux browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'zmux browser back'. Run 'zmux browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'zmux browser forward'. Run 'zmux browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'zmux browser reload'. Run 'zmux browser --help' for details."
        case "get-url":
            return "Legacy alias for 'zmux browser get-url'. Run 'zmux browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'zmux browser focus-webview'. Run 'zmux browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'zmux browser is-webview-focused'. Run 'zmux browser --help' for details."
        case "markdown":
            return """
            Usage: zmux markdown open <path> [options]
                   zmux markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $ZMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Source surface to split from (default: focused surface)
              --window <id|ref|index>      Target window
              --direction <left|right|up|down>  Split direction (default: right)

            Examples:
              zmux markdown open plan.md
              zmux markdown ~/project/CHANGELOG.md
              zmux markdown open ./docs/design.md --workspace 0
              zmux markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private static let zmuxThemeOverrideBundleIdentifier = "com.zmuxterm.app"
    private static let zmuxThemesBlockStart = "# zmux themes start"
    private static let zmuxThemesBlockEnd = "# zmux themes end"
    private static let zmuxThemesReloadNotificationName = "com.zmuxterm.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["ZMUX_THEME_PICKER_CONFIG"] = try zmuxThemeOverrideConfigURL().path
        environment["ZMUX_THEME_PICKER_BUNDLE_ID"] = currentZmuxAppBundleIdentifier() ?? Self.zmuxThemeOverrideBundleIdentifier
        environment["ZMUX_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["ZMUX_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["ZMUX_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["ZMUX_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'zmux themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try zmuxThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'zmux themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.zmuxThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.zmuxThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func zmuxThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.zmuxThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try zmuxThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.zmuxThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.zmuxThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentZmuxAppBundleIdentifier() ?? Self.zmuxThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.zmuxThemesReloadNotificationName),
            object: nil,
    ...
    private func currentZmuxAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["ZMUX_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !bundleIdentifier.isEmpty {
            return bundleIdentifier
        }
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["ZMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["ZMUX_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["ZMUX_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"])
    }
    ...
    private func tmuxResolveSurfaceTarget(
        ...
        if tmuxPaneSelector(from: raw) != nil {
            ...
            // exact surface (ZMUX_SURFACE_ID) over the pane's currently selected
    ...
    private func isZmuxClaudeWrapper(at path: String) -> Bool {
        guard let data = FileManager.default.contents(atPath: path) else { return false }
        let prefixData = data.prefix(512)
        guard let prefix = String(data: prefixData, encoding: .utf8) else { return false }
        return prefix.contains("zmux claude wrapper - injects hooks and session tracking")
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isZmuxClaudeWrapper(at: $0) }
        )
    }
    ...
    private func exportAgentLaunchCommandEnvironment(
        ...
        setenv("ZMUX_AGENT_LAUNCH_KIND", launcher, 1)
        setenv("ZMUX_AGENT_LAUNCH_EXECUTABLE", executablePath, 1)
        setenv("ZMUX_AGENT_LAUNCH_ARGV_B64", Self.nulSeparatedBase64(arguments), 1)
        if let workingDirectory,
           !workingDirectory.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            setenv("ZMUX_AGENT_LAUNCH_CWD", workingDirectory, 1)
        } else {
            unsetenv("ZMUX_AGENT_LAUNCH_CWD")
        }
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        if let focusedContext {
            setenv("ZMUX_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("ZMUX_SURFACE_ID", surfaceId, 1)
            }
        }
    }

    private static let claudeNodeOptionsRestoreModule = """
    const hadOriginalNodeOptions = process.env.ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
    if (hadOriginalNodeOptions) {
        process.env.NODE_OPTIONS = process.env.ZMUX_ORIGINAL_NODE_OPTIONS ?? "";
    } else {
        delete process.env.NODE_OPTIONS;
    }
    delete process.env.ZMUX_ORIGINAL_NODE_OPTIONS;
    delete process.env.ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT;
    """
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            tmuxPathPrefix: "zmux-claude-teams",
            zmuxBinEnvVar: "ZMUX_CLAUDE_TEAMS_ZMUX_BIN",
            termOverrideEnvVar: "ZMUX_CLAUDE_TEAMS_TERM",
            extraEnvVars: [
                (key: "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", value: "1"),
            ]
        )
        guard let restoreModuleURL = try? createClaudeNodeOptionsRestoreModule() else {
            unsetenv("ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT")
            unsetenv("ZMUX_ORIGINAL_NODE_OPTIONS")
            return
        }
        if let existing = processEnvironment["NODE_OPTIONS"] {
            setenv("ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "1", 1)
            setenv("ZMUX_ORIGINAL_NODE_OPTIONS", normalizedNodeOptionsForRestore(existing), 1)
        } else {
            setenv("ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "0", 1)
            unsetenv("ZMUX_ORIGINAL_NODE_OPTIONS")
        }
        setenv(
            "NODE_OPTIONS",
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${ZMUX_CLAUDE_TEAMS_ZMUX_BIN:-zmux}" __tmux-compat "$@"
        """
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("zmux-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["ZMUX_SOCKET_PATH"] = socketPath
        launcherEnvironment["ZMUX_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["ZMUX_SOCKET_PASSWORD"] = explicitPassword
        }
        ...
        let claudeExecutablePath: String? = {
            ...
            let candidates = [
                launcherEnvironment["ZMUX_CUSTOM_CLAUDE_PATH"],
            ...
            for raw in candidates {
                ...
                guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDir),
                      ...
                      !isZmuxClaudeWrapper(at: trimmed) else { continue }
                return trimmed
            }
    ...
    // MARK: - zmux omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to zmux __tmux-compat
        ...
        exec "${ZMUX_OMO_ZMUX_BIN:-zmux}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to zmux notify
        let notifierURL = root.appendingPathComponent("terminal-notifier", isDirectory: false)
        let notifierScript = """
        #!/usr/bin/env bash
        # Intercept terminal-notifier calls and route through zmux notify.
        ...
        exec "${ZMUX_OMO_ZMUX_BIN:-zmux}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private static let openCodeSessionPluginConfigSpec = "zmux-session"
    ...
    private func configureOMOEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "zmux-omo",
            zmuxBinEnvVar: "ZMUX_OMO_ZMUX_BIN",
            termOverrideEnvVar: "ZMUX_OMO_TERM",
            extraEnvVars: [
                (key: "OPENCODE_PORT", value: openCodePort),
                (key: "ZMUX_OPENCODE_ZMUX_BIN", value: executablePath),
            ]
        )
    }
    ...
    private func createOMXShimDirectory() throws -> URL {
        ...
        exec "${ZMUX_OMX_ZMUX_BIN:-zmux}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    private func configureOMXEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "zmux-omx",
            zmuxBinEnvVar: "ZMUX_OMX_ZMUX_BIN",
            termOverrideEnvVar: "ZMUX_OMX_TERM"
        )
    }
    ...
    // MARK: - zmux omc (Oh My Claude Code)
    ...
    private func createOMCShimDirectory() throws -> URL {
        ...
        exec "${ZMUX_OMC_ZMUX_BIN:-zmux}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    private func configureOMCEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "zmux-omc",
            zmuxBinEnvVar: "ZMUX_OMC_ZMUX_BIN",
            termOverrideEnvVar: "ZMUX_OMC_TERM"
        )
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in zmux claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in zmux claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        ...
        return URL(fileURLWithPath: homePath)
            .appendingPathComponent(".zmuxterm")
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/zmux-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in zmux CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in zmux CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "zmux", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["ZMUX_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["ZMUX_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["ZMUX_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                zmux claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["ZMUX_CLI_TTY_NAME", "ZMUX_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    /// The hooks.json content that zmux installs into ~/.codex/.
    /// Each hook calls `zmux codex-hook <event>` which gracefully no-ops
    /// when not running inside zmux. The command checks for zmux on PATH
    /// first so it silently succeeds even when zmux is not installed
    /// (e.g. user opened codex in a non-zmux terminal).
    ...
    private func agentLaunchCommandFromEnvironment(
        ...
        let envArguments = decodeNULSeparatedBase64(env["ZMUX_AGENT_LAUNCH_ARGV_B64"])
        ...
        let executablePath = normalizedHookValue(env["ZMUX_AGENT_LAUNCH_EXECUTABLE"]) ?? arguments.first
        let workingDirectory = normalizedHookValue(env["ZMUX_AGENT_LAUNCH_CWD"])
            ?? normalizedHookValue(cwd)
            ?? normalizedHookValue(env["PWD"])
        let launcher = normalizedHookValue(env["ZMUX_AGENT_LAUNCH_KIND"]) ?? fallbackKind
    ...
    private func selectedAgentLaunchNodeOptions(from env: [String: String]) -> String? {
        switch normalizedHookValue(env["ZMUX_ORIGINAL_NODE_OPTIONS_PRESENT"]) {
        case "1":
            return sanitizedAgentLaunchNodeOptions(env["ZMUX_ORIGINAL_NODE_OPTIONS"])
        case "0":
            return nil
        default:
    ...
    private func isZmuxNodeOptionsRestoreModulePath(_ value: String) -> Bool {
    ...
    private func agentHookStatePath(sessionStoreSuffix: String, env: [String: String]) -> String {
        ...
        guard let overrideDirectory = normalizedHookValue(env["ZMUX_AGENT_HOOK_STATE_DIR"]) else {
            return "~/.zmuxterm/\(filename)"
        }
    ...
    private struct AgentHookDef {
        ...
        let sessionStoreSuffix: String // e.g. "cursor" -> ~/.zmuxterm/cursor-hook-sessions.json
        let disableEnvVar: String   // e.g. "ZMUX_CURSOR_HOOKS_DISABLED"
        let hookMarker: String      // Marker in commands: "zmux cursor-hook"
        let format: HookFormat
        let events: [HookEvent]
        /// Feed-hook events. Each entry installs a second hook for
        /// `agentEvent` that invokes `zmux feed-hook --source <name>`
    ...
    private static let agentDefs: [AgentHookDef] = [
        AgentHookDef(
            ...
            sessionStoreSuffix: "codex", disableEnvVar: "ZMUX_CODEX_HOOKS_DISABLED",
            hookMarker: "zmux codex-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", zmuxSubcommand: "prompt-submit"),
                .init(agentEvent: "Stop", zmuxSubcommand: "stop"),
            ],
        ...
        AgentHookDef(
            ...
            configDir: ".config/opencode", configFile: "plugins/zmux-session.js", configDirEnvOverride: "OPENCODE_CONFIG_DIR",
            sessionStoreSuffix: "opencode", disableEnvVar: "ZMUX_OPENCODE_HOOKS_DISABLED",
            hookMarker: "zmux opencode-hook", format: .flat,
            events: []
        ),
        AgentHookDef(
            name: "cursor", displayName: "Cursor", statusKey: "cursor",
            configDir: ".cursor", configFile: "hooks.json",
            sessionStoreSuffix: "cursor", disableEnvVar: "ZMUX_CURSOR_HOOKS_DISABLED",
            hookMarker: "zmux cursor-hook", format: .flat,
            events: [
                .init(agentEvent: "beforeSubmitPrompt", zmuxSubcommand: "prompt-submit"),
                .init(agentEvent: "stop", zmuxSubcommand: "stop"),
                .init(agentEvent: "afterAgentResponse", zmuxSubcommand: "agent-response"),
                .init(agentEvent: "beforeShellExecution", zmuxSubcommand: "shell-exec"),
                .init(agentEvent: "afterShellExecution", zmuxSubcommand: "shell-done"),
            ],
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "gemini", disableEnvVar: "ZMUX_GEMINI_HOOKS_DISABLED",
            hookMarker: "zmux gemini-hook", format: .nested(timeoutMs: 10000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "BeforeAgent", zmuxSubcommand: "prompt-submit"),
                .init(agentEvent: "AfterAgent", zmuxSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", zmuxSubcommand: "session-end"),
            ],
            feedHookEvents: ["PreToolUse"]
        ),
        AgentHookDef(
            ...
            sessionStoreSuffix: "copilot", disableEnvVar: "ZMUX_COPILOT_HOOKS_DISABLED",
            hookMarker: "zmux copilot-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "Stop", zmuxSubcommand: "stop"),
                .init(agentEvent: "Notification", zmuxSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", zmuxSubcommand: "session-end"),
            ],
            feedHookEvents: ["PreToolUse"]
        ),
        AgentHookDef(
            ...
            sessionStoreSuffix: "codebuddy", disableEnvVar: "ZMUX_CODEBUDDY_HOOKS_DISABLED",
            hookMarker: "zmux codebuddy-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "Stop", zmuxSubcommand: "stop"),
                .init(agentEvent: "Notification", zmuxSubcommand: "stop"),
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "factory", disableEnvVar: "ZMUX_FACTORY_HOOKS_DISABLED",
            hookMarker: "zmux factory-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "Stop", zmuxSubcommand: "stop"),
                .init(agentEvent: "Notification", zmuxSubcommand: "stop"),
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "qoder", disableEnvVar: "ZMUX_QODER_HOOKS_DISABLED",
            hookMarker: "zmux qoder-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", zmuxSubcommand: "session-start"),
                .init(agentEvent: "Stop", zmuxSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", zmuxSubcommand: "session-end"),
    ...
    private func hookCommand(for def: AgentHookDef, event: AgentHookDef.HookEvent) -> String {
        "[ -n \"$ZMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v zmux >/dev/null 2>&1 && zmux \(def.name)-hook \(event.zmuxSubcommand) || echo '{}'"
    }
    ...
    private func feedHookCommand(for def: AgentHookDef, agentEvent: String) -> String {
        "[ -n \"$ZMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v zmux >/dev/null 2>&1 && zmux feed-hook --source \(def.name) --event \(agentEvent) || echo '{}'"
    }

    /// Marker substring we look for when removing / upgrading our own
    /// feed-hook entries on reinstall or uninstall.
    private static let feedHookMarker = "zmux feed-hook --source"
    ...
    private static let openCodeSessionPluginMarker = "zmux-opencode-session-plugin-marker"
    private static let openCodeSessionPluginFilename = "zmux-session.js"
    private static let openCodeSessionPluginSource = #"""
// zmux-opencode-session-plugin-marker v1
// Bridges OpenCode session lifecycle events into zmux's restorable session store.
// Installed by `zmux opencode install-hooks` or `zmux setup-hooks`.
...
const ZMUX_PLUGIN_INSTALLED_KEY = Symbol.for("zmux.session.restore.plugin.installed");
...
function hookEnvironment(cwd) {
  ...
  if (!env.ZMUX_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.ZMUX_AGENT_LAUNCH_KIND = "opencode";
    env.ZMUX_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("opencode");
    env.ZMUX_AGENT_LAUNCH_ARGV_B64 = base64NulSeparated(argv);
    env.ZMUX_AGENT_LAUNCH_CWD = cwd || process.cwd();
  }
  return env;
}

function sendHook(subcommand, ctx, event, extra = {}) {
  if (process.env.ZMUX_OPENCODE_HOOKS_DISABLED === "1") return;
  if (!process.env.ZMUX_SURFACE_ID) return;
  ...
  const zmux = process.env.ZMUX_OPENCODE_ZMUX_BIN || "zmux";
  try {
    spawnSync(zmux, ["opencode-hook", subcommand], {
...
const ZMUXSessionRestore = async (ctx) => {
  if (globalThis[ZMUX_PLUGIN_INSTALLED_KEY]) return {};
  globalThis[ZMUX_PLUGIN_INSTALLED_KEY] = true;
  return {
    event: async ({ event }) => {
      const props = eventProperties(event);
...
export { ZMUXSessionRestore };
export default ZMUXSessionRestore;
"""#
    ...
    private func installOpenCodePluginHooks(_ def: AgentHookDef) throws {
        ...
        if !skipConfirm {
            print("Will write OpenCode zmux plugin to \(pluginURL.path):")
    ...
    private func uninstallOpenCodePluginHooks(_ def: AgentHookDef) throws {
        ...
        print("Removed OpenCode zmux plugin from \(pluginURL.path)")
    }
    ...
    private func installAgentHooks(_ def: AgentHookDef) throws {
        ...
        // Remove existing zmux-owned entries (both the per-agent hook
        // dispatcher and the feed-hook bridge). Non-zmux entries are
        // always preserved — even when the user mixed them into the
        // same group as a zmux hook, we only prune our own entries
        // within that group so the user's stays put.
        let isZmuxOwnedCommand: (String) -> Bool = { cmd in
        ...
        for (event, value) in hooks {
            ...
            case .flat:
                ...
                entries.removeAll { isZmuxOwnedCommand($0["command"] as? String ?? "") }
            ...
            case .nested:
                ...
                for var group in groups {
                    ...
                    hookList.removeAll { isZmuxOwnedCommand($0["command"] as? String ?? "") }
                    if hookList.isEmpty {
                        // Fully zmux-owned group → drop it entirely.
                        continue
                    }
    ...
    private func uninstallAgentHooks(_ def: AgentHookDef) throws {
        ...
        try newData.write(to: URL(fileURLWithPath: filePath), options: .atomic)
        print("Removed \(removed) zmux hook(s) from \(filePath)")
    ...
    private func runGenericAgentHook(def: AgentHookDef, commandArgs: [String], client: SocketClient, telemetry: CLISocketSentryTelemetry) throws {
        ...
        let hookWsFlag = optionValue(hookArgs, name: "--workspace")
        let workspaceArg = hookWsFlag ?? env["ZMUX_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? env["ZMUX_SURFACE_ID"] : nil)
        ...
        let store = ClaudeHookSessionStore(
            processEnv: env.merging(
                ["ZMUX_CLAUDE_HOOK_STATE_PATH": agentHookStatePath(sessionStoreSuffix: def.sessionStoreSuffix, env: env)],
                uniquingKeysWith: { _, new in new }
            )
        )

        let sessionId = input.sessionId ?? env["ZMUX_SURFACE_ID"] ?? ""
    ...
    private static func unifiedDiff(old: String, new: String) -> String? {
        if old == new { return "" }
        let tempDir = FileManager.default.temporaryDirectory
        let oldURL = tempDir.appendingPathComponent("zmux-old-\(UUID().uuidString)")
        let newURL = tempDir.appendingPathComponent("zmux-new-\(UUID().uuidString)")
        defer {
    ...
    /// Marker matching the `// zmux-feed-plugin-marker` line emitted at
    /// the top of the generated plugin JS. Lets us detect our own
    /// plugin file and upgrade/uninstall without touching user plugins.
    private static let openCodePluginMarker = "zmux-feed-plugin-marker"

    private static let openCodePluginFileName = "zmux-feed.js"
    ...
    private func installOpenCodePlugin(projectLocal: Bool) throws {
        ...
        let fm = FileManager.default
        // If an existing non-zmux plugin lives at the same path, refuse
        ...
        if !existing.isEmpty, !existing.contains(Self.openCodePluginMarker) {
            throw CLIError(message: "\(path) exists and is not a zmux plugin; leaving it alone")
        }
    ...
    private func uninstallOpenCodePlugin(projectLocal: Bool = false) throws {
        ...
        for path in [openCodePluginPath(projectLocal: false),
            ...
            guard let existing = try? String(contentsOfFile: path, encoding: .utf8),
                ...
                print("Skipping \(path) (no zmux marker)")
                continue
            }
    ...
    /// running zmux app via the `feed.push` V2 socket verb, and (for
    ...
    ///
    /// Usage:
    ///   echo "<hook_json>" | zmux feed-hook --source <claude|codex|...>
    ///
    ...
    private func runFeedHook(
        ...
        guard !source.isEmpty else {
            throw CLIError(message: "zmux feed-hook requires --source <agent-name>")
        }

        // Outside a zmux terminal (no ZMUX_SURFACE_ID) → silently no-op.
        ...
        // killed/crashed. Claude's wrapper exports ZMUX_CLAUDE_PID.
        ...
        let agentPid: Int = {
            let envKey: String
            switch source {
            case "claude":   envKey = "ZMUX_CLAUDE_PID"
            case "codex":    envKey = "ZMUX_CODEX_PID"
            case "cursor":   envKey = "ZMUX_CURSOR_PID"
            case "gemini":   envKey = "ZMUX_GEMINI_PID"
            case "copilot":  envKey = "ZMUX_COPILOT_PID"
            default:         envKey = ""
            }
            if !envKey.isEmpty,
               let raw = env[envKey],
    ...
    private static func renderAgentDecision(
        ...
        func claudePermissionRequestDecision(
            ...
            if behavior == "deny" {
                inner["message"] = message ?? "User denied permission via zmux Feed."
            }
        ...
        case "permission":
            ...
            if source == "claude" {
                if mode == "deny" {
                    return encode(claudePermissionRequestDecision(
                        behavior: "deny",
                        message: "User denied permission via zmux Feed."
                    ))
                }
            ...
            if source == "codex" {
                if mode == "deny" {
                    return encode([
                        "decision": "block",
                        "reason": "User denied permission via zmux Feed."
                    ])
                }
            ...
            var reasonText = "User approved via zmux Feed."
            if mode == "always" || mode == "all" || mode == "bypass" {
                reasonText = "User granted \(mode) permission via zmux Feed. Reduce subsequent approval prompts for similar calls."
            }
            return encode(nonClaudePreToolDecision(
                permission: "allow",
                reason: reasonText
        ...
        case "exit_plan":
            ...
            if source == "claude" {
                if let feedback, !feedback.isEmpty {
                    return encode(claudePermissionRequestDecision(
                        behavior: "deny",
                        message: "User rejected the plan via zmux Feed and wants this change: \(feedback)"
                    ))
                }
                if mode == "deny" {
                    return encode(claudePermissionRequestDecision(
                        behavior: "deny",
                        message: "User rejected the plan via zmux Feed."
                    ))
                }
                if mode == "ultraplan" {
                    return encode(claudePermissionRequestDecision(
                        behavior: "deny",
                        message: "User chose Ultraplan via zmux Feed. Refine this plan with Ultraplan on Claude Code on the web."
                    ))
                }
                var updatedPermissions: [[String: Any]]?
                if mode == "autoAccept" {
            ...
            if let feedback, !feedback.isEmpty {
                let reason = "User rejected the plan via zmux Feed and wants this change: \(feedback)"
                return encode(nonClaudePreToolDecision(
                    permission: "deny",
                    reason: reason,
            ...
            if mode == "deny" {
                return encode(nonClaudePreToolDecision(
                    permission: "deny",
                    reason: "User rejected the plan via zmux Feed."
                ))
            }
            if mode == "ultraplan" {
                let reason = "User chose Ultraplan via zmux Feed. Refine this plan with Ultraplan if available."
                return encode(nonClaudePreToolDecision(
                    permission: "deny",
                    reason: reason,
                    additionalContext: reason
            ...
            let ctx = "User accepted this plan via zmux Feed with \(modeText). Exit plan mode now and proceed to implement without re-entering ExitPlanMode. Do not ask again."
            return encode(nonClaudePreToolDecision(
                permission: "deny",
                reason: ctx,
        ...
        case "question":
            ...
            if selections == [Self.skipInterviewAndPlanAnswer] {
                let message = "User chose Skip interview and plan immediately via zmux Feed. Do not ask more interview questions. Write the plan now."
                if source == "claude" {
                    return encode(claudePermissionRequestDecision(
                        behavior: "deny",
                        message: message
            ...
            let ctx = "[zmux Feed] \(body). Treat these as the user's response to your AskUserQuestion prompt; do not call AskUserQuestion again for the same question."
            return encode(nonClaudePreToolDecision(
                permission: "deny",
                reason: ctx,
                additionalContext: ctx
    ...
    private func runSetupHooks(uninstall: Bool = false) throws {
        ...
        print("zmux \(isUninstall ? "uninstall" : "setup")-hooks: \(verb) agent hooks")
        if !isUninstall {
    ...
    private func versionSummary() -> String {
        let info = resolvedVersionInfo()
        let commit = info["ZMUXCommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "zmux \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "zmux \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "zmux build \(build)"
        } else {
            baseSummary = "zmux version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print(shortcuts)
        print()
        print("  \(bold)Docs\(reset)\(subdued)                https://zmux.com/docs\(reset)")
        print("  \(bold)Discord\(reset)\(subdued)             https://discord.gg/xsgFEVrWCZ\(reset)")
        print("  \(bold)GitHub\(reset)\(subdued)              https://github.com/manaflow-ai/zmux (please leave a star ⭐)\(reset)")
        print("  \(bold)Email\(reset)\(subdued)               founders@manaflow.com\(reset)")
        print()
        print("  \(subdued)Run \(reset)\(bold)zmux --help\(reset)\(subdued) for all commands.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)zmux shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)zmux feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["ZMUXCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["ZMUX_COMMIT"]) {
            info["ZMUXCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["ZMUXCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["ZMUXCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return """
        zmux - control zmux via Unix socket

        Usage:
          zmux <path>                Open a directory in a new workspace (launches zmux if needed)
          zmux [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then ZMUX_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Environment:
          ZMUX_WORKSPACE_ID   Auto-set in zmux terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          ZMUX_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          ZMUX_SURFACE_ID     Auto-set in zmux terminals. Used as default --surface.
          ZMUX_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/zmux/zmux.sock and auto-discovers tagged/debug sockets.
        """
    }
...
struct ZMUXTermMain {
    static func main() {
        ...
        let cli = ZMUXCLI(args: CommandLine.arguments)
        do {
            try cli.run()
        } catch {
