#!/usr/bin/env python3
"""Regression test: CLI should auto-discover tagged debug sockets from ZMUX_TAG."""
...
def main() -> int:
    ...
    env = os.environ.copy()
    env["ZMUX_SOCKET_PATH"] = "/tmp/zmux.sock"
    env["ZMUX_TAG"] = tag
    ...
    try:
        ...
        print(f"FAIL: invoking zmux ping failed: {exc}")
        return 1
    finally:
    ...
    if proc.returncode != 0:
        print("FAIL: zmux ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    if proc.stdout.strip() != "PONG":
        print("FAIL: zmux ping did not use auto-discovered socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    print("PASS: zmux ping auto-discovers tagged socket from ZMUX_TAG")
    return 0
