...
def _git_stub() -> str:
    return textwrap.dedent(
        ...
        if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
          printf 'https://github.com/manaflow-ai/zmux.git\\n'
          exit 0
        fi
...
def _gh_stub() -> str:
    return textwrap.dedent(
        """\
        #!/bin/sh
        args_log="${ZMUX_TEST_GH_ARGS_LOG:?}"
        printf '%s\\n' "$*" >> "$args_log"
        exit 0
        """
    )
...
def _shell_command(kind: str, scenario: str) -> str:
    shared = {
        ...
        "merge_action": (
            'cd "$ZMUX_TEST_REPO"\n'
            '_zmux_pr_command_entry "gh pr merge"\n'
            '_zmux_prompt_entry\n'
            'sleep 1\n'
            '_zmux_cleanup\n'
        ),
        "close_action_target": (
            'cd "$ZMUX_TEST_REPO"\n'
            '_zmux_pr_command_entry "gh pr close 2580"\n'
            '_zmux_prompt_entry\n'
            'sleep 1\n'
            '_zmux_cleanup\n'
        ),
        ...
        "non_pr_gh_no_action": (
            'cd "$ZMUX_TEST_REPO"\n'
            '_zmux_pr_command_entry "gh issue view 1"\n'
            '_zmux_prompt_entry\n'
            'sleep 1\n'
            '_zmux_cleanup\n'
        ),
    ...
    if kind == "zsh":
        return textwrap.dedent(
            f"""\
            source "$ZMUX_TEST_SCRIPT"
            _zmux_send() {{ print -r -- "$1" >> "$ZMUX_TEST_SEND_LOG"; }}
            _zmux_prompt_entry() {{ _zmux_precmd; }}
            _zmux_pr_command_entry() {{ _zmux_preexec "$1"; }}
            _zmux_cleanup() {{ _zmux_zshexit; }}
            {shared}"""
        )
    ...
    if kind == "bash":
        return textwrap.dedent(
            f"""\
            source "$ZMUX_TEST_SCRIPT"
            _zmux_send() {{ printf '%s\\n' "$1" >> "$ZMUX_TEST_SEND_LOG"; }}
            _zmux_prompt_entry() {{ _zmux_prompt_command; }}
            _zmux_pr_command_entry() {{ _zmux_bash_preexec_hook "$1"; }}
            _zmux_cleanup() {{ type _zmux_bash_cleanup >/dev/null 2>&1 && _zmux_bash_cleanup; }}
            {shared}"""
        )
...
def _run_case(base: Path, *, shell: str, shell_args: list[str], script: Path, scenario: str) -> tuple[int, str]:
    ...
    env["ZMUX_TAB_ID"] = "00000000-0000-0000-0000-000000000001"
    env["ZMUX_PANEL_ID"] = "00000000-0000-0000-0000-000000000002"
    env["ZMUX_TEST_SCRIPT"] = str(script)
    env["ZMUX_TEST_REPO"] = str(repo)
    env["ZMUX_TEST_SEND_LOG"] = str(send_log)
    env["ZMUX_TEST_GH_ARGS_LOG"] = str(gh_args_log)
    env["ZMUX_TEST_HEAD_FILE"] = str(head_file)
...
def main() -> int:
    ...
    cases = [
        ("zsh", ["-f", "-c"], root / "Resources" / "shell-integration" / "zmux-zsh-integration.zsh"),
        ("bash", ["--noprofile", "--norc", "-c"], root / "Resources" / "shell-integration" / "zmux-bash-integration.bash"),
    ]
    scenarios = [
        "prompt_does_not_call_gh",
        "merge_action",
    ...
    base = Path("/tmp") / f"zmux_issue_1138_pr_poll_{os.getpid()}"
    try:
