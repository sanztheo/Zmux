#!/usr/bin/env python3
"""
Regression test: `zmux claude-teams` reuses an existing tmux shim.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zmux-claude-teams-shim-") as td:
        tmp = Path(td)
        home = tmp / "home"
        ...
        shim_dir = home / ".zmuxterm" / "claude-teams-bin"
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zmux claude-teams --version` failed with an existing shim")
    ...
    print("PASS: zmux claude-teams reuses an existing tmux shim")
    return 0
