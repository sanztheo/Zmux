...
In zmuxOnly mode (default), only processes descended from the zmux
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: zmuxOnly — external processes get rejected
  Phase 2: zmuxOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("ZMUX_TAG") or os.environ.get("ZMUX_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/zmux DEV.app")
...
def _find_app():
    explicit = os.environ.get("ZMUX_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/zmux-{preferred_slug}/Build/Products/Debug/zmux DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/zmux-{preferred_slug}/Build/Products/Debug/zmux DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/zmux DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/zmux-*/Build/Products/Debug/zmux DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/zmux-*/Build/Products/Debug/zmux DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "zmux")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/zmux"
    )))
    candidates.extend(glob.glob("/tmp/zmux-*/Build/Products/Debug/zmux"))
    candidates.extend(glob.glob("/private/tmp/zmux-*/Build/Products/Debug/zmux"))
...
def _kill_zmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/zmux DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "zmux DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_zmux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"ZMUX_SOCKET_MODE={mode}"]
    launch_env = {
        "ZMUX_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a zmux-spawned terminal process CAN connect in zmuxOnly mode.
    Inject a test via the shell rc file, then launch zmux in zmuxOnly mode.
    The shell (a descendant of zmux) runs the test on startup.
    """
    result = TestResult("Internal process can connect (zmuxOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"zmux_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"zmux_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing zmux, launch in zmuxOnly mode (default)
        _kill_zmux(app_path)
        _launch_zmux(app_path, socket_path, mode="zmuxOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in zmuxOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify ZMUX_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_zmux(app_path)
        _launch_zmux(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"zmux-pass-{os.getpid()}"
    try:
        _kill_zmux(app_path)
        _launch_zmux(
            app_path,
            socket_path,
            mode="password",
            extra_env={"ZMUX_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find zmux CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("zmux Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find zmux DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/zmux-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: zmuxOnly — external rejection ──
    print("Phase 1: zmuxOnly mode — external rejection")
    print("-" * 50)

    # Ensure zmux is running in zmuxOnly mode
    _kill_zmux(app_path)
    print("  Launching zmux in zmuxOnly mode...")
    _launch_zmux(app_path, socket_path, mode="zmuxOnly")
    ...
    # ── Phase 2: zmuxOnly — internal process CAN connect ──
    print("Phase 2: zmuxOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave zmux in zmuxOnly mode ──
    _kill_zmux(app_path)
    _launch_zmux(app_path, socket_path, mode="zmuxOnly")
