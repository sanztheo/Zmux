#!/usr/bin/env python3
"""
Regression: the zmux zsh wrapper should prefer a bundled Ghostty zsh
integration file in ZMUX_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged zmux builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    base = Path("/tmp") / f"zmux_bundled_ghostty_zsh_{os.getpid()}"
    try:
        ...
        env["ZMUX_SHELL_INTEGRATION_DIR"] = str(bundled)
        env["GHOSTTY_RESOURCES_DIR"] = str(fallback)
        env["ZMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        env["ZMUX_SHELL_INTEGRATION"] = "0"
        env["ZMUX_TEST_OUT"] = str(marker)
