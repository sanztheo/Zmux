...
def _wait_for_terminal_focus(c: zmux, panel_id: str, timeout_s: float = 8.0) -> None:
    start = time.time()
    while time.time() - start < timeout_s:
        try:
            c.activate_app()
...
def _wait_for_text(c: zmux, panel_id: str, needle: str, timeout_s: float = 2.5) -> None:
    start = time.time()
    last = ""
    while time.time() - start < timeout_s:
    ...
    raise zmuxError(f"Timed out waiting for token in terminal text: {needle}\nLast tail:\n{tail}")


def _type_and_wait_visible(c: zmux, panel_id: str, cmd: str) -> bool:
...
def _wait_for_tmp_write(c: zmux, panel_id: str, tmp: str, token: str) -> None:
...
def main() -> int:
    with zmux(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise zmuxError(f"expected multiple panes, got: {panes}")
        ...
        for i in range(6):
            ...
            token = f"ZMUX_NEW_TAB_OK_{i}_{uuid.uuid4().hex[:10]}"
            tmp = f"/tmp/zmux_new_tab_{token}.txt"
