...
A client library for programmatically controlling zmux via Unix socket.
...
_DEFAULT_DEBUG_BUNDLE_ID = "com.zmuxterm.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("ZMUX_BUNDLE_ID")
    if override:
        return override
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/zmux-debug-{slug}.sock",
            f"/tmp/zmux-{slug}.sock",
        ]
    ...
    override = os.environ.get("ZMUX_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/zmux-debug-*.sock")
    tagged.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "zmux*.sock")))
...
class zmux:
    """Client for controlling zmux via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise zmuxError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise zmuxError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise zmuxError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise zmuxError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise zmuxError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise zmuxError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise zmuxError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for zmux"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="zmux CLI")
    ...
    try:
        with zmux(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("zmux CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
