...
def get_browser_url(client: zmux, panel_id: str) -> str:
    """Get the current URL of a browser panel."""
    return client._send_command(f"get_url {panel_id}").strip()


def navigate_browser(client: zmux, panel_id: str, url: str) -> None:
...
def wait_for_url(client: zmux, panel_id: str, expected_url: str,
...
def test_cmd_bracket_back_forward(client: zmux) -> tuple[bool, str]:
    """
...
def test_cmd_bracket_noop_on_terminal(client: zmux) -> tuple[bool, str]:
    """
...
def test_browser_back_forward_socket_commands(client: zmux) -> tuple[bool, str]:
    """
