...
Run with a tagged instance to avoid unix socket conflicts:
  ZMUX_TAG=<tag> python3 tests/test_sidebar_cwd_git.py
"""
...
def _init_git_repo(repo: Path) -> None:
    ...
    _git(repo, "init")
    _git(repo, "config", "user.email", "zmux-test@example.com")
    _git(repo, "config", "user.name", "zmux-test")
...
def main() -> int:
    ...
    base = Path("/tmp") / f"zmux_sidebar_test_{os.getpid()}"
    repo = base / "repo"
    other = base / "other"
    ...
    except (zmuxError, subprocess.CalledProcessError, AssertionError) as e:
