...
def wait_for_overlay_probe_ready(client: zmux, timeout_s: float = 8.0) -> None:
    start = time.time()
    last_error = None
    while time.time() - start < timeout_s:
        try:
            ...
            last_error = e
            time.sleep(0.1)
    raise zmuxError(f"overlay_hit_gate probe unavailable: {last_error}")


def assert_gate(client: zmux, event_type: str, expected: bool, reason: str) -> None:
...
def assert_drop_gate(client: zmux, source: str, expected: bool, reason: str) -> None:
...
def assert_portal_gate(client: zmux, event_type: str, expected: bool, reason: str) -> None:
...
def assert_sidebar_gate(client: zmux, state: str, expected: bool, reason: str) -> None:
...
def main() -> int:
    ...
    if not os.path.exists(socket_path):
        ...
        print("Tip: start zmux first (or set ZMUX_TAG / ZMUX_SOCKET_PATH).")
        return 0
