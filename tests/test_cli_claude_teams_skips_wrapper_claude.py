#!/usr/bin/env python3
"""
Regression test: `zmux claude-teams` skips zmux wrapper scripts on PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zmux-claude-teams-wrapper-") as td:
        tmp = Path(td)
        ...
        make_executable(
            wrapper_bin / "claude",
            """#!/usr/bin/env bash
# zmux claude wrapper - injects hooks and session tracking
set -euo pipefail
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zmux claude-teams --version` executed a wrapper instead of the real claude binary")
    ...
    print("PASS: zmux claude-teams skips zmux wrapper scripts on PATH")
    return 0
