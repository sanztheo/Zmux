...
Requires zmux to be running with socket control enabled.
"""
...
def get_zmux_pid() -> Optional[int]:
    ...
    if not socket_path:
        # Ask zmux.py to resolve default socket path (supports ZMUX_TAG and last-socket file).
        try:
            socket_path = zmux().socket_path
        except Exception:
            socket_path = None
...
def test_cpu_after_notification_burst(client: zmux, pid: int) -> tuple[bool, str]:
    """
...
def test_cpu_after_popover_close(client: zmux, pid: int) -> tuple[bool, str]:
    """
    ...
    # Ensure the correct zmux instance is frontmost (tag-safe).
    bundle_id = zmux.default_bundle_id()
    subprocess.run(
...
def test_cpu_idle_with_notifications(client: zmux, pid: int) -> tuple[bool, str]:
    """
...
def main():
    print("=" * 60)
    print("zmux Notification CPU Tests")
    print("=" * 60)
    ...
    client = zmux(socket_path)
    try:
        client.connect()
        print(f"Connected to {socket_path}")
    except zmuxError:
        print("\n❌ SKIP: Could not connect to zmux socket")
        print("Tip: set ZMUX_TAG=<tag> or ZMUX_SOCKET_PATH=<path> to target a tagged instance.")
        return 0
