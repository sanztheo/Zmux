...
zmux wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["ZMUX_TEST_OUT"] = str(out_path)
    ...
    if ghostty_enabled:
        ...
        env["ZMUX_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"zmux_issue_734_{os.getpid()}"
    try:
