#!/usr/bin/env python3
"""
Regression test: `zmux claude-teams` supports Claude's tmux teammate flow.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zmux-claude-teams-seq-") as td:
        tmp = Path(td)
        home = tmp / "home"
...
printf '%s\\n' "${ZMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `zmux claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected ZMUX_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1

        window_target = read_text(window_target_log)
        if window_target != "zmux:1":
            print(f"FAIL: expected tmux window target 'zmux:1', got {window_target!r}")
            return 1
    ...
    print("PASS: zmux claude-teams supports Claude's tmux teammate flow")
    return 0
