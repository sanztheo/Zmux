#!/bin/bash
# Test script that sends keystrokes to zmux via AppleScript
# This tests the actual keyboard input path through the app

set -e

echo "=== zmux Keystroke Test ==="
echo ""

# Check if zmux is running
if ! pgrep -x "zmux" > /dev/null; then
    echo "Error: zmux is not running"
    echo "Please start zmux first"
    exit 1
fi

echo "zmux is running"
echo ""

# Activate zmux
osascript -e 'tell application "zmux" to activate'
sleep 0.5
...
echo "Please check the zmux window to verify:"
