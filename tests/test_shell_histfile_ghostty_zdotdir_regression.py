...
zmux also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If zmux incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    zmux_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (zmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing zmux wrapper .zshenv at {zmux_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"zmux_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: zmux stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["ZMUX_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(zmux_wrapper_dir)
        env["ZMUX_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  zmux_wrapper_dir={zmux_wrapper_dir}")
