...
# Catches issues like https://github.com/manaflow-ai/zmux/issues/110 where a race
# condition caused the cask to contain the SHA of a 404 page instead of the DMG.
set -euo pipefail

CASK_FILE="$(dirname "$0")/../homebrew-zmux/Casks/zmux.rb"

if [ ! -f "$CASK_FILE" ]; then
  echo "SKIP: homebrew-zmux submodule not initialized"
  exit 0
fi
