...
overlay used for focus tracking. Fix: ZmuxWebView.mouseDown posts a notification
...
def _marker(name: str) -> Path:
    return MARKER_DIR / f"zmux_mwf_{name}_{os.getpid()}"
...
def _verify_responsive(c: zmux, marker: Path, surface_idx: int, retries: int = 3) -> bool:
...
def _wait_terminal_in_window(c: zmux, surface_idx: int, timeout: float = 5.0) -> bool:
    start = time.time()
    while time.time() - start < timeout:
        try:
...
def test_multi_workspace_terminal_responsive(c: zmux) -> None:
    """
...
def test_three_workspaces_non_last_responsive(c: zmux) -> None:
    """
...
def test_rapid_workspace_switching_preserves_focus(c: zmux) -> None:
    """
...
def test_browser_panel_focus_and_return(c: zmux) -> None:
    """
    ...
    if not surfaces:
        raise zmuxError("No surfaces after new_workspace")
...
def test_browser_focus_across_workspaces(c: zmux) -> None:
    """
    ...
    with zmux() as c:
        ...
        for name, test_fn in tests:
            ...
            try:
                ...
                print("PASS")
                passed += 1
            except (AssertionError, zmuxError) as e:
                print(f"FAIL: {e}")
                failed += 1
