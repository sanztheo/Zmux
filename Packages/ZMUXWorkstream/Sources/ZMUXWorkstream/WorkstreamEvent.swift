...
/// zmux socket, then materialized into a `WorkstreamItem` by the store.
///
...
/// `_opencode_request_id`. `context` is zmux-specific and optional.
public struct WorkstreamEvent: Codable, Sendable, Equatable {
    ...
    /// zmux hook wrappers already emit on stdin so no translation layer is
    /// needed.
