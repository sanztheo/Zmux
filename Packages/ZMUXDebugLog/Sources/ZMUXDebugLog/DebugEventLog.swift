...
/// Ring-buffer event log used by zmux debug builds.
///
...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "zmux.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["ZMUX_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["ZMUX_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/zmux-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["ZMUX_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("zmux-debug-") {
                return "/tmp/\(socketBase).log"
            }
        }

        if let bundleId = Bundle.main.bundleIdentifier,
           bundleId != "com.zmuxterm.app.debug" {
            return "/tmp/zmux-debug-\(sanitizePathToken(bundleId)).log"
        }
