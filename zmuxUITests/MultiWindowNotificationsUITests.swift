...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/zmux-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/zmux-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["ZMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["ZMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["ZMUX_SOCKET_PATH"] = socketPath
        app.launchEnvironment["ZMUX_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["ZMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["ZMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "zmux_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["ZMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["ZMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveZmuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled zmux CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `zmux notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected zmux to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `zmux notify` command to finish without foregrounding zmux. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected zmux to remain in background after bundled `zmux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `zmux notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForZmuxPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runZmuxCommand(
        ...
        let result = runZmuxCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runZmuxCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: ZmuxCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["ZMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveZmuxCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled zmux CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeZmuxCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled zmux CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["zmux"] + args
        let fallbackResult = executeZmuxCommand(
    ...
    private enum ZmuxCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveZmuxCLIPaths(strategy: ZmuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["ZMUX_UI_TEST_CLI_PATH", "ZMUXTERM_CLI"] {
        ...
        candidates.append("/tmp/zmux-\(launchTag)/Build/Products/Debug/zmux DEV.app/Contents/Resources/bin/zmux")
        candidates.append("/tmp/zmux-\(launchTag)/Build/Products/Debug/zmux.app/Contents/Resources/bin/zmux")
        if strategy == .any {
            candidates.append("/tmp/zmux-\(launchTag)/Build/Products/Debug/zmux")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: ZmuxCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/zmux DEV.app/Contents/Resources/bin/zmux")
        candidates.append("\(productsDir)/zmux.app/Contents/Resources/bin/zmux")
        if strategy == .any {
            candidates.append("\(productsDir)/zmux")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/zmux")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "zmux" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeZmuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run zmux command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested ZMUX_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/zmux-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/zmux-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/zmux.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("zmux.sock", isDirectory: false)
            .path ?? "/tmp/zmux.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("zmux") && $0.hasSuffix(".sock") }
