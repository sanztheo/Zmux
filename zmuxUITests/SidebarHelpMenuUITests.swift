...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        let app = XCUIApplication()
        app.launchEnvironment["ZMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_FEED_URL"] = "https://zmux.test/appcast.xml"
        app.launchEnvironment["ZMUX_UI_TEST_FEED_MODE"] = "available"
        app.launchEnvironment["ZMUX_UI_TEST_UPDATE_VERSION"] = "9.9.9"
        app.launchEnvironment["ZMUX_UI_TEST_AUTO_ALLOW_PERMISSION"] = "1"
        launchAndActivate(app)
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["ZMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let debugDefaultsDomain = "com.zmuxterm.app.debug"
    private let hiddenSurfaceToken = "zmux-command-palette-hidden-surface"
    private let visibleSurfaceToken = "zmux-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "zmux-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/zmux-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/zmux-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "zmux.settings" && data["settingsWindowIsKey"] == "1"
        }
    ...
    func testMenuBarOnlyToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/zmux-ui-test-menu-bar-only-focus-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: diagnosticsPath)
