...
final class TerminalCmdClickUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        fixtureDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ui-test-terminal-cmd-click-\(UUID().uuidString)", isDirectory: true)
    ...
    private func launchApp(
        ...
        app.launchEnvironment["ZMUX_TAG"] = "ui-test-terminal-cmd-click"
        app.launchEnvironment["ZMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] = "1"
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_PATH"] = setupDataPath
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"] = commandPath
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"] = fixtureDirectoryURL.path
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"] = fileName
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"] = displayMode.rawValue
        app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"] = lineFormat.rawValue
        if !linePrefix.isEmpty {
            app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] = linePrefix
        }
        if !extraFileNames.isEmpty,
           let data = try? JSONSerialization.data(withJSONObject: extraFileNames, options: [.sortedKeys]),
           let json = String(data: data, encoding: .utf8) {
            app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"] = json
        }
        if captureOpenPaths {
            app.launchEnvironment["ZMUX_UI_TEST_CAPTURE_OPEN_PATH"] = openCapturePath
        }
        if captureHoverDiagnostics {
            app.launchEnvironment["ZMUX_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"] = hoverDiagnosticsPath
        }
        if let quicklookOverride {
            app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"] = quicklookOverride
        }
        if let viewportOffsetDelta {
            app.launchEnvironment["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"] = String(viewportOffsetDelta)
        }
