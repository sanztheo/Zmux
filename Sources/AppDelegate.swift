...
func zmuxJavaScriptStringLiteral(_ value: String?) -> String? {
...
final class ZmuxApplicationAccessibilityHierarchyCache {
    enum Resolution {
        case passthrough
        case handled(Any?)
    }
    ...
    static let shared = ZmuxApplicationAccessibilityHierarchyCache()
...
private enum ZmuxThemeNotifications {
    static let reloadConfig = Notification.Name("com.zmuxterm.themes.reload-config")
}
...
#if DEBUG
enum ZmuxTypingTiming {
    static let isEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["ZMUX_TYPING_TIMING_LOGS"] == "1" || environment["ZMUX_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        let defaults = UserDefaults.standard
        return defaults.bool(forKey: "zmuxTypingTimingLogs") || defaults.bool(forKey: "zmuxKeyLatencyProbe")
    }()
    static let isVerboseProbeEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["ZMUX_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "zmuxKeyLatencyProbe")
    }()
    ...
    static func logEventDelay(path: String, event: NSEvent) {
        ...
        zmuxDebugLog("typing.delay path=\(path) delayMs=\(format(delayMs)) \(eventFields(event))")
    }

    @inline(__always)
    static func logDuration(path: String, startedAt: TimeInterval?, event: NSEvent? = nil, extra: String? = nil) {
        ZmuxMainThreadTurnProfiler.endMeasure(path, startedAt: startedAt)
...
final class ZmuxMainRunLoopStallMonitor {
    static let shared = ZmuxMainRunLoopStallMonitor()
    ...
    func installIfNeeded() {
        guard ZmuxTypingTiming.isEnabled else { return }
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                ...
                let monitor = Unmanaged<ZmuxMainRunLoopStallMonitor>.fromOpaque(info).takeUnretainedValue()
...
final class ZmuxMainThreadTurnProfiler {
    static let shared = ZmuxMainThreadTurnProfiler()
    ...
    static func endMeasure(_ bucket: String, startedAt: TimeInterval?) {
        guard let startedAt, ZmuxTypingTiming.isEnabled, Thread.isMainThread else { return }
        let elapsedMs = max(0, (ProcessInfo.processInfo.systemUptime - startedAt) * 1000.0)
    ...
    func installIfNeeded() {
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                guard let info else { return }
                let profiler = Unmanaged<ZmuxMainThreadTurnProfiler>.fromOpaque(info).takeUnretainedValue()
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private static func detectRunningUnderXCTest(_ env: [String: String]) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("ZMUX_UI_TEST_") }) { return true }
        return false
    }
    ...
    private final class MainWindowContext {
        ...
        var zmuxConfigStore: ZmuxConfigStore?
        ...
        init(
            ...
            zmuxConfigStore: ZmuxConfigStore?,
            window: NSWindow?
        ) {
            ...
            self.zmuxConfigStore = zmuxConfigStore
            self.window = window
    ...
    private nonisolated static let persistedWindowGeometryDefaultsKey = "zmux.session.lastWindowGeometry.v2"
    private nonisolated static let legacyPersistedWindowGeometryDefaultsKeys = [
        "zmux.session.lastWindowGeometry.v1"
    ]
    ...
    private static let didInstallWindowKeyEquivalentSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.zmux_performKeyEquivalent(with:))
    ...
    private static let didInstallWindowFirstResponderSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.zmux_makeFirstResponder(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallWindowSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.zmux_sendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.zmux_applicationSendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationAccessibilitySwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.zmux_accessibilityAttributeValue(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private func childExitKeyboardProbePath() -> String? {
        ...
        guard env["ZMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = env["ZMUX_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private let sessionPersistenceQueue = DispatchQueue(
        label: "com.zmuxterm.app.sessionPersistence",
        qos: .utility
    )
    private nonisolated static let launchServicesRegistrationQueue = DispatchQueue(
        label: "com.zmuxterm.app.launchServicesRegistration",
        qos: .utility
    )
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: ZmuxThemeNotifications.reloadConfig,
            object: nil,
        ...
        ZmuxMainRunLoopStallMonitor.shared.installIfNeeded()
        ZmuxMainThreadTurnProfiler.shared.installIfNeeded()
        ...
        let forceDuplicateLaunchObserver = env["ZMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] == "1"
        ...
        if env["ZMUX_UI_TEST_MODE"] == "1" {
            let trigger = env["ZMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? "<nil>"
            let feed = env["ZMUX_UI_TEST_FEED_URL"] ?? "<nil>"
            UpdateLogStore.shared.append("ui test env: trigger=\(trigger) feed=\(feed)")
        }
        if env["ZMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" {
        ...
        if isRunningUnderXCTest {
            if let rawVariant = env["ZMUX_UI_TEST_BROWSER_IMPORT_HINT_VARIANT"] {
                UserDefaults.standard.set(
                    BrowserImportHintSettings.variant(for: rawVariant).rawValue,
                    forKey: BrowserImportHintSettings.variantKey
                )
            }
            if let rawShow = env["ZMUX_UI_TEST_BROWSER_IMPORT_HINT_SHOW"] {
                UserDefaults.standard.set(
                    rawShow == "1",
                    forKey: BrowserImportHintSettings.showOnBlankTabsKey
                )
            }
            if let rawDismissed = env["ZMUX_UI_TEST_BROWSER_IMPORT_HINT_DISMISSED"] {
                UserDefaults.standard.set(
                    rawDismissed == "1",
            ...
            if env["ZMUX_UI_TEST_BROWSER_IMPORT_HINT_OPEN_BLANK_BROWSER"] == "1" {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self] in
                    guard let self else { return }
                    _ = self.openBrowserAndFocusAddressBar(insertAtEnd: true)
                }
            }
            if env["ZMUX_UI_TEST_BROWSER_IMPORT_HINT_OPEN_SETTINGS"] == "1" {
            ...
            if env["ZMUX_UI_TEST_BROWSER_IMPORT_AUTO_OPEN"] == "1" {
    ...
    private func writeUITestDiagnosticsIfNeeded(stage: String) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["ZMUX_UI_TEST_DIAGNOSTICS_PATH"], !path.isEmpty else { return }
        ...
        let screenIDs = windows.map { $0.screen?.zmuxDisplayID.map(String.init) ?? "" }.joined(separator: ",")
        let targetDisplayID = env["ZMUX_UI_TEST_TARGET_DISPLAY_ID"] ?? ""
        ...
        if let rawDisplayID = UInt32(targetDisplayID) {
            let screenPresent = NSScreen.screens.contains(where: { $0.zmuxDisplayID == rawDisplayID })
            let movedWindow = windows.contains(where: { $0.screen?.zmuxDisplayID == rawDisplayID })
    ...
    private func appendUITestSocketDiagnosticsIfNeeded(
        ...
        guard env["ZMUX_UI_TEST_SOCKET_SANITY"] == "1" else { return }

        guard let config = socketListenerConfigurationIfEnabled() else {
            payload["socketExpectedPath"] = env["ZMUX_SOCKET_PATH"] ?? ""
    ...
    private func appendUITestRenderDiagnosticsIfNeeded(
        ...
        guard env["ZMUX_UI_TEST_DISPLAY_RENDER_STATS"] == "1" else { return }
    ...
    private func moveUITestWindowToTargetDisplayIfNeeded(attempt: Int = 0) {
        let env = ProcessInfo.processInfo.environment
        guard let rawDisplayID = env["ZMUX_UI_TEST_TARGET_DISPLAY_ID"],
              let targetDisplayID = UInt32(rawDisplayID) else {
            return
        }

        guard let screen = NSScreen.screens.first(where: { $0.zmuxDisplayID == targetDisplayID }) else {
            if attempt < 20 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
        ...
        window.orderFrontRegardless()
        if window.screen?.zmuxDisplayID != targetDisplayID, attempt < 20 {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
                self?.moveUITestWindowToTargetDisplayIfNeeded(attempt: attempt + 1)
            }
            return
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        DispatchQueue.main.async {
            ...
            alert.messageText = String(localized: "dialog.quitZmux.title", defaultValue: "Quit zmux?")
            alert.informativeText = String(localized: "dialog.quitZmux.message", defaultValue: "This will close all windows and workspaces.")
            alert.addButton(withTitle: String(localized: "dialog.quitZmux.quit", defaultValue: "Quit"))
    ...
    func configure(tabManager: TabManager, notificationStore: TerminalNotificationStore, sidebarState: SidebarState) {
        ...
        let env = ProcessInfo.processInfo.environment
        if isRunningUnderXCTest(env) || env["ZMUX_UI_TEST_MODE"] == "1" {
    ...
    private func setupTerminalCmdClickUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] == "1" else {
            zmuxDebugLog("cmdclick.ui.setup skip reason=env_missing tag=\(env["ZMUX_TAG"] ?? "nil")")
            return
        }
        guard let manifestPath = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !manifestPath.isEmpty else {
            zmuxDebugLog("cmdclick.ui.setup skip reason=missing_manifest_path")
            return
        }
        didSetupTerminalCmdClickUITest = true
        guard let fixtureDirectory = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !fixtureDirectory.isEmpty else {
            zmuxDebugLog("cmdclick.ui.setup error reason=missing_fixture_dir manifest=\(manifestPath)")
            writeTerminalCmdClickUITestData(at: manifestPath, updates: [
                "setupError": "Missing fixture directory"
            ])
            return
        }
        let commandPath = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let displayMode = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let lineFormat = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let linePrefix = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] ?? ""
        let extraFileNamesJSON = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)

        let fileName = env["ZMUX_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        zmuxDebugLog("cmdclick.ui.setup poller_started manifest=\(manifestPath)")
        poller.resume()
    }
    ...
    private func writeTerminalCmdClickUITestData(at path: String, updates: [String: Any]) {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: [.sortedKeys]) else {
            zmuxDebugLog("cmdclick.ui.write skip reason=json path=\(path)")
            return
        }
        do {
            try FileManager.default.createDirectory(
            ...
            zmuxDebugLog("cmdclick.ui.write error path=\(path) error=\(error.localizedDescription)")
        }
    }
    ...
    private func currentDisplayGeometries() -> (
        ...
        let available = NSScreen.screens.map { screen in
            SessionDisplayGeometry(
                displayID: screen.zmuxDisplayID,
                frame: screen.frame,
...
#if DEBUG
            zmuxDebugLog("session.save.skipped reason=session_restore_in_progress includeScrollback=0")
#endif
            return false
        }
...
#if DEBUG
        let timingStart = ZmuxTypingTiming.start()
        defer {
            ZmuxTypingTiming.logDuration(
    ...
    func registerMainWindow(
        ...
        zmuxConfigStore: ZmuxConfigStore? = nil
    ) {
        ...
        if let existing = mainWindowContexts[key] {
            ...
            if let zmuxConfigStore {
                existing.zmuxConfigStore = zmuxConfigStore
            }
...
#if DEBUG
            zmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=sourcePanelNotFound elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let sourceWorkspace = source.tabManager.tabs.first(where: { $0.id == source.workspaceId }) else {
#if DEBUG
            zmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=sourceWorkspaceMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let destinationManager = tabManagerFor(tabId: targetWorkspaceId) else {
#if DEBUG
            zmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=destinationManagerMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let destinationWorkspace = destinationManager.tabs.first(where: { $0.id == targetWorkspaceId }) else {
#if DEBUG
            zmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=destinationWorkspaceMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
#if DEBUG
    ...
    private func pruneExpiredCommandPalettePendingOpenStates(
        ...
        for windowId in Array(commandPalettePendingOpenByWindowId.keys) {
            ...
            guard let requestedAt = commandPaletteRecentRequestAtByWindowId[windowId] else {
                commandPalettePendingOpenByWindowId.removeValue(forKey: windowId)
#if DEBUG
                zmuxDebugLog("shortcut.palette.pendingPrune windowId=\(windowId.uuidString.prefix(8)) reason=missingTimestamp")
#endif
                continue
            }
...
#if DEBUG
        zmuxDebugLog("shortcut.escape suppressionClear target={nil} clearedAll=1 keyUpConsumed=\(didConsume ? 1 : 0)")
#endif
        return didConsume
    }
    ...
    private func keyRoutingOwnerView(for responder: NSResponder?) -> NSView? {
        ...
        if let editor = responder as? NSTextView,
           editor.isFieldEditor {
            return zmuxFieldEditorOwnerView(editor) ?? editor
        }
    ...
    private func responderHasViableKeyRoutingOwner(
        ...
        if let ghosttyView = zmuxOwningGhosttyView(for: responder) {
...
#if DEBUG
        zmuxDebugLog("reload.config.surfaceRefresh source=\(source) count=\(refreshedCount)")
#endif
    }
    ...
    private func windowForMainWindowId(_ windowId: UUID) -> NSWindow? {
        ...
        let expectedIdentifier = "zmux.main.\(windowId.uuidString)"
    ...
    private func mainWindowId(for window: NSWindow) -> UUID? {
        ...
        guard let rawIdentifier = window.identifier?.rawValue,
              rawIdentifier.hasPrefix("zmux.main.") else { return nil }
        let idPart = String(rawIdentifier.dropFirst("zmux.main.".count))
    ...
    private func focusedTerminalShortcutContext(preferredWindow: NSWindow? = nil) -> FocusedTerminalShortcutContext? {
        ...
        guard let ghosttyView = zmuxOwningGhosttyView(for: responder),
    ...
    func bootstrapInitialMainWindowIfNeeded(debugSource: String, shouldActivate: Bool = true) -> UUID {
        ...
        if ProcessInfo.processInfo.environment["ZMUX_UI_TEST_SHOW_SETTINGS"] == "1" {
    ...
    private func executeConfiguredNewWorkspaceCommandIfAvailable(
        ...
        guard let zmuxConfigStore = context.zmuxConfigStore,
              let configured = zmuxConfigStore.resolvedNewWorkspaceCommand() else {
            return false
        }
        ...
        let didExecute = ZmuxConfigExecutor.execute(
            command: configured.command,
            tabManager: context.tabManager,
            baseCwd: baseCwd,
            configSourcePath: configured.sourcePath,
            globalConfigPath: zmuxConfigStore.globalConfigPath
    ...
    private func preferredMainWindowContextForShortcutRouting(event: NSEvent) -> MainWindowContext? {
        ...
        if shortcutEventHasAddressableWindow(event) {
            if let eventWindow = resolvedShortcutEventWindow(event),
               zmuxWindowShouldOwnCloseShortcut(eventWindow) {
                // Auxiliary zmux windows do not own a terminal tab manager. Let them fall back
    ...
    func createMainWindow(
        ...
        let zmuxConfigStore = ZmuxConfigStore()
        zmuxConfigStore.wireDirectoryTracking(tabManager: tabManager)
        zmuxConfigStore.loadAll()
        ...
        let root = ContentView(updateViewModel: updateViewModel, windowId: windowId)
            ...
            .environmentObject(zmuxConfigStore)
        ...
        // zmux persists and restores main windows itself. Disable AppKit window
    ...
    func sendWelcomeCommandWhenReady(to workspace: Workspace, markShownOnSend: Bool = false) {
        sendTextWhenReady("zmux welcome\n", to: workspace) {
            if markShownOnSend {
    ...
    func isZmuxCLIInstalledInPATH() -> Bool {
        ZmuxCLIPathInstaller().isInstalled()
    }

    @objc func installZmuxCLIInPath(_ sender: Any?) {
        let installer = ZmuxCLIPathInstaller()
        do {
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.installed", defaultValue: "zmux CLI Installed"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.installFailed", defaultValue: "Couldn't Install zmux CLI"),
    ...
    @objc func uninstallZmuxCLIInPath(_ sender: Any?) {
        let installer = ZmuxCLIPathInstaller()
        do {
            ...
            let prefix = outcome.removedExistingEntry
                ...
                : String(localized: "cli.uninstall.notFound", defaultValue: "No zmux CLI symlink was found at \(outcome.destinationURL.path).")
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.uninstalled", defaultValue: "zmux CLI Uninstalled"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.uninstallFailed", defaultValue: "Couldn't Uninstall zmux CLI"),
                informativeText: error.localizedDescription,
                style: .warning
            )
        }
...
#if DEBUG
        zmuxDebugLog("settings.open.present path=customWindowDirect")
#endif
...
#if DEBUG
        zmuxDebugLog("settings.open.present activate=1")
#endif
    }
...
#if DEBUG
        zmuxDebugLog("settings.open.request source=\(debugSource)")
#endif
    ...
    private func waitForDebugStressMountedWorkspaces(_ workspaces: [Workspace]) async -> Int {
        ...
        zmuxDebugLog("stress.setup.mount mounted=\(mountedWorkspaceCount)/\(workspaces.count)")
    ...
    private func setupJumpUnreadUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1" else { return }
        guard let notificationStore else { return }
    ...
    func armJumpUnreadFocusRecord(tabId: UUID, surfaceId: UUID) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["ZMUX_UI_TEST_JUMP_UNREAD_PATH"], !path.isEmpty else { return }
    ...
    private func setupGotoSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }

        let useGhosttyConfig = env["ZMUX_UI_TEST_GOTO_SPLIT_USE_GHOSTTY_CONFIG"] == "1"
        ...
        func runSetupWhenWindowReady() {
            ...
            let requestedBrowserURL = env["ZMUX_UI_TEST_GOTO_SPLIT_BROWSER_URL"]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func setupBonsplitTabDragUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }
        let startWithHiddenSidebar = env["ZMUX_UI_TEST_BONSPLIT_START_WITH_HIDDEN_SIDEBAR"] == "1"
    ...
    private func bonsplitTabDragUITestDataPath() -> String? {
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1",
              let path = env["ZMUX_UI_TEST_BONSPLIT_TAB_DRAG_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private func isGotoSplitUITestRecordingEnabled() -> Bool {
        let env = ProcessInfo.processInfo.environment
        return env["ZMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" || env["ZMUX_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1"
    }

    private func gotoSplitUITestDataPath() -> String? {
        guard isGotoSplitUITestRecordingEnabled() else { return nil }
        let env = ProcessInfo.processInfo.environment
        guard let path = env["ZMUX_UI_TEST_GOTO_SPLIT_PATH"], !path.isEmpty else { return nil }
        return path
    }
    ...
    private func focusWebViewForGotoSplitUITest(tab: Workspace, browserPanelId: UUID) {
        ...
        func recordFocusedState() {
            ...
            if ProcessInfo.processInfo.environment["ZMUX_UI_TEST_GOTO_SPLIT_INPUT_SETUP"] == "1" {
        ...
        (() => {
          const snapshot = () => {
            ...
            return {
              ...
              trackerInstalled: window.__zmuxAddressBarFocusTrackerInstalled === true,
              trackedStateId:
                window.__zmuxAddressBarFocusState &&
                typeof window.__zmuxAddressBarFocusState.id === "string"
                  ? window.__zmuxAddressBarFocusState.id
                  : "",
          ...
          const seed = () => {
            ...
            let container = document.getElementById("zmux-ui-test-focus-container");
            if (!container || !container.tagName || container.tagName.toLowerCase() !== "div") {
              container = document.createElement("div");
              container.id = "zmux-ui-test-focus-container";
            ...
            const input = ensureInput("zmux-ui-test-focus-input", "zmux-ui-focus-primary");
            const secondaryInput = ensureInput("zmux-ui-test-focus-input-secondary", "zmux-ui-focus-secondary");
            ...
            let trackedFocusId = input.getAttribute("data-zmux-addressbar-focus-id");
            if (!trackedFocusId) {
              trackedFocusId = "zmux-ui-test-focus-input-tracked";
              input.setAttribute("data-zmux-addressbar-focus-id", trackedFocusId);
            }
            ...
            if (
              !window.__zmuxAddressBarFocusState ||
              typeof window.__zmuxAddressBarFocusState.id !== "string" ||
              window.__zmuxAddressBarFocusState.id !== trackedFocusId
            ) {
              window.__zmuxAddressBarFocusState = { id: trackedFocusId, selectionStart, selectionEnd };
            }
          ...
          const ready = () =>
            window.__zmuxAddressBarFocusTrackerInstalled === true &&
    ...
    private func evaluateGotoSplitUITestActiveElement(
        ...
        let expectedInputIdLiteral = zmuxJavaScriptStringLiteral(awaitingInputId) ?? "null"
        let script = """
        (() => {
            ...
            try {
              ...
              if (!active) {
                return {
                  ...
                  editable: "false",
                  trackedFocusStateId: "",
                  focusTrackerInstalled: window.__zmuxAddressBarFocusTrackerInstalled === true ? "true" : "false"
                };
              }
              ...
              return {
                ...
                focusTrackerInstalled:
                  window.__zmuxAddressBarFocusTrackerInstalled === true ? "true" : "false"
              };
            } catch (_) {
              return {
                id: "",
    ...
    private func setupMultiWindowNotificationsUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] == "1" else { return }
        guard let path = env["ZMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"], !path.isEmpty else { return }
    ...
    private func prepareMultiWindowNotificationSourceTerminalIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["ZMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] == "1" else { return }
    ...
    private func publishMultiWindowNotificationSocketStateIfNeeded(at path: String) {
        ...
        guard let config = socketListenerConfigurationIfEnabled() else {
            writeMultiWindowNotificationTestData([
                "socketExpectedPath": env["ZMUX_SOCKET_PATH"] ?? "",
                "socketMode": "off",
                "socketReady": "0",
                "socketPingResponse": "",
                "socketIsRunning": "0",
    ...
    func jumpToLatestUnread() {
        guard let notificationStore else { return }
#if DEBUG
        if ProcessInfo.processInfo.environment["ZMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1" {
            writeJumpUnreadTestData([
                "jumpUnreadInvoked": "1",
    ...
    static func setWindowFirstResponderGuardTesting(currentEvent: NSEvent?, hitView: NSView?) {
        zmuxFirstResponderGuardCurrentEventOverride = currentEvent
        zmuxFirstResponderGuardHitViewOverride = hitView
    }

    static func clearWindowFirstResponderGuardTesting() {
        zmuxFirstResponderGuardCurrentEventOverride = nil
        zmuxFirstResponderGuardHitViewOverride = nil
    }
#endif
    ...
    private func installShortcutMonitor() {
        ...
        shortcutMonitor = NSEvent.addLocalMonitorForEvents(
            ...
            if event.type == .keyDown || event.type == .systemDefined {
                ...
                ZmuxTypingTiming.logEventDelay(path: "appMonitor", event: event)
                let shortcutMonitorTraceEnabled =
                    ProcessInfo.processInfo.environment["ZMUX_SHORTCUT_MONITOR_TRACE"] == "1"
                    || UserDefaults.standard.bool(forKey: "zmuxShortcutMonitorTrace")
                ...
                let shortcutTimingStart = ZmuxTypingTiming.start()
#endif
...
#if DEBUG
                    zmuxDebugLog("  → consumed by handleCustomShortcut")
#endif
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
...
#if DEBUG
                zmuxDebugLog("shortcut.escape paletteDismiss consumed=1 target={\(debugWindowToken(paletteWindow))}")
#endif
                return true
            }
        ...
        if browserAddressBarFocusedPanelId != nil,
           zmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder) != nil {
#if DEBUG
        ...
        if !normalizedFlags.contains(.command),
           let ghosttyView = zmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder),
...
#if DEBUG
            zmuxDebugLog("handleCustomShortcut: unresolved event window context; bypassing app shortcut handling")
#endif
            return false
        }
        ...
        if isControlD {
            ...
            let frType = NSApp.keyWindow?.firstResponder.map { String(describing: type(of: $0)) } ?? "nil"
            zmuxDebugLog("shortcut.ctrlD stage=preReconcile selected=\(selected) focused=\(focused) fr=\(frType)")
#endif
            ...
            zmuxDebugLog("shortcut.ctrlD stage=postReconcile fr=\(frAfterType)")
        ...
        let configuredZmuxShortcutContext = preferredMainWindowContextForShortcutRouting(event: event)
        let configuredZmuxShortcutActions = configuredZmuxShortcutActions(for: configuredZmuxShortcutContext)

        if activeConfiguredShortcutChordPrefixForCurrentEvent == nil,
           armConfiguredShortcutChordIfNeeded(
               event: event,
               shortcuts: configuredZmuxShortcutActions.compactMap(\.shortcut)
           ) {
            return true
        }
        ...
        if handleConfiguredZmuxShortcut(
            event: event,
            actions: configuredZmuxShortcutActions,
            context: configuredZmuxShortcutContext
        ) {
            return true
        }
...
#if DEBUG
            zmuxDebugLog("shortcut.action name=newWorkspace \(debugShortcutRouteSnapshot(event: event))")
#endif
        ...
        if matchConfiguredShortcut(event: event, action: .closeOtherTabsInPane) {
            if let targetWindow = event.window ?? NSApp.keyWindow ?? NSApp.mainWindow,
               targetWindow.identifier?.rawValue == "zmux.settings" {
        ...
        if matchConfiguredShortcut(event: event, action: .closeTab) {
            ...
            if let targetWindow = [targetWindow, NSApp.keyWindow]
                .compactMap({ $0 })
                .first(where: { $0.identifier?.rawValue == "zmux.browser-popup" }) {
#if DEBUG
                zmuxDebugLog("shortcut.cmdW route=browserPopup")
#endif
                targetWindow.performClose(nil)
                return true
            } else if let targetWindow,
               zmuxWindowShouldOwnCloseShortcut(targetWindow) {
                targetWindow.performClose(nil)
            } else {
...
#if DEBUG
                    zmuxDebugLog("shortcut.cmdW route=noManager")
#endif
                    return false
                }
            }
...
#if DEBUG
            zmuxDebugLog("shortcut.action name=splitRight \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            zmuxDebugLog("shortcut.action name=splitDown \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            zmuxDebugLog("shortcut.action name=splitBrowserRight \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            zmuxDebugLog("shortcut.action name=splitBrowserDown \(debugShortcutRouteSnapshot(event: event))")
#endif
    ...
    private func scheduleBrowserOmnibarSelectionRepeatTick() {
        ...
        guard browserAddressBarFocusedPanelId != nil else {
#if DEBUG
            zmuxDebugLog("browser.focus.omnibar.repeat.tick result=stop_no_focused_address_bar")
#endif
            stopBrowserOmnibarSelectionRepeat()
            return
        }
    ...
    private func isLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
        zmuxIsLikelyWebInspectorResponder(responder)
    }
    ...
    func performSplitShortcut(direction: SplitDirection, preferredWindow: NSWindow? = nil) -> Bool {
        ...
        if let browser = tabManager?.focusedBrowserPanel {
            ...
            zmuxDebugLog("split.shortcut dir=\(directionLabel) pre panel=\(browser.id.uuidString.prefix(5)) \(browser.debugDeveloperToolsStateSummary()) webWin=\(webWindow) webSuper=\(webSuperview) \(splitContext)")
        } else {
            zmuxDebugLog("split.shortcut dir=\(directionLabel) pre panel=nil \(splitContext)")
        }
        #endif
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { [weak self] in
            ...
            if let browser = self?.tabManager?.focusedBrowserPanel {
                let webWindow = browser.webView.window?.windowNumber ?? -1
                let webSuperview = browser.webView.superview.map { String(describing: Unmanaged.passUnretained($0).toOpaque()) } ?? "nil"
                zmuxDebugLog("split.shortcut dir=\(directionLabel) post panel=\(browser.id.uuidString.prefix(5)) \(browser.debugDeveloperToolsStateSummary()) webWin=\(webWindow) webSuper=\(webSuperview) \(splitContext)")
            } else {
                zmuxDebugLog("split.shortcut dir=\(directionLabel) post panel=nil \(splitContext)")
            }
        }
    ...
    func performBrowserSplitShortcut(direction: SplitDirection) -> Bool {
        ...
        guard let panelId = tabManager?.createBrowserSplit(direction: direction) else {
            #if DEBUG
            zmuxDebugLog("split.browser.shortcut failed dir=\(directionLabel)")
            #endif
            return false
        }
    ...
    private func configuredZmuxShortcutActions(
        for context: MainWindowContext?
    ) -> [ZmuxResolvedConfigAction] {
        context?.zmuxConfigStore?.shortcutActions() ?? []
    }

    private func handleConfiguredZmuxShortcut(
        event: NSEvent,
        actions: [ZmuxResolvedConfigAction],
        ...
        for action in actions {
            ...
            return executeConfiguredZmuxActionShortcut(
                action,
                event: event,
                context: context
            )
    ...
    private func executeConfiguredZmuxActionShortcut(
        _ action: ZmuxResolvedConfigAction,
        event: NSEvent,
        context: MainWindowContext?
    ) -> Bool {
        ...
        case .command, .agent, .workspaceCommand:
            guard let context,
                  let zmuxConfigStore = context.zmuxConfigStore else {
                return false
            }
            let rawCwd = context.tabManager.selectedWorkspace?.currentDirectory
            let baseCwd = (rawCwd?.isEmpty == false) ? rawCwd!
                : FileManager.default.homeDirectoryForCurrentUser.path
            return ZmuxConfigExecutor.execute(
                action: action,
                commands: zmuxConfigStore.loadedCommands,
                commandSourcePaths: zmuxConfigStore.commandSourcePaths,
                tabManager: context.tabManager,
                baseCwd: baseCwd,
                globalConfigPath: zmuxConfigStore.globalConfigPath
            )
    ...
    private func handleFeedNotificationResponse(_ response: UNNotificationResponse) -> Bool {
        ...
        guard categoryId == "ZMUXFeedPermission"
    ...
    private func observeDuplicateLaunches() {
        ...
        let embeddedCLIURL = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Resources/bin/zmux", isDirectory: false)
            .standardizedFileURL
    ...
    private func installBrowserAddressBarFocusObservers() {
...
#if DEBUG
            zmuxDebugLog("addressBar FOCUS panelId=\(panelId.uuidString.prefix(8))")
#endif
        }
        ...
        browserAddressBarBlurObserver = NotificationCenter.default.addObserver(
            ...
            if self.browserAddressBarFocusedPanelId == panelId {
                ...
                self.stopBrowserOmnibarSelectionRepeat()
#if DEBUG
                zmuxDebugLog("addressBar BLUR panelId=\(panelId.uuidString.prefix(8))")
#endif
            }
        }
        ...
        browserWebViewFirstResponderObserver = NotificationCenter.default.addObserver(
            ...
            guard let self else { return }
            guard let webView = notification.object as? ZmuxWebView,
    ...
    func browserFindBarIsVisible(for webView: ZmuxWebView) -> Bool {
    ...
    private func shouldLetFocusedBrowserOwnFindShortcut(_ event: NSEvent) -> Bool {
        let shortcutWindow = resolvedShortcutEventWindow(event) ?? NSApp.keyWindow ?? NSApp.mainWindow
        let shortcutResponder = shortcutWindow?.firstResponder
        let owningWebView = tabManager?.focusedBrowserPanel?.webView as? ZmuxWebView
    ...
    private func browserPanelOwning(_ webView: ZmuxWebView) -> BrowserPanel? {
    ...
    private func browserPanelOwning(_ webView: ZmuxWebView, in manager: TabManager) -> BrowserPanel? {
        for workspace in manager.tabs {
    ...
    func closeMainWindowContainingTabId(_ tabId: UUID) {
        ...
        let expectedIdentifier = "zmux.main.\(context.windowId.uuidString)"
...
#if DEBUG
        let isJumpUnreadUITest = ProcessInfo.processInfo.environment["ZMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1"
...
#if DEBUG
private var zmuxFirstResponderGuardCurrentEventOverride: NSEvent?
private var zmuxFirstResponderGuardHitViewOverride: NSView?
#endif
private var zmuxFirstResponderGuardCurrentEventContext: NSEvent?
private var zmuxFirstResponderGuardHitViewContext: NSView?
private var zmuxFirstResponderGuardContextWindowNumber: Int?
private var zmuxBrowserReturnForwardingDepth = 0
private var zmuxBrowserArrowForwardingDepth = 0
private var zmuxWindowFirstResponderBypassDepth = 0
private var zmuxFieldEditorOwningWebViewAssociationKey: UInt8 = 0

@discardableResult
func zmuxWithWindowFirstResponderBypass<T>(_ body: () -> T) -> T {
    zmuxWindowFirstResponderBypassDepth += 1
    defer {
        zmuxWindowFirstResponderBypassDepth = max(0, zmuxWindowFirstResponderBypassDepth - 1)
    }
    return body()
}

func zmuxIsWindowFirstResponderBypassActive() -> Bool {
    zmuxWindowFirstResponderBypassDepth > 0
}

private final class ZmuxFieldEditorOwningWebViewBox: NSObject {
    weak var webView: ZmuxWebView?

    init(webView: ZmuxWebView?) {
        self.webView = webView
    }
}

private extension NSApplication {
    @objc func zmux_accessibilityAttributeValue(_ attribute: NSAccessibility.Attribute) -> Any? {
        if Thread.isMainThread {
            switch ZmuxApplicationAccessibilityHierarchyCache.shared.resolve(
                attribute: attribute,
                application: self
            ) {
        ...
        return zmux_accessibilityAttributeValue(attribute)
    }

    @objc func zmux_applicationSendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? ZmuxTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        if event.type == .keyDown {
            ZmuxTypingTiming.logEventDelay(path: "app.sendEvent", event: event)
        }
        defer {
            if event.type == .keyDown {
                let totalMs = (ProcessInfo.processInfo.systemUptime - phaseTotalStart) * 1000.0
...
#endif
        zmux_applicationSendEvent(event)
    }
}
...
private extension AppDelegate {
    @objc func handleThemesReloadNotification(_ notification: Notification) {
        DispatchQueue.main.async {
            GhosttyApp.shared.reloadConfiguration(source: "distributed.zmux.themes")
        }
    }
}

private extension NSWindow {
    @objc func zmux_makeFirstResponder(_ responder: NSResponder?) -> Bool {
        if zmuxIsWindowFirstResponderBypassActive() {
#if DEBUG
            zmuxDebugLog(
        ...
        let currentEvent = Self.zmuxCurrentEvent(for: self)
        let responderWebView = responder.flatMap {
            Self.zmuxOwningWebView(for: $0, in: self, event: currentEvent)
        }
        ...
        if let request = AppDelegate.shared?.terminalKeyboardFocusRequest(for: responder),
           Self.zmuxShouldAllowPointerInitiatedTerminalFocus(
               window: self,
               request: request,
               event: currentEvent
           ) {
        ...
        if let responder,
           ...
           !webView.allowsFirstResponderAcquisitionEffective {
            let pointerInitiatedFocus = Self.zmuxShouldAllowPointerInitiatedWebViewFocus(
                window: self,
                webView: webView,
                event: currentEvent
            )
        ...
        if pointerInitiatedWebFocus, let webView = responderWebView {
            // `NSWindow.makeFirstResponder` may run before `ZmuxWebView.mouseDown(with:)`.
            // Preserve pointer intent during this synchronous responder change.
            result = webView.withPointerFocusAllowance {
                zmux_makeFirstResponder(responder)
            }
        } else {
            result = zmux_makeFirstResponder(responder)
        }
        if result {
            if let fieldEditor = responder as? NSTextView, fieldEditor.isFieldEditor {
                Self.zmuxTrackFieldEditor(fieldEditor, owningWebView: responderWebView)
    ...
    @objc func zmux_sendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? ZmuxTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        ...
        let typingTimingExtra: String? = {
            ...
            let responderWebView = self.firstResponder.flatMap {
                Self.zmuxOwningWebView(for: $0, in: self, event: event)
            }
            let hitWebView = Self.zmuxHitViewForEventDispatch(in: self, event: event).flatMap {
                Self.zmuxOwningWebView(for: $0)
            }
        ...
        if event.type == .keyDown {
            ZmuxTypingTiming.logEventDelay(path: "window.sendEvent", event: event)
        }
#endif
...
#endif
        let previousContextEvent = zmuxFirstResponderGuardCurrentEventContext
        let previousContextHitView = zmuxFirstResponderGuardHitViewContext
        let previousContextWindowNumber = zmuxFirstResponderGuardContextWindowNumber
        zmuxFirstResponderGuardCurrentEventContext = event
        zmuxFirstResponderGuardHitViewContext = Self.zmuxHitViewForEventDispatch(in: self, event: event)
        zmuxFirstResponderGuardContextWindowNumber = self.windowNumber
#if DEBUG
        if event.type == .keyDown {
...
#endif
        defer {
            zmuxFirstResponderGuardCurrentEventContext = previousContextEvent
            zmuxFirstResponderGuardHitViewContext = previousContextHitView
            zmuxFirstResponderGuardContextWindowNumber = previousContextWindowNumber
        }
        ...
        zmuxDebugLog("window.sendEvent.folderDown suppress=1 hit=\(hitDesc) wasMovable=\(previousMovableState)")
        #endif
        ...
        #if DEBUG
        zmuxDebugLog("window.sendEvent.folderDown restore nowMovable=\(isMovable)")
        #endif
    }

    @objc func zmux_performKeyEquivalent(with event: NSEvent) -> Bool {
#if DEBUG
        let typingTimingStart = ZmuxTypingTiming.start()
        defer {
            ZmuxTypingTiming.logDuration(
        ...
        zmuxDebugLog("performKeyEquiv: \(Self.keyDescription(event)) fr=\(frType)")
#endif
        ...
        let firstResponderGhosttyView = zmuxOwningGhosttyView(for: self.firstResponder)
        ...
        if let ghosttyView = firstResponderGhosttyView {
            ...
            if ghosttyView.hasMarkedText(), !event.modifierFlags.intersection(.deviceIndependentFlagsMask).contains(.command) {
                return zmux_performKeyEquivalent(with: event)
            }
...
#if DEBUG
                zmuxDebugLog("  → ghostty direct: \(result)")
#endif
                return result
            }
...
#if DEBUG
                zmuxDebugLog("zoom.shortcut stage=window.ghosttyKeyDownDirect event=\(Self.keyDescription(event)) handled=1")
#endif
                return true
            }
        }
        ...
        if shouldDispatchBrowserReturnViaFirstResponderKeyDown(
            ...
            if zmuxBrowserReturnForwardingDepth > 0 {
#if DEBUG
                zmuxDebugLog("  → browser Return/Enter reentry; using normal dispatch")
#endif
                return false
            }
            zmuxBrowserReturnForwardingDepth += 1
            defer { zmuxBrowserReturnForwardingDepth = max(0, zmuxBrowserReturnForwardingDepth - 1) }
#if DEBUG
            zmuxDebugLog("  → browser Return/Enter routed to firstResponder.keyDown")
#endif
        ...
        if shouldDispatchBrowserArrowViaFirstResponderKeyDown(
            ...
            if zmuxBrowserArrowForwardingDepth > 0 {
#if DEBUG
                zmuxDebugLog("  → browser Up/Down reentry; using normal dispatch")
#endif
                return false
            }
            zmuxBrowserArrowForwardingDepth += 1
            defer { zmuxBrowserArrowForwardingDepth = max(0, zmuxBrowserArrowForwardingDepth - 1) }
#if DEBUG
            zmuxDebugLog("  → browser Up/Down routed to firstResponder.keyDown")
#endif
            self.firstResponder?.keyDown(with: event)
            return true
        }
...
#if DEBUG
            if result {
                zmuxDebugLog("  → browser find command resolved before window menu path")
            } else {
                zmuxDebugLog("  → browser find command preflight left unclaimed; suppressing replay")
            }
#endif
...
#if DEBUG
            zmuxDebugLog("  → consumed by handleBrowserSurfaceKeyEquivalent")
#endif
            return true
        }
...
#if DEBUG
                zmuxDebugLog("  → mainMenu miss; ghostty command path: \(consumedByGhostty)")
#endif
...
#if DEBUG
                zmuxDebugLog("  → consumed by mainMenu (bypassed SwiftUI)")
#endif
                return true
            }
        }

        let result = zmux_performKeyEquivalent(with: event)
#if DEBUG
        if result { zmuxDebugLog("  → consumed by original performKeyEquivalent") }
#endif
        return result
    }
    ...
    private static func zmuxOwningWebView(for responder: NSResponder) -> ZmuxWebView? {
        if let webView = responder as? ZmuxWebView {
            return webView
        }

        if let view = responder as? NSView,
           let webView = zmuxOwningWebView(for: view) {
            return webView
        }
        ...
        while let next = current {
            if let webView = next as? ZmuxWebView {
                return webView
            }
    ...
    private static func zmuxOwningWebView(
        ...
        if let webView = zmuxOwningWebView(for: responder) {
            return webView
        }
        ...
        if let event,
           let hitWebView = zmuxPointerHitWebView(in: window, event: event) {
            zmuxTrackFieldEditor(textView, owningWebView: hitWebView)
            return hitWebView
        }

        return zmuxTrackedOwningWebView(for: textView)
    }

    private static func zmuxOwningWebView(for view: NSView) -> ZmuxWebView? {
        if let webView = view as? ZmuxWebView {
            return webView
        }

        var current: NSView? = view.superview
        while let candidate = current {
            if let webView = candidate as? ZmuxWebView {
                return webView
            }
            if String(describing: type(of: candidate)).contains("WindowBrowserSlotView"),
               let portalWebView = zmuxUniqueBrowserWebView(in: candidate) {
                ...
                if zmuxAllowsPortalSlotTextEntryFocus(view) {
                    return nil
                }
                return portalWebView
            }
    ...
    private static func zmuxAllowsPortalSlotTextEntryFocus(_ view: NSView) -> Bool {
        var current: NSView? = view
        while let candidate = current {
    ...
    private static func zmuxUniqueBrowserWebView(in root: NSView) -> ZmuxWebView? {
        var stack: [NSView] = [root]
        var found: ZmuxWebView?
        while let current = stack.popLast() {
            if let webView = current as? ZmuxWebView {
                if found == nil {
                    found = webView
    ...
    private static func zmuxCurrentEvent(for window: NSWindow) -> NSEvent? {
#if DEBUG
        if let override = zmuxFirstResponderGuardCurrentEventOverride {
            return override
        }
#endif
        if zmuxFirstResponderGuardContextWindowNumber == window.windowNumber {
            return zmuxFirstResponderGuardCurrentEventContext
        }
        return NSApp.currentEvent
    }

    private static func zmuxHitViewInThemeFrame(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func zmuxHitViewInContentView(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func zmuxTopHitViewForEvent(in window: NSWindow, event: NSEvent) -> NSView? {
        if let hitInThemeFrame = zmuxHitViewInThemeFrame(in: window, event: event) {
            return hitInThemeFrame
        }
        return zmuxHitViewInContentView(in: window, event: event)
    }

    private static func zmuxHitViewForEventDispatch(in window: NSWindow, event: NSEvent) -> NSView? {
        ...
        return zmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func zmuxHitViewForCurrentEvent(in window: NSWindow, event: NSEvent) -> NSView? {
#if DEBUG
        if let override = zmuxFirstResponderGuardHitViewOverride {
            return override
        }
#endif
        if zmuxFirstResponderGuardContextWindowNumber == window.windowNumber,
           let contextHitView = zmuxFirstResponderGuardHitViewContext {
            return contextHitView
        }
        return zmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func zmuxTrackFieldEditor(_ fieldEditor: NSTextView, owningWebView webView: ZmuxWebView?) {
        if let webView {
            objc_setAssociatedObject(
                fieldEditor,
                &zmuxFieldEditorOwningWebViewAssociationKey,
                ZmuxFieldEditorOwningWebViewBox(webView: webView),
    ...
    private static func zmuxTrackedOwningWebView(for fieldEditor: NSTextView) -> ZmuxWebView? {
        guard let box = objc_getAssociatedObject(
            fieldEditor,
            &zmuxFieldEditorOwningWebViewAssociationKey
        ) as? ZmuxFieldEditorOwningWebViewBox else {
            return nil
        }
        guard let webView = box.webView else {
            zmuxTrackFieldEditor(fieldEditor, owningWebView: nil)
            return nil
        }
        return webView
    }

    private static func zmuxIsPointerDownEvent(_ event: NSEvent) -> Bool {
        switch event.type {
    ...
    private static func zmuxPointerEventTargetsWindow(_ event: NSEvent, _ window: NSWindow) -> Bool {
        if event.windowNumber != 0, event.windowNumber != window.windowNumber {
            return false
        }
        if let eventWindow = event.window, eventWindow !== window {
    ...
    private static func zmuxPointerHitWebView(in window: NSWindow, event: NSEvent) -> ZmuxWebView? {
        guard zmuxIsPointerDownEvent(event) else { return nil }
        guard zmuxPointerEventTargetsWindow(event, window) else { return nil }
        ...
        guard let hitView = zmuxHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return zmuxOwningWebView(for: hitView)
    }

    private static func zmuxPointerHitGhosttyView(in window: NSWindow, event: NSEvent) -> GhosttyNSView? {
        guard zmuxIsPointerDownEvent(event) else { return nil }
        guard zmuxPointerEventTargetsWindow(event, window) else { return nil }
        guard let hitView = zmuxHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return zmuxOwningGhosttyView(for: hitView)
    }

    private static func zmuxShouldAllowPointerInitiatedTerminalFocus(
        window: NSWindow,
        request: AppDelegate.TerminalKeyboardFocusRequest,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitGhosttyView = zmuxPointerHitGhosttyView(in: window, event: event) else {
            return false
        }
        return hitGhosttyView === request.ghosttyView
    }

    private static func zmuxShouldAllowPointerInitiatedWebViewFocus(
        window: NSWindow,
        webView: ZmuxWebView,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitWebView = zmuxPointerHitWebView(in: window, event: event) else {
            return false
        }
