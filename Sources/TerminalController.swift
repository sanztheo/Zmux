...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("zmux.socketListenerDidStart")
    static let terminalSurfaceDidBecomeReady = Notification.Name("zmux.terminalSurfaceDidBecomeReady")
    static let terminalSurfaceHostedViewDidMoveToWindow = Notification.Name("zmux.terminalSurfaceHostedViewDidMoveToWindow")
    static let mainWindowContextsDidChange = Notification.Name("zmux.mainWindowContextsDidChange")
    static let browserDownloadEventDidArrive = Notification.Name("zmux.browserDownloadEventDidArrive")
    static let reactGrabDidCopySelection = Notification.Name("zmux.reactGrabDidCopySelection")
}
...
class TerminalController {
    ...
    private var accessMode: SocketControlMode = .zmuxOnly
    ...
    private static let v2BrowserEvalEnvelopeTypeKey = "__zmux_t"
    private static let v2BrowserEvalEnvelopeValueKey = "__zmux_v"
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.zmux.socket-fast-path")
    ...
    private func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        ...
        // In zmuxOnly mode, verify the connecting process is a descendant of zmux.
        // In allowAll mode (env-var only), skip the ancestry check.
        if accessMode == .zmuxOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside zmux can connect\n"
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        // Cloud VMs. Socket → Mac app → VMClient HTTP → zmux web backend. The CLI never touches
    ...
    private func v2WorkspaceCreate(params: [String: Any]) -> V2CallResult {
        ...
        // Decode optional layout param (same JSON schema as zmux.json layout field).
        // Validate before creating the workspace so malformed layouts fail fast.
        var layoutNode: ZmuxLayoutNode?
        if let rawLayout = params["layout"] {
            ...
            do {
                layoutNode = try JSONDecoder().decode(ZmuxLayoutNode.self, from: layoutData)
            } catch {
    ...
    private func v2FeedJump(params: [String: Any]) -> V2CallResult {
        ...
        // MVP: resolve to a zmux surface via `SessionIndexStore` lands in
    ...
    private func v2RunBrowserJavaScript(
        ...
        if let frameSelector = v2BrowserCurrentFrameSelector(surfaceId: surfaceId) {
            ...
            framePrelude = """
            let __zmuxDoc = document;
            try {
              const __zmuxFrame = document.querySelector(\(selectorLiteral));
              if (__zmuxFrame && __zmuxFrame.contentDocument) {
                __zmuxDoc = __zmuxFrame.contentDocument;
              }
            } catch (_) {}
            """
        } else {
            framePrelude = "const __zmuxDoc = document;"
        }
        ...
        const __zmuxMaybeAwait = async (__r) => {
        ...
        const __zmuxEvalInFrame = async function() {
          const document = __zmuxDoc;
          \(executionBlock)
          const __value = await __zmuxMaybeAwait(__r);
          return {
            __zmux_t: (typeof __value === 'undefined') ? 'undefined' : 'value',
            __zmux_v: __value
          };
        };

        return await __zmuxEvalInFrame();
        """
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        let injector = """
        (() => {
          window.__zmuxInitScriptsApplied = window.__zmuxInitScriptsApplied || { scripts: [], styles: [] };
          return true;
        })()
        """
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'zmux-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default.temporaryDirectory
                .appendingPathComponent("zmux-browser-screenshots", isDirectory: true)
    ...
    private func v2BrowserFindWithScript(
        ...
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            \(finderBody)
              ...
              if (!__zmuxFound) return { ok: false, error: 'not_found' };
              const selector = __zmuxCssPath(__zmuxFound);
              if (!selector) return { ok: false, error: 'not_found' };
              return {
                ok: true,
                selector,
                tag: String(__zmuxFound.tagName || '').toLowerCase(),
                text: String(__zmuxFound.textContent || '').trim()
              };
            })()
            """
    ...
    private func v2BrowserDialogRespond(params: [String: Any], accept: Bool) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const q = window.__zmuxDialogQueue || [];
              ...
              if (entry.type === 'confirm') {
                ...
                window.__zmuxDialogDefaults.confirm = \(acceptLiteral);
              }
              if (entry.type === 'prompt') {
                window.__zmuxDialogDefaults = window.__zmuxDialogDefaults || { confirm: false, prompt: null };
                if (\(acceptLiteral)) {
                  window.__zmuxDialogDefaults.prompt = \(textLiteral);
                } else {
                  window.__zmuxDialogDefaults.prompt = null;
                }
              }
    ...
    private func v2BrowserConsoleList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__zmuxConsoleLog) ? window.__zmuxConsoleLog.slice() : [];
              if (\(clearLiteral)) {
                window.__zmuxConsoleLog = [];
              }
    ...
    private func v2BrowserErrorsList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__zmuxErrorLog) ? window.__zmuxErrorLog.slice() : [];
              if (\(clearLiteral)) {
                window.__zmuxErrorLog = [];
              }
              return { ok: true, items };
            })()
            """
    ...
    private func helpText() -> String {
        ...
        Split & surface commands:
          ...
          reload_config                   - Reload Ghostty config, zmux settings, and refresh terminals
    ...
    private func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-screenshots")
