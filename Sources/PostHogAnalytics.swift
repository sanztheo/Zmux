...
final class PostHogAnalytics {
    ...
    // PostHog Cloud US default (matches other zmux properties).
    private let host = "https://us.i.posthog.com"

    private let dailyActiveEvent = "zmux_daily_active"
    private let hourlyActiveEvent = "zmux_hourly_active"
    ...
    private init() {
        workQueue = DispatchQueue(label: "com.zmux.posthog.analytics", qos: .utility)
    ...
    private var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["ZMUX_POSTHOG_ENABLE"] == "1"
#else
...
#if DEBUG
        config.debug = ProcessInfo.processInfo.environment["ZMUX_POSTHOG_DEBUG"] == "1"
#endif
    ...
    nonisolated static func superProperties(infoDictionary: [String: Any]) -> [String: Any] {
        var properties: [String: Any] = ["platform": "zmuxterm"]
    ...
    nonisolated static func shouldFlushAfterCapture(event: String) -> Bool {
        switch event {
        case "zmux_daily_active", "zmux_hourly_active":
            return true
        default:
            return false
        }
