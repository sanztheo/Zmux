...
enum SocketControlMode: String, CaseIterable, Identifiable {
    ...
    static var uiCases: [SocketControlMode] { [.off, .zmuxOnly, .automation, .password, .allowAll] }
    ...
    var displayName: String {
        ...
        case .zmuxOnly:
            return String(localized: "socketControl.zmuxOnly.name", defaultValue: "zmux processes only")
        case .automation:
    ...
    var description: String {
        ...
        case .zmuxOnly:
            return String(localized: "socketControl.zmuxOnly.description", defaultValue: "Only processes started inside zmux terminals can send commands.")
        case .automation:
    ...
    var socketFilePermissions: UInt16 {
        ...
        case .allowAll:
            return 0o666
        case .off, .zmuxOnly, .automation, .password:
            return 0o600
        }
    }
...
enum SocketControlPasswordStore {
    static let directoryName = "zmux"
    static let fileName = "socket-control-password"
    static let didChangeNotification = Notification.Name("zmux.socketControlPasswordDidChange")
    ...
    private static let legacyKeychainService = "com.zmuxterm.app.socket-control"
...
struct SocketControlSettings {
    ...
    static let allowSocketPathOverrideKey = "ZMUX_ALLOW_SOCKET_OVERRIDE"
    static let socketPasswordEnvKey = "ZMUX_SOCKET_PASSWORD"
    static let launchTagEnvKey = "ZMUX_TAG"
    static let baseDebugBundleIdentifier = "com.zmuxterm.app.debug"
    private static let socketDirectoryName = "zmux"
    private static let stableSocketFileName = "zmux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyStableDefaultSocketPath = "/tmp/zmux.sock"
    static let legacyLastSocketPathFile = "/tmp/zmux-last-socket-path"
    ...
    static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any ZMUX_UI_TEST_ env var.
    ...
    static func socketPath(
        ...
        if let taggedDebugPath = taggedDebugSocketPath(
            ...
            if isTruthy(environment[allowSocketPathOverrideKey]),
               let override = environment["ZMUX_SOCKET_PATH"],
               !override.isEmpty {
                return override
            }
        ...
        guard let override = environment["ZMUX_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    static func defaultSocketPath(
        ...
        if bundleIdentifier == "com.zmuxterm.app.nightly" {
            return "/tmp/zmux-nightly.sock"
        }
    ...
    static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            .appendingPathComponent("zmux-\(currentUserID).sock", isDirectory: false)
            .path ?? "/tmp/zmux-\(currentUserID).sock"
    }
    ...
    static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.zmuxterm.app.debug"
            || bundleIdentifier.hasPrefix("com.zmuxterm.app.debug.")
    }

    /// Tagged DEV builds have bundle IDs like `com.zmuxterm.app.debug.<tag>`.
    ...
    static func taggedDebugSocketPath(
        ...
        if bundleId.hasPrefix("\(baseDebugBundleIdentifier).") {
            ...
            if !slug.isEmpty {
                return "/tmp/zmux-debug-\(slug).sock"
            }
        }
        ...
        return "/tmp/zmux-debug-\(tag).sock"
    }

    static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        guard let bundleIdentifier else { return false }
        return bundleIdentifier == "com.zmuxterm.app.staging"
            || bundleIdentifier.hasPrefix("com.zmuxterm.app.staging.")
    }
    ...
    static func envOverrideEnabled(
        ...
        guard let raw = environment["ZMUX_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func envOverrideMode(
        ...
        guard let raw = environment["ZMUX_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func effectiveMode(
        ...
        if let overrideEnabled = envOverrideEnabled(environment: environment) {
            ...
            return userMode == .off ? .zmuxOnly : userMode
        }
