...
extension WKWebView {
    var zmuxIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func zmuxIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard zmuxIsElementFullscreenActiveOrTransitioning else { return false }
...
/// Find-family shortcuts. Cmd+F stays app-owned so zmux can choose browser find
/// or right-sidebar file search from the current focus owner.
final class ZmuxWebView: WKWebView {
    ...
    private static let pasteAsPlainTextFocusMessageHandlerName = "zmuxPasteAsPlainTextFocus"
    ...
    const __zmuxPasteAsPlainTextHelpers = (() => {
      const existing = window.__zmuxPasteAsPlainTextHelpers;
      ...
      window.__zmuxPasteAsPlainTextHelpers = helpers;
      return helpers;
    })();
    """
    ...
    (() => {
      try {
        if (window.__zmuxPasteAsPlainTextFocusTrackerInstalled) return true;
        window.__zmuxPasteAsPlainTextFocusTrackerInstalled = true;
        ...
        const publish = (canPaste) => {
          ...
          window.__zmuxPasteAsPlainTextTargetAvailable = canPaste;
          try {
            handler?.postMessage({ canPaste });
          } catch (_) {}
        };

        window.__zmuxCanPasteAsPlainTextIntoCurrentFocus = () => {
          return __zmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(document.activeElement);
        };

        const publishForElement = (el) => {
          publish(__zmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(el));
        };
        ...
        document.addEventListener("mousedown", (ev) => {
          ...
          if (!__zmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(target)) {
            publish(false);
          }
        }, true);
    ...
    private final class PasteAsPlainTextFocusMessageHandler: NSObject, WKScriptMessageHandler {
        func userContentController(
            ...
            guard let webView = message.webView as? ZmuxWebView else {
                return
            }
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? ZmuxWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: ZmuxWebView) {
    ...
    var contextMenuLinkURLProvider: ((ZmuxWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    private func pageCanAcceptPlainTextPaste() -> Bool {
        ...
        (() => {
            try {
                const fn = window.__zmuxCanPasteAsPlainTextIntoCurrentFocus;
    ...
    override func otherMouseDown(with event: NSEvent) {
        ...
        case 3:
#if DEBUG
            zmuxDebugLog("browser.mouse.otherDown.action web=\(ObjectIdentifier(self)) kind=goBack canGoBack=\(canGoBack ? 1 : 0)")
#endif
            goBack()
            return
        case 4:
#if DEBUG
            zmuxDebugLog("browser.mouse.otherDown.action web=\(ObjectIdentifier(self)) kind=goForward canGoForward=\(canGoForward ? 1 : 0)")
#endif
            goForward()
            return
        default:
...
#if DEBUG
        zmuxDebugLog(Self.redactedContextDownloadDebugMessage(message()))
#endif
    }
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.zmux.sidebar-tab-reorder"),
    ]
