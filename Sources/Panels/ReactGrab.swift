...
private let reactGrabMessageHandlerName = "zmuxReactGrab"
...
class ReactGrabMessageHandler: NSObject, WKScriptMessageHandler {
    ...
    func userContentController(
        ...
        case .stateChange(let isActive):
            zmuxDebugLog("reactGrab.messageHandler type=stateChange isActive=\(isActive)")
        case .copySuccess(let content, _):
            zmuxDebugLog("reactGrab.messageHandler type=copySuccess len=\(content.count)")
        }
        #endif
        Task { @MainActor in
            #if DEBUG
            switch bridgeMessage {
            case .stateChange(let isActive):
                zmuxDebugLog("reactGrab.messageHandler.mainActor type=stateChange isActive=\(isActive)")
            case .copySuccess(let content, _):
                zmuxDebugLog("reactGrab.messageHandler.mainActor type=copySuccess len=\(content.count)")
            }
            #endif
...
extension BrowserPanel {
    ...
    func injectReactGrab() async {
        #if DEBUG
        zmuxDebugLog("reactGrab.inject.start")
        #endif
        guard let scriptSource = await ReactGrabScriptLoader.fetch() else {
            #if DEBUG
            zmuxDebugLog("reactGrab.inject.fetchFailed")
            #endif
            return
        }
        #if DEBUG
        zmuxDebugLog("reactGrab.inject.fetched len=\(scriptSource.count)")
        #endif
        ...
        (function() {
            ...
            var installBridge = function(api) {
                if (!api || window.__ZMUX_REACT_GRAB_BRIDGE_INSTALLED__) return;
                window.__ZMUX_REACT_GRAB_BRIDGE_INSTALLED__ = true;
        ...
        """
        #if DEBUG
        zmuxDebugLog("reactGrab.inject.evalJS len=\(combined.count)")
        #endif
        webView.evaluateJavaScript(combined) { [weak self] _, error in
            #if DEBUG
            zmuxDebugLog("reactGrab.inject.evalJS.done error=\(error?.localizedDescription ?? "none")")
            #endif
            if let error {
        ...
        #if DEBUG
        zmuxDebugLog("reactGrab.inject.end")
        #endif
    }

    func toggleReactGrab() {
        #if DEBUG
        zmuxDebugLog("reactGrab.toggle.start")
        #endif
        let script = "window.__REACT_GRAB__?.toggle()"
        webView.evaluateJavaScript(script, completionHandler: nil)
        #if DEBUG
        zmuxDebugLog("reactGrab.toggle.end")
        #endif
    }
...
#if DEBUG
            zmuxDebugLog("reactGrab.bridgeSessionRefresh.error error=\(error.localizedDescription)")
#endif
            return false
        }
    }
