...
@MainActor
struct ZmuxConfigExecutor {

    @discardableResult
    static func execute(
        command: ZmuxCommandDefinition,
    ...
    @discardableResult
    static func execute(
        action: ZmuxResolvedConfigAction,
    ...
    static func prepareShellInputIfAuthorized(
        ...
        confirm: Bool,
        actionID: String,
        target: ZmuxConfigTerminalCommandTarget,
        configSourcePath: String?,
        globalConfigPath: String,
        displayTitle: String? = nil,
        icon: ZmuxButtonIcon? = nil,
    ...
    private static func authorizeProjectActionIfNeeded(
        descriptor: ZmuxActionTrustDescriptor,
        confirm: Bool,
        configSourcePath: String?,
        globalConfigPath: String,
        ...
        let isTrusted = ZmuxActionTrust.shared.isTrusted(descriptor)
    ...
    private static func makeConfirmDialog(
        ...
        alert.messageText = (trimmedDisplayTitle?.isEmpty == false)
            ? trimmedDisplayTitle!
            : String(
                localized: "dialog.zmuxConfig.confirmCommand.title",
                defaultValue: "Run Project Action?"
            )
        let messageFormat = String(
            localized: "dialog.zmuxConfig.confirmCommand.messageWithCommand",
        ...
        alert.addButton(withTitle: String(
            localized: "dialog.zmuxConfig.confirmCommand.run",
            defaultValue: "Run Once"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.zmuxConfig.confirmCommand.trustAndRun",
            defaultValue: "Trust and Run"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.zmuxConfig.confirmCommand.cancel",
            defaultValue: "Cancel"
        ))
        return alert
    }
    ...
    private static func handleConfirmDialogResponse(
        ...
        case .alertSecondButtonReturn:
            ZmuxActionTrust.shared.trust(descriptor)
            return true
        default:
            return false
        }
    ...
    private static func terminalTrustDescriptor(
        ...
        ZmuxActionTrustDescriptor(
            ...
            projectRoot: configSourcePath.map { canonicalPath(ZmuxButtonIcon.projectRoot(forConfigPath: $0)) },
    ...
    static func isTrustedSurfaceButton(
        ...
        workspaceCommand: ZmuxResolvedCommand?,
        ...
        return ZmuxActionTrust.shared.isTrusted(descriptor)
    }
    ...
    private static func executeWorkspaceCommand(
        command: ZmuxCommandDefinition,
        workspace wsDef: ZmuxWorkspaceDefinition,
        tabManager: TabManager,
        baseCwd: String
    ) {
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                ...
                alert.messageText = String(
                    localized: "dialog.zmuxConfig.confirmRestart.title",
                    defaultValue: "Workspace Already Exists"
                )
                alert.informativeText = String(
                    localized: "dialog.zmuxConfig.confirmRestart.message",
                    defaultValue: "A workspace with this name already exists. Close it and create a new one?"
                )
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(localized: "dialog.zmuxConfig.confirmRestart.recreate", defaultValue: "Recreate"))
                alert.addButton(withTitle: String(localized: "dialog.zmuxConfig.confirmRestart.cancel", defaultValue: "Cancel"))
        ...
        let resolvedCwd = ZmuxConfigStore.resolveCwd(wsDef.cwd, relativeTo: baseCwd)
