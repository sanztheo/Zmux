...
struct ZmuxCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled zmux CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled zmux CLI."
    ...
    init(
        ...
        destinationURL: URL = URL(fileURLWithPath: "/usr/local/bin/zmux"),
        bundledCLIURLProvider: @escaping () -> URL? = {
            ZmuxCLIPathInstaller.defaultBundledCLIURL()
        },
        expectedBundledCLIPath: String = ZmuxCLIPathInstaller.defaultBundledCLIExpectedPath(),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/zmux", isDirectory: false)
    }
