...
enum BrowserFindJavaScript {
    ...
    static func searchScript(query: String) -> String {
        ...
        return """
        (() => {
          const MARK_CLASS = '__zmux-find';
          const CURRENT_CLASS = '__zmux-find-current';
          ...
          window.__zmuxFindMatches = matches;
          window.__zmuxFindIndex = 0;
          ...
          if (!document.getElementById('__zmux-find-style')) {
            const style = document.createElement('style');
            style.id = '__zmux-find-style';
            style.textContent = `
              mark.__zmux-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__zmux-find.__zmux-find-current { background: #f97316; color: #fff; }
            `;
    ...
    static func nextScript() -> String {
        """
        (() => {
          const matches = window.__zmuxFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__zmuxFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__zmuxFindMatches = [];
            window.__zmuxFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__zmux-find-current');
          ...
          matches[idx].classList.add('__zmux-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__zmuxFindIndex = idx;
    ...
    static func clearScript() -> String {
        ...
        (() => {
          ...
          window.__zmuxFindMatches = [];
          window.__zmuxFindIndex = 0;
          const style = document.getElementById('__zmux-find-style');
    ...
    document.querySelectorAll('mark.__zmux-find').forEach(mark => {
