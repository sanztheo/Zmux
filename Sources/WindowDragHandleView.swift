...
func windowDragHandleShouldCaptureHit(
    ...
    guard dragHandleView.bounds.contains(point) else {
        #if DEBUG
        zmuxDebugLog("titlebar.dragHandle.hitTest capture=false reason=outside point=\(windowDragHandleFormatPoint(point))")
        #endif
        return false
    }
    ...
    guard let superview = dragHandleView.superview else {
        #if DEBUG
        zmuxDebugLog("titlebar.dragHandle.hitTest capture=true reason=noSuperview point=\(windowDragHandleFormatPoint(point))")
        #endif
        return true
    }
    ...
    guard !_windowDragHandleIsResolvingSiblingHits else {
        #if DEBUG
        zmuxDebugLog("titlebar.dragHandle.hitTest capture=false reason=reentrant point=\(windowDragHandleFormatPoint(point))")
        #endif
        return false
    }
    ...
    #if DEBUG
    zmuxDebugLog("titlebar.dragHandle.hitTest capture=true point=\(windowDragHandleFormatPoint(point)) siblingCount=\(siblingCount)")
    #endif
    return true
}
    ...
    private final class DraggableView: NSView {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            if event.clickCount >= 2 {
                ...
                #if DEBUG
                zmuxDebugLog("titlebar.dragHandle.mouseDownDoubleClick action=\(String(describing: action))")
                #endif
                if action != nil {
                    return
                }
            ...
            guard !isWindowDragSuppressed(window: window) else {
                #if DEBUG
                zmuxDebugLog("titlebar.dragHandle.mouseDownIgnored reason=suppressed")
                #endif
                return
            }
            ...
            if let window {
                ...
                zmuxDebugLog("titlebar.dragHandle.mouseDownComplete restoredMovable=\(restored) nowMovable=\(window.isMovable)")
                #endif
            } else {
