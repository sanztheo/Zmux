...
struct zmuxApp: App {
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.zmuxOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged zmux DEV; start with ./scripts/reload.sh --tag <name> (or set ZMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
        WindowGroup {
            MainWindowBootstrapView()
                .onAppear {
#if DEBUG
                    if ProcessInfo.processInfo.environment["ZMUX_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: zmuxApp onAppear")
                    }
#endif
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openZmuxSettingsFile", defaultValue: "Open settings.json")) {
                    openZmuxSettingsFileInEditor()
                }
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About zmux")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitZmux", defaultValue: "Quit zmux"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
...
#if DEBUG
                        zmuxDebugLog("find.menu Cmd+F fired")
#endif
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           zmuxWindowShouldOwnCloseShortcut(window) {
    ...
    var body: some View {
        Color.clear
            ...
            .background(WindowAccessor { window in
                window.identifier = NSUserInterfaceItemIdentifier("zmux.bootstrap")
...
private let zmuxAuxiliaryWindowIdentifiers: Set<String> = [
    "zmux.settings",
    "zmux.about",
    "zmux.licenses",
    "zmux.browser-popup",
    "zmux.settingsAboutTitlebarDebug",
    "zmux.debugWindowControls",
    "zmux.browserImportHintDebug",
    "zmux.sidebarDebug",
    "zmux.menubarDebug",
    "zmux.backgroundDebug",
    "zmux.startupAppearanceDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func zmuxWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return zmuxAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("zmux.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("zmux.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, zmux uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("zmux.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.settings")
        window.center()
    ...
    func show(navigationTarget: SettingsNavigationTarget? = nil) {
        guard let window else { return }
#if DEBUG
        zmuxDebugLog("settings.window.show requested isVisible=\(window.isVisible ? 1 : 0) isKey=\(window.isKeyWindow ? 1 : 0)")
#endif
        SettingsAboutTitlebarDebugStore.shared.applyCurrentOptions(to: window, for: .settings)
        if !window.isVisible {
            window.center()
...
#if DEBUG
        zmuxDebugLog("settings.window.show completed isVisible=\(window.isVisible ? 1 : 0) isKey=\(window.isKeyWindow ? 1 : 0)")
#endif
    }
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("zmux.settings.navigate")
...
private final class FileExplorerStyleDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private let githubURL = URL(string: "https://github.com/manaflow-ai/zmux")
    private let docsURL = URL(string: "https://zmux.com/docs")

    private var version: String? { Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String }
    private var build: String? { Bundle.main.infoDictionary?["CFBundleVersion"] as? String }
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["ZMUXCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["ZMUX_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "zmux"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/zmux/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.splitButtonLayoutDebug")
        window.center()
...
private final class TabBarBackdropLabWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("zmux.tabBarBackdropLab")
        window.center()
...
private struct TabBarBackdropLabSample: View {
    ...
    private static func makeController(for variant: TabBarBackdropLabVariant) -> BonsplitController {
        ...
        let titles = [
            ...
            String(localized: "debug.tabBarBackdropLab.tab.terminalTransparency", defaultValue: "zmux terminal transparency"),
...
private struct TabBarBackdropLabTerminalPane: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(String(localized: "debug.tabBarBackdropLab.terminal.prompt", defaultValue: "lawrence in ~/zmux")) \(title)")
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.backgroundDebug")
        window.center()
...
private final class StartupAppearanceDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("zmux.startupAppearanceDebug")
        window.center()
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.zmuxterm.appIconDidChange")
...
enum WelcomeSettings {
    static let shownKey = "zmuxWelcomeShown"
}
...
enum CmdClickMarkdownRouteSettings {
    static let key = "openMarkdownInZmuxViewer"
...
enum PreferredEditorSettings {
    ...
    static func open(_ url: URL) {
        if ZmuxUITestCapture.appendLineIfConfigured(
            envKey: "ZMUX_UI_TEST_CAPTURE_OPEN_PATH",
            line: url.path
        ) {
            return
        }
...
enum ZmuxRuntimeDebugCapture {
    ...
    private static let configuration: Configuration? = {
        let env = ProcessInfo.processInfo.environment
        guard let baseURLString = env["ZMUX_RUNTIME_DEBUG_BASE_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              let baseURL = URL(string: baseURLString),
              let token = env["ZMUX_RUNTIME_DEBUG_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !token.isEmpty,
              let sessionID = env["ZMUX_RUNTIME_DEBUG_SESSION_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
struct SettingsView: View {
    ...
    private let shortcutChordsDocsURL = URL(string: "https://zmux.com/docs/keyboard-shortcuts#shortcut-chords")!
    ...
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowserKey) private var openTerminalLinksInZmuxBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInZmuxBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowserKey)
    private var interceptTerminalOpenCommandInZmuxBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInZmuxBrowserValue()
    ...
    @AppStorage(BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowserKey)
    private var openSidebarPullRequestLinksInZmuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInZmuxBrowser
    @AppStorage(BrowserLinkOpenSettings.openSidebarPortLinksInZmuxBrowserKey)
    private var openSidebarPortLinksInZmuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInZmuxBrowser
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When zmux is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When zmux is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
...
#if DEBUG
        zmuxDebugLog("notification.ui enableTapped state=\(state)")
#endif
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart zmux to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(
                                ...
                                defaultValue: "Open the zmux config, standalone Ghostty config, and merged preview in one utility window."
                            )
                        ) {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("app.openMarkdownInZmuxViewer"),
                            String(localized: "settings.app.openMarkdownInZmuxViewer", defaultValue: "Open Markdown in zmux Viewer"),
                            subtitle: String(localized: "settings.app.openMarkdownInZmuxViewer.subtitle", defaultValue: "Cmd-clicking .md/.markdown/.mkd/.mdx files opens the zmux markdown viewer panel instead of the preferred editor.")
                        ) {
                            Toggle("", isOn: $openMarkdownInZmuxViewer)
                                .labelsHidden()
                                .controlSize(.small)
                                .accessibilityLabel(
                                    String(localized: "settings.app.openMarkdownInZmuxViewer", defaultValue: "Open Markdown in zmux Viewer")
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.menuBarOnly.subtitle", defaultValue: "Hide the Dock icon and Cmd+Tab entry. Use the menu bar item to show zmux.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep zmux in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when zmux highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $ZMUX_NOTIFICATION_TITLE, $ZMUX_NOTIFICATION_SUBTITLE, $ZMUX_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve zmux.")
                        ) {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPullRequestLinksInZmuxBrowser"),
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in zmux Browser"),
                            subtitle: openSidebarPullRequestLinksInZmuxBrowser
                                ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside zmux browser.")
                                : String(localized: "settings.app.openSidebarPRLinks.subtitleOff", defaultValue: "Clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPullRequestLinksInZmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                        ...
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPortLinksInZmuxBrowser"),
                            String(localized: "settings.app.openSidebarPortLinks", defaultValue: "Open Sidebar Port Links in zmux Browser"),
                            subtitle: openSidebarPortLinksInZmuxBrowser
                                ? String(localized: "settings.app.openSidebarPortLinks.subtitleOn", defaultValue: "Port clicks open inside zmux browser.")
                                : String(localized: "settings.app.openSidebarPortLinks.subtitleOff", defaultValue: "Port clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPortLinksInZmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: showTerminalScrollBar
                                ? String(localized: "settings.terminal.scrollBar.subtitleOn", defaultValue: "Shows the right-edge terminal scroll bar in shell scrollback. zmux hides it automatically for alternate-screen style TUI surfaces and you can also disable it per workspace.")
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: ZMUX_SOCKET_ENABLE, ZMUX_SOCKET_MODE, and ZMUX_SOCKET_PATH (set ZMUX_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without zmux integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, zmux wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: cursorHooksEnabled
                                ...
                                : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without zmux integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `zmux cursor install-hooks`. They no-op outside zmux terminals."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: geminiHooksEnabled
                                ...
                                : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without zmux integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `zmux gemini install-hooks`. They no-op outside zmux terminals."))
                    }

                    SettingsCard {
                        SettingsCardRow(configurationReview: .json("automation.portBase"), String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for ZMUX_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $zmuxPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(configurationReview: .json("automation.portRange"), String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $zmuxPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets ZMUX_PORT and ZMUX_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.openTerminalLinksInZmuxBrowser"),
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in zmux Browser"),
                            ...
                            Toggle("", isOn: $openTerminalLinksInZmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.interceptTerminalOpenCommandInZmuxBrowser"),
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInZmuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInZmuxBrowser || interceptTerminalOpenCommandInZmuxBrowser {
                            SettingsCardDivider()
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in zmux. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in zmux.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in zmux without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        ...
                        SettingsCardRow(
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open settings.json")) {
                                    openZmuxSettingsFileInTextEdit()
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                ...
                HStack {
                    ...
                    HStack(spacing: 6) {
                        SettingsHeaderActionButton(
                            ...
                            action: openZmuxSettingsFileInTextEdit
                        )
                    }
                }
    ...
    private func resetAllSettings() {
        ...
        openSidebarPullRequestLinksInZmuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInZmuxBrowser
        openSidebarPortLinksInZmuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInZmuxBrowser
...
private struct AuthSettingsRow: View {
    ...
    private var subtitleText: String? {
        ...
        return String(
            ...
            defaultValue: "Sign in with your zmux account to enable sync across devices."
        )
    }
...
private enum SettingsConfigurationReview: Equatable {
    ...
    func validate(file: StaticString = #fileID, line: UInt = #line) {
        ...
        let unknownPaths = paths.filter { !ZmuxSettingsFileStore.supportedSettingsJSONPaths.contains($0) }
        precondition(
    ...
    private var enableSubtitle: String {
        if isEnabled {
            return String(
                ...
                defaultValue: "Press the shortcut from any app to show or hide all zmux windows."
            )
        }
        return String(
            localized: "settings.globalHotkey.enable.subtitleOff",
            defaultValue: "Turn this on to show or hide all zmux windows from any app."
        )
    }
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("zmux.") else { continue }
