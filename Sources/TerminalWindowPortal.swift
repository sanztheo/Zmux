...
private var zmuxWindowTerminalPortalKey: UInt8 = 0
private var zmuxWindowTerminalPortalCloseObserverKey: UInt8 = 0
...
#if ZMUX_ISSUE_483_PORTAL_RECOVERY
...
#if DEBUG
        zmuxDebugLog("portal.hideEntry hosted=\(portalDebugToken(entry.hostedView)) reason=workspaceUnmount")
#endif
    }
    ...
    private func synchronizeHostedView(withId hostedId: ObjectIdentifier) {
        ...
        guard let anchorView = entry.anchorView, let window else {
            ...
            if !hostedView.isHidden {
                zmuxDebugLog("portal.hidden hosted=\(portalDebugToken(hostedView)) value=1 reason=missingAnchorOrWindow")
            }
#endif
            hostedView.isHidden = true
            if entry.visibleInUI {
...
enum TerminalWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &zmuxWindowTerminalPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &zmuxWindowTerminalPortalCloseObserverKey,
            observer,
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        if let observer = objc_getAssociatedObject(window, &zmuxWindowTerminalPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &zmuxWindowTerminalPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &zmuxWindowTerminalPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowTerminalPortal {
        if let existing = objc_getAssociatedObject(window, &zmuxWindowTerminalPortalKey) as? WindowTerminalPortal {
        ...
        objc_setAssociatedObject(window, &zmuxWindowTerminalPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
