...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/zmux-theme-a"
        let pathB = "/tmp/zmux-theme-b"
    ...
    func testZmuxDefaultThemeConfigContentsSkipsInvalidUTF8Candidate() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-managed-theme-search-\(UUID().uuidString)")
        ...
        let contents = GhosttyConfig.zmuxDefaultThemeConfigContents(
            preferredColorScheme: .light,
    ...
    func testLoadReadsSymlinkedGhosttyConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-ghostty-config-symlink-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testZmuxAppSupportConfigURLsUseReleaseConfigForDebugBundleWithoutCurrentConfig() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            let releaseConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.zmuxterm.app",
                filename: "config",
                contents: "font-size = 13\n"
            )

            XCTAssertEqual(
                GhosttyApp.zmuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.zmuxterm.app.debug",
                    appSupportDirectory: appSupportDirectory
                ),
                [releaseConfigURL]
            )
    ...
    func testZmuxAppSupportConfigURLsPreferCurrentBundleConfigWhenPresent() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            ...
            let currentConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.zmuxterm.app.debug.issue-829",
            ...
            XCTAssertEqual(
                GhosttyApp.zmuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.zmuxterm.app.debug.issue-829",
                    appSupportDirectory: appSupportDirectory
                ),
                [currentConfigURL]
            )
    ...
    func testZmuxAppSupportConfigURLsSkipReleaseFallbackForNonDebugBundle() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.zmuxterm.app",
    ...
    func testZmuxAppSupportConfigURLsIgnoreMissingOrEmptyFiles() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.zmuxterm.app",
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "zmux.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "zmux.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    private func withTemporaryAppSupportDirectory(
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-app-support-\(UUID().uuidString)", isDirectory: true)
...
final class WindowTransparencyDecisionTests: XCTestCase {
    ...
    func testTranslucentOpacityForcesClearWindowBackgroundOutsideSidebarBlendModePath() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertFalse(zmuxShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(zmuxShouldUseClearWindowBackground(for: 0.80))
            XCTAssertFalse(zmuxShouldUseClearWindowBackground(for: 1.0))
        }
    }
    ...
    func testBehindWindowGlassPathKeepsTransparentWindowEnabled() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertTrue(zmuxShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(zmuxShouldUseClearWindowBackground(for: 1.0))
        }
    }
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/manaflow-ai/zmux/releases/tag/v0.62.0",
          "checksumsAssetName": "zmuxd-remote-checksums.txt",
          "checksumsURL": "https://github.com/manaflow-ai/zmux/releases/download/v0.62.0/zmuxd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "zmuxd-remote-linux-amd64",
              "downloadURL": "https://github.com/manaflow-ai/zmux/releases/download/v0.62.0/zmuxd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "zmuxd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/Application Support/zmux/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "zmuxd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: zmux-loopback.localtest.me:3000\r\n" +
                "Origin: http://zmux-loopback.localtest.me:3000\r\n" +
                "Referer: http://zmux-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "zmux-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("zmux-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://zmux-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: zmux-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://zmux-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://zmux-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=zmux-loopback.localtest.me; Path=/"))
    }
}
...
final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    ...
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvZMUXKeys() {
        let replayPath = "/tmp/zmux-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "ZMUX_SURFACE_ID": "managed-surface"
            ],
            protectedKeys: ["PATH", "ZMUX_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "ZMUX_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        XCTAssertEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        XCTAssertEqual(merged["ZMUX_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "ZMUX_SURFACE_ID"],
            additionalEnvironment: [
                "ZMUX_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1"
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "ZMUX_SURFACE_ID": "override-surface"
            ]
        )

        XCTAssertEqual(merged["PATH"], "/usr/bin")
        XCTAssertEqual(merged["ZMUX_SURFACE_ID"], "managed-surface")
...
final class BrowserNewTabNavigationSeedTests: XCTestCase {
    func testPreservesOriginalRequestHeadersMethodBodyAndBypassHost() throws {
        ...
        request.setValue("keep-me", forHTTPHeaderField: "X-Zmux-Test")
        ...
        XCTAssertEqual(
            seed.initialRequest.value(forHTTPHeaderField: "X-Zmux-Test"),
            "keep-me"
        )
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "zmux-loopback.localtest.me")
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/zmux-store-dest.sock",
                terminalStartupCommand: "ssh zmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/zmux-store-source.sock",
                terminalStartupCommand: "ssh zmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/zmux-test.sock",
            terminalStartupCommand: "ssh zmux-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/zmux-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/zmux-a.sock",
            terminalStartupCommand: "ssh zmux-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/zmux-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/zmux-b.sock",
            terminalStartupCommand: "ssh zmux-macmini"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedZMUXRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/zmux-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 zmux-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no zmux-macmini sh -c 'exec .zmux/bin/zmuxd-remote/0.63.1/darwin-arm64/zmuxd-remote serve --stdio'
          103 999 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 zmux-macmini
          104 1 /usr/bin/ssh -tt zmux-macmini
          105 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56082:127.0.0.1:64050 other-host
          106 1 /usr/bin/ssh -T -S none zmux-macmini /bin/sh
        """

        XCTAssertEqual(
            WorkspaceRemoteSessionController.orphanedZMUXRemoteSSHPIDs(
                psOutput: psOutput,
                destination: "zmux-macmini"
            ),
            [101, 102]
    ...
    func testOrphanedZMUXRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          201 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 zmux-macmini
          202 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 zmux-macmini
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no zmux-macmini sh -c 'exec .zmux/bin/zmuxd-remote/0.63.1/darwin-arm64/zmuxd-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("zmuxOnly"), .zmuxOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.zmuxOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["ZMUX_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ZMUX_SOCKET_PATH": "/tmp/zmux-debug-issue-153-tmux-compat.sock",
            ],
            bundleIdentifier: "com.zmuxterm.app",
            isDebugBuild: false,
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.zmuxterm.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/zmux-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ZMUX_SOCKET_PATH": "/tmp/zmux-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.zmuxterm.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/zmux-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ZMUX_SOCKET_PATH": "/tmp/zmux-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.zmuxterm.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/zmux-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ZMUX_SOCKET_PATH": "/tmp/zmux-debug-forced.sock",
                "ZMUX_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.zmuxterm.app",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        ...
        XCTAssertEqual(path, "/tmp/zmux-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/zmux-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.zmuxterm.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/zmux-debug.sock"
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.zmuxterm.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/zmux-staging.sock"
        )
    }
    ...
    func testUntaggedDebugBundleBlockedWithoutLaunchTag() {
        XCTAssertTrue(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.zmuxterm.app.debug",
                isDebugBuild: true
            )
        )
    }
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.zmuxterm.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives ZMUX_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["ZMUX_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.zmuxterm.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["zmux", "-zmuxUITestLaunchManifest", "/tmp/zmux-ui-test-launch.json"]
            ),
            "/tmp/zmux-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["zmux", "-zmuxUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"ZMUX_TAG":"ui-tests-display","ZMUX_SOCKET_PATH":"/tmp/zmux-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["zmux", UITestLaunchManifest.argumentName, "/tmp/zmux-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["ZMUX_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["ZMUX_SOCKET_PATH"], "/tmp/zmux-ui-tests.sock")
    }
}
...
final class PostHogAnalyticsPropertiesTests: XCTestCase {
    ...
    func testSuperPropertiesIncludePlatformVersionAndBuild() {
        ...
        XCTAssertEqual(properties["platform"] as? String, "zmuxterm")
        XCTAssertEqual(properties["app_version"] as? String, "0.31.0")
        XCTAssertEqual(properties["app_build"] as? String, "230")
    }
    ...
    func testPropertiesOmitVersionFieldsWhenUnavailable() {
        ...
        XCTAssertEqual(superProperties["platform"] as? String, "zmuxterm")
    ...
    func testFlushPolicyIncludesDailyAndHourlyActiveEvents() {
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "zmux_daily_active"))
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "zmux_hourly_active"))
        XCTAssertFalse(PostHogAnalytics.shouldFlushAfterCapture(event: "zmux_other_event"))
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "zmux-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsIncludesNativeGhosttyAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.zmux-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.zmux-dev",
    ...
    func testShouldApplyManagedDefaultAppearanceScansNativeGhosttyAppSupport() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-appearance-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testShouldApplyManagedDefaultAppearanceFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-theme-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearancePreservesQuotedQuestionMarkConfigFile() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-theme-quoted-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearanceProcessesIncludeQueuedAfterReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-test-theme-reset-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenZmuxRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(zmuxLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutZmuxHandoffFlag() throws {
        let output = try runInteractiveZsh(zmuxLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: true,
            zmuxLoadShellIntegration: true,
            command: """
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: false,
            zmuxLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            zmuxLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}" >> "$ZMUX_TEST_OUTPUT"
            """,
            userZshRCContents: """
            export ZMUX_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export ZMUX_STARTUP_THEME_BRANCH=extended
            else
              export ZMUX_STARTUP_THEME_BRANCH=basic
            fi

            zmux_test_ready() {
              [[ -e "$ZMUX_TEST_READY" ]] && return 0
              print -r -- "PRE=$ZMUX_STARTUP_THEME_TERM|$ZMUX_STARTUP_THEME_BRANCH|$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}" > "$ZMUX_TEST_OUTPUT"
              : > "$ZMUX_TEST_READY"
              precmd_functions=(${precmd_functions:#zmux_test_ready})
            }
            precmd_functions+=(zmux_test_ready)
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            zmuxLoadShellIntegration: true,
            command: """
            print -r -- "$ZMUX_STARTUP_TERM|$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
            export ZMUX_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenIntegrationDisabled() throws {
        let output = try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: false,
            zmuxLoadShellIntegration: false,
            command: """
            print -r -- "$ZMUX_STARTUP_TERM|$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            zmuxLoadShellIntegration: true,
            command: """
            print -r -- "$ZMUX_STARTUP_TERM|$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}|${ZMUX_SHELL_INTEGRATION:-unset}"
            """,
            userZshEnvContents: """
            export ZMUX_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export ZMUX_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            zmuxLoadShellIntegration: true,
            command: """
            _zmux_preexec 'echo $TERM'
            print -r -- "$TERM|${ZMUX_ZSH_RESTORE_TERM-unset}"
            """,
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "ZMUX_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedZmuxEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: false,
            zmuxLoadShellIntegration: true,
            command: "_zmux_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "ZMUX_SOCKET_PATH": "/tmp/zmux-current.sock",
                "ZMUX_TAG": "feat-tmux-notification-attention-state",
                "ZMUX_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "ZMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "ZMUX_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "ZMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g ZMUX_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g ZMUX_SOCKET_PATH /tmp/zmux-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g ZMUX_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g ZMUX_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g ZMUX_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'ZMUX_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'ZMUX_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu ZMUX_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu ZMUX_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedZmuxEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: false,
            zmuxLoadShellIntegration: true,
            command: "_zmux_precmd; print -r -- \"$ZMUX_TAG|$ZMUX_SOCKET_PATH|$ZMUX_WORKSPACE_ID|$ZMUX_SURFACE_ID|$ZMUX_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "ZMUX_SOCKET_PATH": "/tmp/zmux-stale.sock",
                "ZMUX_TAG": "feat-tmux-integration-experiments",
                "ZMUX_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "ZMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "ZMUX_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "ZMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/zmux-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            zmuxLoadShellIntegration: true,
            command: """
            _ZMUX_TTY_NAME=ttys999
            print -r -- "$(_zmux_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        try writeExecutableScript(
            at: binDir.appendingPathComponent("zmux", isDirectory: false),
            contents: """
            #!/bin/sh
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
    ...
    func testShellIntegrationRelayPortsKickOmitsSurfaceIDUntilAvailableInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-relay-kick-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _zmux_ports_kick_via_relay refresh
            repeat 20; do
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-relay-precmd-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-bash-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayPreexecWorksBeforeSurfaceIDExistsInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-bash-relay-preexec-no-surface-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            command: """
            : > "\(logPath.path)"
            _ZMUX_TTY_NAME=ttys889
            _ZMUX_TTY_REPORTED=0
            _zmux_preexec_command "python3 -m http.server 8899"
            for _zmux_i in $(seq 1 20); do
              [ -s "\(logPath.path)" ] && break
              sleep 0.05
            done
            cat "\(logPath.path)"
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInBashWithoutPromptNoise() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-bash-relay-prompt-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        XCTAssertFalse(result.stderr.contains("_zmux_report_tmux_state"), result.stderr)
        XCTAssertTrue(
    ...
    private func runInteractiveZsh(zmuxLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            zmuxLoadGhosttyIntegration: zmuxLoadGhosttyIntegration,
            zmuxLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        zmuxLoadGhosttyIntegration: Bool,
        zmuxLoadShellIntegration: Bool,
        command: String,
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let zmuxZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": zmuxZdotdir.path,
            "ZMUX_ZSH_ZDOTDIR": userZdotdir.path,
            "ZMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if zmuxLoadGhosttyIntegration {
            process.environment?["ZMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if zmuxLoadShellIntegration {
            process.environment?["ZMUX_SHELL_INTEGRATION"] = "1"
            process.environment?["ZMUX_SHELL_INTEGRATION_DIR"] = zmuxZdotdir.path
            process.environment?["ZMUX_SOCKET_PATH"] = root.appendingPathComponent("zmux-test.sock").path
            process.environment?["ZMUX_TAB_ID"] = "tab-test"
            process.environment?["ZMUX_PANEL_ID"] = "panel-test"
        }
    ...
    private func runPromptInteractiveZsh(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-zsh-prompt-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        process.environment = [
            ...
            "ZMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
            "ZMUX_TEST_READY": readyPath.path,
            "ZMUX_TEST_OUTPUT": outputPath.path,
        ]
        if zmuxLoadGhosttyIntegration {
            process.environment?["ZMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
    ...
    private func runInteractiveBash(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zmux-bash-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot.appendingPathComponent("Resources/shell-integration/zmux-bash-integration.bash")
        let rcfilePath = root.appendingPathComponent(".bashrc")
        let rcfileContents: String = {
            guard zmuxLoadShellIntegration else { return ":\n" }
            return """
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("zmux-browser-detect-\(UUID().uuidString)")
    }
