...
final class GhosttyPasteboardHelperTests: XCTestCase {
    ...
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<p>Hello <strong>world</strong></p>", forType: .html)

        XCTAssertEqual(zmuxPasteboardStringContentsForTesting(pasteboard), "Hello world")
        XCTAssertNil(zmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testAlternatePlainTextUTIExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-plain-text-uti-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(
            zmuxPasteboardStringContentsForTesting(pasteboard),
            "hello from public.plain-text"
        )
    }

    /// Regression test for https://github.com/manaflow-ai/zmux/issues/2818 —
    ...
    func testPrefersUTF8PlainTextOverLegacyMacRomanType() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-utf8-priority-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testFallsBackWhenOnlyNonPreferredPlainTextTypePresent() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-only-legacy-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testEmptyPlainTextFallsBackToRichTextPayload() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-empty-plain-rich-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    /// Regression test for https://github.com/manaflow-ai/zmux/issues/2940.
    /// Some apps place the same large clipboard payload onto `.string`, `.html`,
    /// and `.rtf`. zmux should hand the plain text to the terminal quickly
    ...
    func testLargePlainTextPasteStaysFastWhenRichTextTypesAreAlsoPresent() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("zmux-test-large-fast-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testXHTMLTypeFallsBackToRenderedHTMLText() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-xhtml-html-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testPublicURLPastePreservesOriginalURLText() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-public-url-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(zmuxPasteboardStringContentsForTesting(pasteboard), rawURL)
    ...
    func testImageClipboardWithPlainTextFallbackStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-image-plain-text-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
        ...
        XCTAssertNil(zmuxPasteboardStringContentsForTesting(pasteboard))

        let imagePath = try XCTUnwrap(zmuxPasteboardImagePathForTesting(pasteboard))
    ...
    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageHTMLClipboardWithGenericPlainTextStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-image-html-generic-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<meta charset='utf-8'><img src=\"https://example.com/keyboard.png\">", forType: .html)
        pasteboard.setString(
            "https://example.com/keyboard.png",
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(zmuxPasteboardStringContentsForTesting(pasteboard), "Hello")
        XCTAssertNil(zmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardWithPlainTextFallbackStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-rtfd-attachment-string-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageOnlyPasteboardProducesTempFileURL() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-drop-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .red), forType: .png)

        let fileURL = try XCTUnwrap(zmuxPasteboardImageFileURLForTesting(pasteboard))
    ...
    func testRemoteImageDropPlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageDropPlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePastePlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteFileURLPastePlanUploadsReadableFile() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-file-url-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteDirectoryPastePlanFallsBackToEscapedPathInsertion() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-directory-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanSkipsTargetResolutionForPlainText() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-lazy-text-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testPastePlanFallsBackToAlternatePlainTextWhenImageTypeIsUnusable() {
        let pasteboard = NSPasteboard(name: .init("zmux-test-raycast-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
            "hello from Raycast",
            forType: NSPasteboard.PasteboardType(UTType.plainText.identifier)
    ...
    func testLazyPastePlanResolvesTargetForFileURLPaste() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("zmux-test-lazy-file-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([fileURL as NSURL]))
    ...
    func testLocalImagePastePlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-local-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePasteExecutionUploadsAndCompletesWithRemotePath() throws {
        ...
        TerminalImageTransferPlanner.executeForTesting(
            ...
            uploadWorkspaceRemote: { _, _, finish in finish(.success(["/tmp/zmux-drop-123.png"])) },
        ...
        XCTAssertEqual(completedText, "/tmp/zmux-drop-123.png")
    }
    ...
    func testCancelledRemoteImagePasteExecutionSuppressesCompletionHandlers() throws {
        ...
        completion?(.success(["/tmp/zmux-drop-cancelled.png"]))
    ...
    func testCancelledOperationSuppressesLateLocalInsert() {
        ...
        let returnedOperation = TerminalImageTransferPlanner.executeForTesting(
            plan: .insertText("/tmp/zmux-drop-local.png"),
            operation: operation,
    ...
    func testRemoteImageDropHandlerUploadsAndSendsRemotePath() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-handler-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        let handled = GhosttyNSView.handleDropForTesting(
            ...
            uploadRemote: { urls, finish in
                uploadedURLs = urls
                finish(.success(["/tmp/zmux-drop-abc123.png"]))
            },
        ...
        XCTAssertEqual(sentText, ["/tmp/zmux-drop-abc123.png"])
        XCTAssertEqual(failureCount, 0)
    }

    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterSuccess() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-handler-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .orange), forType: .png)
    ...
    func testRemoteDropUploadFailureTriggersFailureHandler() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-handler-fail-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterFailure() throws {
        let pasteboard = NSPasteboard(name: .init("zmux-test-remote-handler-failure-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .cyan), forType: .png)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(zmuxOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(zmuxOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(zmuxOwningGhosttyView(for: view))
    }
    ...
    func testDoesNotReadTextViewDelegateForGhosttyResponderResolution() {
        ...
        XCTAssertNil(zmuxOwningGhosttyView(for: textView))
        XCTAssertEqual(
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/zmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/zmux.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/zmux-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/zmux-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/zmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class TerminalCmdClickPathPunctuationTrimmingTests: XCTestCase {
    func testTrimsTrailingPeriodAfterMarkdownFile() {
        XCTAssertEqual(
            zmuxTrimTerminalPathTrailingPunctuationForTesting(
    ...
    func testResolveQuicklookFallsBackToStrippedPathWhenLiteralPathIsMissing() {
        let strippedPath = "/tmp/zmux-cmdclick-path.md"

        XCTAssertEqual(
            zmuxResolveQuicklookPathForTesting(
                "\(strippedPath).",
                cwd: "/tmp",
    ...
    func testResolveQuicklookPrefersLiteralPathThatReallyEndsWithDot() {
        let literalPath = "/tmp/zmux-cmdclick-literal-dot.md."
        let strippedPath = "/tmp/zmux-cmdclick-literal-dot.md"
    ...
    func testResolveQuicklookPrefersLiteralPathThatReallyEndsWithParen() {
        let literalPath = "/tmp/zmux-cmdclick-literal-paren)"
        let strippedPath = "/tmp/zmux-cmdclick-literal-paren"
...
final class GhosttyModifierFlagsChangedActionTests: XCTestCase {
    func testLeftShiftPressReturnsPress() {
        XCTAssertEqual(
            zmuxGhosttyModifierActionForFlagsChanged(
                keyCode: 0x38,
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testNonStableSocketBindFailureDoesNotFallback() {
        XCTAssertNil(
            TerminalController.fallbackSocketPathAfterBindFailure(
                requestedPath: "/tmp/zmux-debug.sock",
                stage: "bind",
                errnoCode: EACCES,
                currentUserID: 501
            )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/zmux-socket-health-\(UUID().uuidString).sock"
    }
