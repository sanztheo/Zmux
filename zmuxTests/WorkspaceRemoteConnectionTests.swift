...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("zmux-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".zmux/relay/64011.shell")
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("zmux-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".zmux/relay")
        let socketAddrURL = home.appendingPathComponent(".zmux/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("zmux-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".zmux/relay")
        let socketAddrURL = home.appendingPathComponent(".zmux/socket_addr")
    ...
    func testDaemonSocketForwardArgumentsTargetBakedVMSocket() {
        ...
        let arguments = WorkspaceRemoteSSHBatchCommandBuilder.daemonSocketForwardArguments(
            ...
            localPort: 64123,
            remoteSocketPath: "/run/zmuxd-remote.sock"
        )
        ...
        XCTAssertTrue(arguments.contains("127.0.0.1:64123:/run/zmuxd-remote.sock"))
        XCTAssertEqual(arguments.last, "zmux-macmini")
    }
    ...
    func testProxyBrokerTransportKeySeparatesVMBakedSSHFromStandardSSH() {
        let standard = WorkspaceRemoteConfiguration(
            ...
            port: 2222,
            identityFile: "/Users/test/.ssh/id_ed25519",
            sshOptions: ["ControlPath /tmp/zmux-ssh-%C"],
            localProxyPort: nil,
            relayPort: 64099,
            relayID: "relay-a",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/zmux.sock",
        ...
        let vmWebSocket = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "zmux vm-pty-attach --id abcd1234",
    ...
    func testWebSocketVMWithoutDaemonEndpointSkipsProxyStartup() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "zmux vm-pty-attach --id test-no-daemon",
            skipDaemonBootstrap: true
        )
    ...
    func testWebSocketVMWithDaemonEndpointStartsProxyCapableConnection() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "zmux vm-pty-attach --id test-with-daemon",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/zmux-debug-test.sock",
            terminalStartupCommand: "ssh zmux-macmini"
        )
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to zmux-macmini",
            target: "zmux-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "zmux-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/zmux-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "zmux-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/zmux-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/zmux-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/zmux-ssh-%C"))
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = WorkspaceRemoteSSHBatchCommandBuilder.daemonTransportArguments(
            configuration: configuration,
            remotePath: "/remote/zmuxd-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/zmux-ssh-%C" || $0 == "ControlPath=/tmp/zmux-ssh-%C" }))
        XCTAssertTrue(arguments.contains("zmux-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/zmuxd-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/zmux-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/zmux-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to zmux-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "zmux-macmini")

        XCTAssertEqual(workspace.remoteConnectionState, .connected)
        XCTAssertEqual(workspace.remoteConnectionDetail, proxyError)
        XCTAssertEqual(
            workspace.statusEntries["remote.error"]?.value,
            "Remote proxy unavailable (zmux-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to zmux-macmini", target: "zmux-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to zmux-macmini via shared local proxy 127.0.0.1:9999",
            target: "zmux-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "zmux",
                  item.path.contains(".app/Contents/Resources/bin/zmux") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled zmux CLI not found in \(appBundleURL.path)")
    }
    ...
    func testOpenCodeInstallHooksRegistersSessionPlugin() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-opencode-hooks-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["ZMUX_CLI_SENTRY_DISABLED"] = "1"
        let result = runProcess(
        ...
        let pluginURL = configDir
            ...
            .appendingPathComponent("zmux-session.js", isDirectory: false)
        let pluginSource = try String(contentsOf: pluginURL, encoding: .utf8)
        XCTAssertTrue(pluginSource.contains("zmux-opencode-session-plugin-marker"))
        ...
        XCTAssertEqual(plugins, ["other-plugin", "zmux-session"])
    }
    ...
    func testAgentHookLaunchEnvironmentDoesNotPersistPathOrShell() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-agent-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["ZMUX_SOCKET_PATH"] = socketPath
        environment["ZMUX_WORKSPACE_ID"] = workspaceId
        environment["ZMUX_SURFACE_ID"] = surfaceId
        environment["ZMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["ZMUX_AGENT_LAUNCH_KIND"] = "codex"
        environment["ZMUX_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/codex"
        environment["ZMUX_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated([
            "/usr/local/bin/codex",
            "--model",
            "gpt-5.4",
            "old prompt"
        ])
        environment["ZMUX_AGENT_LAUNCH_CWD"] = "/tmp/repo"
    ...
    func testNotifyFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["ZMUX_SOCKET_PATH"] = socketPath
        environment["ZMUX_WORKSPACE_ID"] = staleWorkspace
        environment["ZMUX_SURFACE_ID"] = staleSurface
        environment["ZMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["ZMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "--ssh-option", "ControlPath /tmp/zmux-ssh-%C",
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=zmux-macmini state=disconnected\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "zmux-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/zmux-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `zmux ssh` should land the user in the new SSH workspace immediately.
    ...
    func testSSHCommandDoesNotDeferReconnectWhenWhitespaceControlMasterDisablesMultiplexing() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=zmux-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let fileManager = FileManager.default
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("zmux-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["ZMUX_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        startupEnvironment["ZMUX_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["ZMUX_SOCKET_PATH"] = socketPath
        startupEnvironment["ZMUX_WORKSPACE_ID"] = workspaceID
        startupEnvironment["ZMUX_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["ZMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "zmux-macmini"))
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["ZMUX_WORKSPACE_ID"] = staleWorkspace
        environment["ZMUX_SURFACE_ID"] = staleSurface
        environment["ZMUX_CLI_TTY_NAME"] = callerTTY
        environment["ZMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["ZMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
