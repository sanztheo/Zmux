...
final class TabManagerPullRequestProbeTests: XCTestCase {
    func testGitHubRepositorySlugsPrioritizeUpstreamThenOriginAndDeduplicate() {
        let output = """
        origin https://github.com/austinwang/zmux.git (fetch)
        origin https://github.com/austinwang/zmux.git (push)
        upstream git@github.com:manaflow-ai/zmux.git (fetch)
        upstream git@github.com:manaflow-ai/zmux.git (push)
        backup ssh://git@github.com/manaflow-ai/zmux.git (fetch)
        mirror https://gitlab.com/manaflow-ai/zmux.git (fetch)
        """

        XCTAssertEqual(
            TabManager.githubRepositorySlugs(fromGitRemoteVOutput: output),
            ["manaflow-ai/zmux", "austinwang/zmux"]
        )
    }
    ...
    func testPreferredPullRequestPrefersOpenOverMergedAndClosed() {
        let candidates = [
            TabManager.GitHubPullRequestProbeItem(
                number: 1889,
                state: "MERGED",
                url: "https://github.com/manaflow-ai/zmux/pull/1889",
                updatedAt: "2026-03-20T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1891,
                state: "OPEN",
                url: "https://github.com/manaflow-ai/zmux/pull/1891",
                updatedAt: "2026-03-19T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1800,
                state: "CLOSED",
                url: "https://github.com/manaflow-ai/zmux/pull/1800",
    ...
    func testPreferredPullRequestPrefersMostRecentlyUpdatedWithinSameStatus() {
        let olderOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1880,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/zmux/pull/1880",
            updatedAt: "2026-03-18T18:00:00Z"
        )
        let newerOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1890,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/zmux/pull/1890",
            updatedAt: "2026-03-20T18:00:00Z"
        )
    ...
    func testPreferredPullRequestIgnoresMalformedCandidates() {
        let valid = TabManager.GitHubPullRequestProbeItem(
            number: 1888,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/zmux/pull/1888",
            updatedAt: "2026-03-20T18:00:00Z"
        )
        ...
        XCTAssertEqual(
            TabManager.preferredPullRequest(from: [
                TabManager.GitHubPullRequestProbeItem(
                    number: 9999,
                    state: "WHATEVER",
                    url: "https://github.com/manaflow-ai/zmux/pull/9999",
                    updatedAt: "2026-03-21T18:00:00Z"
                ),
                TabManager.GitHubPullRequestProbeItem(
                    number: 10000,
    ...
    func testPullRequestMapDropsStaleMergedHeadPullRequestForLongLivedBaseBranch() throws {
        ...
        let pullRequests = [
            TabManager.GitHubPullRequestProbeItem(
                number: 2400,
                state: "MERGED",
                url: "https://github.com/manaflow-ai/zmux/pull/2400",
            ...
            TabManager.GitHubPullRequestProbeItem(
                number: 2501,
                state: "MERGED",
                url: "https://github.com/manaflow-ai/zmux/pull/2501",
            ...
            TabManager.GitHubPullRequestProbeItem(
                number: 2502,
                state: "OPEN",
                url: "https://github.com/manaflow-ai/zmux/pull/2502",
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/manaflow-ai/zmux/pull/371"))
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesExcludeDirectoriesWithoutResolvedGitMetadata() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "zmux-git-nonrepo-candidate-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testInheritedBackgroundWorkspaceFetchesGitBranchWithoutSelection() throws {
        ...
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "zmux-git-inherited-background-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        try runGit(["config", "user.name", "zmux tests"], in: repoURL)
        try runGit(["config", "user.email", "zmux@example.invalid"], in: repoURL)
        try "seed\n".write(
    ...
    func testPeriodicWorkspaceGitMetadataRefreshUpdatesMainWorkspaceAfterCheckoutToFeatureBranch() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("zmux-git-main-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    func testPeriodicWorkspaceGitMetadataRefreshRestoresClearedBranchForStaleTerminal() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "zmux-git-stale-branch-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    func testResolvedCommandPathFallsBackOutsideAppPATH() throws {
        ...
        let tempDir = fileManager.temporaryDirectory.appendingPathComponent(
            "zmux-command-path-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: tempDir) }

        let executableName = "zmux-gh-test-\(UUID().uuidString)"
    ...
    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("zmux-git-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/manaflow-ai/zmux/pull/1052")),
            status: .open,
