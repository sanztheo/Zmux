...
private extension GhosttyNSView {
    @objc func zmuxUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        zmuxUnitTest_interpretKeyEvents(eventArray)
    }

    @objc func zmuxUnitTest_paste(_ sender: Any?) {
        ghosttyPasteActionHook?(self, sender)
        zmuxUnitTest_paste(sender)
    }

    @objc func zmuxUnitTest_pasteAsPlainText(_ sender: Any?) {
        ghosttyPasteAsPlainTextActionHook?(self, sender)
        zmuxUnitTest_pasteAsPlainText(sender)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.zmuxUnitTest_interpretKeyEvents(_:))
...
private func installGhosttyPasteActionSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.zmuxUnitTest_paste(_:))
    ...
    let plainTextSwizzledSelector = #selector(GhosttyNSView.zmuxUnitTest_pasteAsPlainText(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in zmux)
