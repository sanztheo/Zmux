...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    func testAddWorkspaceInPreferredMainWindowPrunesOrphanedContextWithoutLiveWindow() {
        ...
        autoreleasepool {
            ...
            orphanWindow?.identifier = NSUserInterfaceItemIdentifier("zmux.main.\(orphanWindowId.uuidString)")
    ...
    func testCmdWClosesAuxiliaryWindowInsteadOfMainTerminalPanel() throws {
        ...
        auxiliaryWindow.identifier = NSUserInterfaceItemIdentifier("zmux.about")
        ...
        XCTAssertNotEqual(NSApp.keyWindow?.identifier?.rawValue, "zmux.about", "Closed auxiliary window should not remain key")
    }
    ...
    func testAttachUpdateAccessoryRemovesTitlebarAccessoryWhenMinimalModeEnabled() {
        ...
        let hasTitlebarAccessory: () -> Bool = {
            window.titlebarAccessoryViewControllers.contains {
                $0.view.identifier?.rawValue == "zmux.titlebarControls"
            }
        }
    ...
    func testKeyboardShortcutSettingsSetShortcutPostsSpecificChangeNotification() {
        let notificationName = Notification.Name("zmux.keyboardShortcutSettingsDidChange")
    ...
    func testBrowserFirstFindShortcutRoutingExcludesGlobalFindCommand() {
        ...
        XCTAssertFalse(
            shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(event),
            "Cmd+F belongs to zmux focus-aware find routing, not browser-first routing"
        )
    }
    ...
    private func window(withId windowId: UUID) -> NSWindow? {
        let identifier = "zmux.main.\(windowId.uuidString)"
    ...
    private func mainWindowIds() -> Set<UUID> {
        Set(NSApp.windows.compactMap { window in
            guard let raw = window.identifier?.rawValue,
                  raw.hasPrefix("zmux.main.") else {
                return nil
            }
            return UUID(uuidString: String(raw.dropFirst("zmux.main.".count)))
        })
    }
