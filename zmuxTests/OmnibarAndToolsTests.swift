...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "zmux-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/zmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/zmux-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/zmux-services/../zmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/zmux-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/zmux-services/project",
                "/tmp/zmux-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/zmux-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/zmux-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/zmux-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/zmux-services/b/file.txt", isDirectory: false),
        ]
    ...
    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../zmux.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/zmux.app/Contents/MacOS/zmux", isDirectory: false),
            URL(fileURLWithPath: "/Applications/zmux.app/Contents/Resources/bin/zmux", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/zmux", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/zmux/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/zmux",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/zmux.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/zmux.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/zmux.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/zmux.app backup/project",
                "/Applications/zmux.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("zmux.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/zmux", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("zmux.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("zmux.app/Contents/MacOS/zmux", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/zmux"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/zmux")
    }
