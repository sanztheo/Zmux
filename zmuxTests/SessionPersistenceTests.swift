...
final class SessionPersistenceTests: XCTestCase {
    ...
    func testWorkspaceSessionSnapshotRestoresMarkdownPanel() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-session-markdown-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSaveAndLoadRoundTripWithCustomSnapshotPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-session-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLoadReopenSessionSnapshotRequiresPreviousSnapshotFile() throws {
        ...
        let bundleIdentifier = "dev.zmux.tests.\(UUID().uuidString)"
    ...
    func testRestorePolicySkipsWhenLaunchHasExplicitArguments() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/zmux.app/Contents/MacOS/zmux", "--window", "window:1"],
            environment: [:]
        )
    ...
    func testRestorePolicyAllowsFinderStyleLaunchArgumentsOnly() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/zmux.app/Contents/MacOS/zmux", "-psn_0_12345"],
            environment: [:]
        )
    ...
    func testRestorePolicySkipsWhenRunningUnderXCTest() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/zmux.app/Contents/MacOS/zmux"],
    ...
    func testScrollbackReplayEnvironmentWritesReplayFile() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-scrollback-replay-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/zmux-screen.txt"),
            "/tmp/zmux-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/zmux-screen.txt "),
            "/tmp/zmux-screen.txt"
        )
    }
    ...
    func testShouldRemoveExportedScreenDirectoryOnlyWithinTemporaryRoot() {
        let tempRoot = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("zmux-export-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
...
final class SocketListenerAcceptPolicyTests: XCTestCase {
    ...
    func testClaudeResumeCommandPreservesLaunchFlagsAndDropsInjectedHookSettings() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            sessionId: "claude-session-123",
            workingDirectory: "/tmp/zmux project",
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                arguments: [
                    ...
                    "auto",
                    "--settings",
                    #"{"hooks":{"SessionStart":[{"hooks":[{"command":"zmux claude-hook session-start"}]}]}}"#,
                    "--session-id",
                    "old-session",
        ...
        XCTAssertEqual(
            ...
            "cd '/tmp/zmux project' && 'env' 'CLAUDE_CONFIG_DIR=/tmp/claude config' '/opt/Claude Code/bin/claude' '--resume' 'claude-session-123' '--model' 'sonnet' '--permission-mode' 'auto'"
        )
    }
    ...
    func testRestorableAgentStartupInputUsesLauncherScriptWhenCommandExceedsTerminalInputBudget() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-agent-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClaudeTeamsResumeCommandPreservesRemoteControlLauncher() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                executablePath: "/Applications/zmux.app/Contents/Resources/bin/zmux",
                arguments: [
                    "/Applications/zmux.app/Contents/Resources/bin/zmux",
                    "claude-teams",
                    "--teammate-mode",
                    "auto",
                    "--model",
                ...
                workingDirectory: "/tmp/team repo",
                environment: [
                    "ZMUX_CUSTOM_CLAUDE_PATH": "/opt/Claude Code/bin/claude",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd '/tmp/team repo' && 'env' 'ZMUX_CUSTOM_CLAUDE_PATH=/opt/Claude Code/bin/claude' '/Applications/zmux.app/Contents/Resources/bin/zmux' 'claude-teams' '--resume' 'claude-team-session' '--teammate-mode' 'auto' '--model' 'sonnet' '--remote-control-session-name-prefix' 'zmux-team' '--permission-mode' 'auto'"
        )
    }
    ...
    func testClaudeResumeCommandStripsStaleZmuxNodeOptionsRestoreModule() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require=/tmp/zmux-claude-node-options/restore-node-options.cjs --max-old-space-size=4096 --trace-warnings"
                ],
                capturedAt: nil,
                source: nil
            )
    ...
    func testClaudeResumeCommandDropsEmptyStaleZmuxNodeOptionsEnvironment() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require /tmp/zmux-claude-node-options/restore-node-options.cjs --max-old-space-size 4096"
                ],
                capturedAt: nil,
                source: nil
            )
    ...
    func testHookStoreDirectoryCanBeOverriddenForTests() {
        let url = RestorableAgentKind.codex.hookStoreFileURL(
            ...
            environment: ["ZMUX_AGENT_HOOK_STATE_DIR": "/tmp/zmux hook state"]
        )

        XCTAssertEqual(url.path, "/tmp/zmux hook state/codex-hook-sessions.json")
    }
    ...
    func testOpenCodeWrapperResumeCommandAndUnsupportedOhMyLaunchers() {
        ...
        let omo = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omo",
                executablePath: "/usr/local/bin/zmux",
                arguments: [
                    "/usr/local/bin/zmux",
                    "omo",
                    "--model",
        ...
        let omx = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omx",
                executablePath: "/usr/local/bin/zmux",
                arguments: ["/usr/local/bin/zmux", "omx", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        let omc = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omc",
                executablePath: "/usr/local/bin/zmux",
                arguments: ["/usr/local/bin/zmux", "omc", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        XCTAssertEqual(
            omo.resumeCommand,
            "cd '/tmp/opencode repo' && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/zmux' 'omo' '--session' 'opencode-session-123' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
    ...
    func testRestorableAgentIndexLoadsLaunchCommandFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
        let storeDir = home.appendingPathComponent(".zmuxterm", isDirectory: true)
