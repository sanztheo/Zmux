...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testNotificationCustomStagingPreservesSourceFileWithZmuxPrefix() {
        let suiteName = "NotificationDockBadgeTests.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated UserDefaults suite")
            return
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "zmux-custom-notification-sound.source-\(UUID().uuidString).wav",
            isDirectory: false
        )
        defer {
        ...
        XCTAssertTrue(stagedName.hasPrefix("zmux-custom-notification-sound-"))
    ...
    func testNotificationCustomUnsupportedExtensionsStageAsCaf() {
        ...
        XCTAssertTrue(stagedA.hasPrefix("zmux-custom-notification-sound-"))
    ...
    func testNotificationCustomPreparationKeepsActiveSourceMetadataSidecar() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "zmux-custom-notification-sound.metadata-\(UUID().uuidString).wav",
            isDirectory: false
        )
        do {
            try Data("test".utf8).write(to: sourceURL, options: .atomic)
    ...
    func testNotificationCustomSoundReturnsNilWhenPreparationFails() {
        ...
        let invalidSourceURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-invalid-sound-\(UUID().uuidString).mp3", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: invalidSourceURL)
            let stagedURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent("Library", isDirectory: true)
                .appendingPathComponent("Sounds", isDirectory: true)
                .appendingPathComponent("zmux-custom-notification-sound.caf", isDirectory: false)
    ...
    func testNotificationCustomPreparationReportsMissingFile() {
        let missingPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-missing-\(UUID().uuidString).wav", isDirectory: false)
            .path
    ...
    func testFocusedTerminalSuppressedNotificationRunsCustomCommand() throws {
        ...
        let commandOutputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-notification-command-\(UUID().uuidString).txt", isDirectory: false)
        ...
        defaults.set("none", forKey: NotificationSoundSettings.key)
        defaults.set(
            "printf '%s\\n%s\\n%s' \"$ZMUX_NOTIFICATION_TITLE\" \"$ZMUX_NOTIFICATION_SUBTITLE\" \"$ZMUX_NOTIFICATION_BODY\" > '\(commandOutputURL.path)'",
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "zmux DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "zmux DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "zmux DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}
