...
var zmuxUnitTestInspectorAssociationKey: UInt8 = 0
var zmuxUnitTestInspectorOverrideInstalled = false
var zmuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = false
var zmuxUnitTestWKWebViewPerformKeyEquivalentHook: ((WKWebView, NSEvent) -> Bool?)?

extension ZmuxWebView {
    @objc func zmuxUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &zmuxUnitTestInspectorAssociationKey) as? NSObject
    }
}

extension WKWebView {
    @objc func zmuxUnitTest_performKeyEquivalent(with event: NSEvent) -> Bool {
        if let hook = zmuxUnitTestWKWebViewPerformKeyEquivalentHook,
           let result = hook(self, event) {
            return result
        }
        return zmuxUnitTest_performKeyEquivalent(with: event)
    }

    func zmuxSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &zmuxUnitTestInspectorAssociationKey,
            inspector,
...
func installZmuxUnitTestInspectorOverride() {
    guard !zmuxUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        ZmuxWebView.self,
        #selector(ZmuxWebView.zmuxUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install ZmuxWebView _inspector test override")
    }

    zmuxUnitTestInspectorOverrideInstalled = true
}

func installZmuxUnitTestWKWebViewPerformKeyEquivalentOverride() {
    guard !zmuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled else { return }

    let originalSelector = #selector(NSResponder.performKeyEquivalent(with:))
    let swizzledSelector = #selector(WKWebView.zmuxUnitTest_performKeyEquivalent(with:))
    ...
    zmuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = true
}
...
final class ZmuxWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = ZmuxWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = ZmuxWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = ZmuxWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "ZmuxWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
    ...
    func testCmdFDoesNotPreflightIntoPageWhenWebInspectorResponderIsFocused() {
        _ = NSApplication.shared
        installZmuxUnitTestWKWebViewPerformKeyEquivalentOverride()
        ...
        zmuxUnitTestWKWebViewPerformKeyEquivalentHook = { currentWebView, event in
        ...
        window.makeKeyAndOrderFront(nil)
        defer {
            zmuxUnitTestWKWebViewPerformKeyEquivalentHook = nil
            window.orderOut(nil)
        }
        ...
        XCTAssertEqual(
            ...
            0,
            "Did not expect ZmuxWebView to preflight Cmd+F into page content while Web Inspector is focused"
        )
    }
...
@MainActor
final class ZmuxWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = ZmuxWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zmux-browser-history-\(UUID().uuidString)", isDirectory: true)
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installZmuxUnitTestInspectorOverride()
    }
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.zmuxSetUnitTestInspector(inspector)
...
@MainActor
final class ZmuxWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            ZmuxWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(ZmuxWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToZmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToZmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInZmuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToZmuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInZmuxBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInZmuxBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInZmuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInZmuxBrowserValue(defaults: defaults))
    }
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/zmux-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/zmux-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/zmux-local-test.html"))
