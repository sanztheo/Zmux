...
final class AppDelegateWindowContextRoutingTests: XCTestCase {
    private func makeMainWindow(id: UUID) -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("zmux.main.\(id.uuidString)")
        return window
    }
    ...
    func testApplicationOpenURLsIgnoresBundleSelfPaths() {
        ...
        let embeddedExecutableURL = Bundle.main.bundleURL
            .appendingPathComponent("Contents/MacOS/zmux", isDirectory: false)
...
final class AppDelegateLaunchServicesRegistrationTests: XCTestCase {
    func testScheduleLaunchServicesRegistrationDefersRegisterWork() {
        ...
        app.scheduleLaunchServicesBundleRegistrationForTesting(
            bundleURL: URL(fileURLWithPath: "/tmp/../tmp/zmux-launch-services-test.app"),
            scheduler: { work in
                scheduledWork = work
            },
            register: { _ in
...
final class InternalTabDragBundleDeclarationTests: XCTestCase {
    ...
    func testAppBundleExportsInternalDragTypes() {
        ...
        XCTAssertTrue(
            exported.contains("com.zmux.sidebar-tab-reorder"),
...
final class FileDropOverlayViewTests: XCTestCase {
    ...
    func testOverlayForwardsFullDragLifecycleToPortalHostedBrowserWebView() {
        ...
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("zmux.test.drag.\(UUID().uuidString)"))
...
final class ApplicationAccessibilityHierarchyCacheTests: XCTestCase {
    ...
    func testRepeatedWindowsQueriesReuseSingleHierarchyBuildUntilStateChanges() {
        ...
        let cache = ZmuxApplicationAccessibilityHierarchyCache()
        let state = ZmuxApplicationAccessibilityHierarchyCache.StateToken(windows: [firstWindow, secondWindow])
        var buildCount = 0
    ...
    func testChangedStateTokenInvalidatesCachedHierarchySnapshot() {
        ...
        let cache = ZmuxApplicationAccessibilityHierarchyCache()
        let initialState = ZmuxApplicationAccessibilityHierarchyCache.StateToken(windows: [window])
        let updatedState = ZmuxApplicationAccessibilityHierarchyCache.StateToken(windows: [window, otherWindow])
        var buildCount = 0
    ...
    func testWindowCloseNotificationInvalidatesCache() {
        ...
        let cache = ZmuxApplicationAccessibilityHierarchyCache(notificationCenter: center)
        let state = ZmuxApplicationAccessibilityHierarchyCache.StateToken(windows: [window])
        var buildCount = 0
