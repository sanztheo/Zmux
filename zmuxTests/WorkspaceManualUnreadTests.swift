...
final class CommandPaletteSwitcherSearchIndexerTests: XCTestCase {
    func testKeywordsIncludeDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/Users/example/dev/zmuxterm-hq/worktrees/feat-cmd-palette"],
        ...
        XCTAssertTrue(keywords.contains("/Users/example/dev/zmuxterm-hq/worktrees/feat-cmd-palette"))
    ...
    func testFuzzyMatcherMatchesDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/zmuxterm/worktrees/issue-123-switcher-search"],
    ...
    func testSurfaceDetailOutranksWorkspaceDetailForPathToken() throws {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/worktrees/zmux"],
        ...
        let workspaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "zmux", candidates: workspaceKeywords)
        )
        let surfaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "zmux", candidates: surfaceKeywords)
        )
