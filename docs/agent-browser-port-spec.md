...
This document tracks implemented behavior and remaining parity gaps for the zmux browser port.

## Goals

1. Provide an LLM-friendly browser automation API in zmux with stable handles.
...
1. `./scripts/run-tests-v1.sh` passes on `zmux-vm`.
2. `./scripts/run-tests-v2.sh` passes on `zmux-vm`.
...
6. `browser.tab.*` compatibility aliases mapped to zmux surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in zmux context)
...
Primary form:
```bash
zmux browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
zmux browser <surface-id> <agent-browser-style-command...>
```
...
zmux identify
zmux capabilities
zmux browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
zmux trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/zmux.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/zmux.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for zmux browser parity (different transport/runtime).

### Implemented zmux Browser Suites
...
1. `ssh zmux-vm 'cd /Users/zmux/GhosttyTabs && ./scripts/run-tests-v2.sh'`
2. `ssh zmux-vm 'cd /Users/zmux/GhosttyTabs && ./scripts/run-tests-v1.sh'`

## Decision Log (Locked - February 12, 2026)

1. `zmux browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is zmux-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `zmux browser` or gate them behind a second rollout phase.
