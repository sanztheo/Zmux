...
This is the scoped todo list for making the Cloud VM backend production-ready with application logic running in the existing Vercel `manaflow/zmux` project.

## Current State

- Vercel project exists: `manaflow/zmux`.
- Vercel root directory is `web`.
- Production URL is `https://zmux.com`.
- Vercel custom `staging` environment exists for the `manaflow/zmux` project and tracks the
  `staging` git branch.
...
- A separate `manaflow/zmux-staging` Vercel project exists for staging.
...
- [x] Copy minimal DB migration variables from Vercel into both GitHub Cloud VM environments:
  ...
  - `PGUSER`
  - `PGDATABASE`
  - `ZMUX_DB_SSL_REJECT_UNAUTHORIZED`
...
- [ ] Publish a new Freestyle snapshot with zmuxd-remote started with `--rpc-auth-lease-file`.
...
- [x] Staging migration role is scoped to `repo:manaflow-ai/zmux:environment:cloud-vm-staging` and the staging Aurora cluster resource id.
- [x] Production migration role is scoped to `repo:manaflow-ai/zmux:environment:cloud-vm-production` and the production Aurora cluster resource id.
- [x] Staging and production Cloud VM default provider are set to E2B.
- [x] Freestyle creates are disabled in staging and production with `ZMUX_VM_FREESTYLE_ENABLED=0`.
...
- `RESEND_API_KEY`
- `ZMUX_FEEDBACK_FROM_EMAIL`
- `ZMUX_FEEDBACK_RATE_LIMIT_ID`
- `NEXT_PUBLIC_STACK_PROJECT_ID`
- `NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY`
- `STACK_SECRET_SERVER_KEY`
...
- [x] Add a global VM create kill switch, `ZMUX_VM_CREATE_ENABLED`.
- [x] Add per-provider kill switches:
  - `ZMUX_VM_E2B_ENABLED`
  - `ZMUX_VM_FREESTYLE_ENABLED`
- [x] Set kill switches to enabled values in `manaflow/zmux` production and
  `manaflow/zmux-staging` production.
...
- [ ] Set `ZMUX_VM_DEFAULT_PROVIDER` in Vercel development, preview, and production.
- [ ] Set `E2B_API_KEY` in Vercel preview and production.
- [ ] Set `FREESTYLE_API_KEY` in Vercel preview and production.
- [ ] Set `E2B_ZMUXD_WS_TEMPLATE` in Vercel preview and production.
...
- [ ] Confirm Stack Auth callback and trusted domains include:
  ...
  - local `ZMUX_PORT` development callback URLs
- [ ] Redeploy Vercel preview after env injection.
- [ ] Smoke test preview:
  - `zmux auth login`
  - `zmux vm new --provider freestyle`
  - `zmux vm new --provider e2b`
  - `zmux vm attach <id>`
...
- [ ] Keep local Stack/web runtime secrets in `~/.secrets/zmuxterm-dev.env`.
- [ ] Keep production Stack/web runtime secrets in `~/.secrets/zmuxterm.env`.
- [ ] Keep provider image-build secrets in `~/.secrets/zmux.env`.
- [ ] Add runtime VM vars to the relevant `~/.secrets/zmuxterm*.env` file:
  - `ZMUX_VM_DEFAULT_PROVIDER`
  - `ZMUX_VM_CREATE_ENABLED`
  - `ZMUX_VM_E2B_ENABLED`
  - `ZMUX_VM_FREESTYLE_ENABLED`
  - `E2B_ZMUXD_WS_TEMPLATE`
  - `FREESTYLE_SANDBOX_SNAPSHOT`
  - Axiom/OpenTelemetry vars
- [x] Document the split between `~/.secrets/zmuxterm-dev.env`, `~/.secrets/zmuxterm.env`, and
  `~/.secrets/zmux.env` in `AGENTS.md`.
...
- [x] Build E2B template and Freestyle snapshot from the same zmuxd-remote commit.
- [x] Record artifact provenance:
  - zmuxd-remote git commit
  - zmuxd-remote build command
  - binary SHA256
...
- [x] Add ZMUX_PORT-derived local Postgres so parallel worktrees do not collide.
...
- [ ] Connect the Vercel Marketplace Aurora resource to `manaflow/zmux` for both `staging` and production so these env names are present:
  - `AWS_ROLE_ARN`
  - `AWS_REGION`
  - `PGHOST`
  - `PGPORT`
...
- [ ] Add `ZMUX_VM_CREATE_ENABLED` and provider kill-switch docs.
...
- [ ] Add a future `zmux-infra` or `backend-rollout` skill so agents follow this workflow consistently.
