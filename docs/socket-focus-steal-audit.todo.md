...
## Goal
Ensure commands run through the zmux Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `ZMUX_TAB_ID` / `ZMUX_SURFACE_ID` / `ZMUX_WORKSPACE_ID` when flags omitted).
